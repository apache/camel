...
The Debezium PostgresSQL Connector component supports 84 options, which are listed below.
...
| *converters* (postgres) | Optional list of custom converters that would be used instead of default ones. The converters are defined using '<converter.prefix>.type' config option and configured using options '<converter.prefix>.<option>' |  | String
