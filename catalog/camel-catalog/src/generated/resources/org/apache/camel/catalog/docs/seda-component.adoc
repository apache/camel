...
The consumers share the queue by its name, so they can use different consumer options,
such as `seda:foo?multipleConsumers=true&concurrentConsumers=5`, and still each receive
their own copy of the message. The `multipleConsumers=true` option must be specified
on every consumer.
