...
When using namespaces in XML DSL then it is different, as you set up the namespaces
...
Declaring the prefixes on the XML root tag only works when the route itself is written in XML.
To share one set of prefix to URI mappings across several expressions, register a
`org.apache.camel.support.builder.Namespaces` bean and refer to it with the `namespacesRef`
option:
...
URI as the value. In Java DSL there is no need for this, as the `Namespaces` builder is passed
to the expression directly, as shown above.

If an expression declares namespaces inline as well, then the inline namespaces win and
