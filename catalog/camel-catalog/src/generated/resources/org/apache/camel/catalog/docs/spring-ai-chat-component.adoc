...
When using Camel Spring Boot, Spring AI's auto-configuration automatically discovers `@Tool`-annotated
methods from Spring beans via `ToolCallbackResolver`. Simply define your tool as a Spring bean and
...
Outside Spring Boot (e.g., in plain Camel or tests), there is no `ToolCallbackResolver`.
...
Tool names are resolved via Spring AI's `ToolCallbackResolver`. You can combine `toolNames` with `tags` —
