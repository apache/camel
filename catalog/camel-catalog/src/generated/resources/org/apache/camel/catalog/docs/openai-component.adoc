...
For Server-Sent Events to web clients (platform-http), structured extraction pipelines, and when to stream through Camel vs. a dedicated handler, see xref:ai-llm-integration-guide.adoc#Streaming responses[LLM Integration Guide — Streaming responses].
...
TIP: For extraction tasks (resumes, invoices, classification), prefer `jsonSchema` or `outputClass` instead of hand-written JSON parsing. Use low `temperature` (0.0–0.2). See xref:ai-llm-integration-guide.adoc#Structured output (recommended for extraction)[LLM Integration Guide — Structured output].
