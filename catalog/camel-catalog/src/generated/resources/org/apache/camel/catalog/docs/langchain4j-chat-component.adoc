...
`hugging-face`, or the fully qualified class name of any LangChain4j model class with a `builder()`. The LangChain4j
module of the provider (`dev.langchain4j:langchain4j-ollama`, `dev.langchain4j:langchain4j-open-ai`, ...) must be
on the classpath; Camel JBang downloads it. The common options are `modelName`, `baseUrl`, `apiKey`, `temperature`
