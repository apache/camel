...
| *verboseLogging* (producer) | JSCH is verbose logging out of the box. Therefore we turn the logging down to DEBUG logging by default. But setting this option to <tt>true turns on the verbose logging again. | false | boolean
...
| *connectTimeout* (advanced) | Sets the connect timeout for waiting for a connection to be established <p/> Used by both FTPClient and JSCH | 10000 | int
...
| *timeout* (advanced) | Sets the data timeout for waiting for reply <p/> Used only by FTPClient | 30s | int
