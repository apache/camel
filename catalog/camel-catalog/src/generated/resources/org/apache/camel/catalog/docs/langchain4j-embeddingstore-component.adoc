...
* **REMOVE** - Delete embeddings by single ID, collection of IDs, metadata filter, or clear the entire store.
...
For RAG ingestion pipelines, batch adding is significantly more efficient. Send a `List<Embedding>` via the `CamelLangChain4jEmbeddingsEmbeddings` header:

[source,java]
----
from("direct:batch-store")
    // Assuming the body is a List<String> or List<TextSegment>
    .to("langchain4j-embeddings:embed")
...
Remove all embeddings from the store by sending a REMOVE with no body and no filter:

[source,java]
----
from("direct:clear-store")
    .to("langchain4j-embeddingstore:myStore?action=REMOVE");
----
