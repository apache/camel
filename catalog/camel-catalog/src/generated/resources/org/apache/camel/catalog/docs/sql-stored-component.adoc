...
The Simple expression may use functions with a single argument, such as `${val(1)}` or `${bodyAs(String)}`.
However, the template is not a full Simple expression: functions with multiple, comma-separated arguments (such as `${replace(a,b)}`)
and nested functions are not supported, as the comma is used to separate the parameters of the stored procedure.
If you need such expressions, then compute the value beforehand, for example into a header, and refer to it with `${header.name}`.
