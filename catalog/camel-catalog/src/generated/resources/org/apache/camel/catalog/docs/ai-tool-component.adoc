...
These hints are **advisory only** — Camel does not enforce them. Untrusted MCP clients must not rely on them for security decisions.
...
Supported options: `title`, `readOnlyHint`, `destructiveHint`, `idempotentHint`, and `openWorldHint`. When omitted, the hint is not published to MCP clients.
