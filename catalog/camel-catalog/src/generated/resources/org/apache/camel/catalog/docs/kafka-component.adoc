...
| *pollOnError* (consumer) | What to do if kafka threw an exception while polling for new messages. There are 5 enums and the value can be one of: RETRY, RECONNECT, STOP_CONSUMER, ROUTE_WITH_EXCEPTION, DISCARD_MESSAGE |  | PollOnError
