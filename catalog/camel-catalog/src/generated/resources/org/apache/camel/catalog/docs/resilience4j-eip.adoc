...
When `asynchronous` is enabled, the circuit breaker uses Resilience4j's CompletionStage-based
decorators, which release the caller thread immediately and complete processing asynchronously.

This is most valuable when the downstream processor supports asynchronous processing
(e.g., Netty HTTP, Kafka), as it avoids blocking threads while waiting for responses.
...
NOTE: When using asynchronous mode with timeout, the `timeoutExecutorService` must be
a `ScheduledExecutorService`. If not provided, Camel will automatically create one.
If a custom executor is provided that is not a `ScheduledExecutorService`, the route will
