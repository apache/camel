...
* *Restrict the accepted body types in Camel.* The consumer `deserializationFilter`
option applies a JEP-290 `ObjectInputFilter` pattern to the type of the message
body before the exchange is routed, rejecting bodies whose type is not allowed.
This is a defense-in-depth allow-list applied _after_ JGroups has deserialized
the message; it complements, but does not replace, the JVM-wide `jdk.serialFilter`
