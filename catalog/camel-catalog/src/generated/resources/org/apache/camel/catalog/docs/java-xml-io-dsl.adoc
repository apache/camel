...
(Lombok `@Builder`, Immutables, LangChain4j, the AWS SDK v2, protobuf, the JDK `HttpClient` and many others)
needs no `builderClass` and `builderMethod`: Camel calls the `builder()` method, sets the properties on the returned
builder, and creates the bean with its `build()` method (or, when there is no `build()`, its single public no-arg
...
Set `builderMethod` when the builder has several methods that return the type, or the method has another name.
