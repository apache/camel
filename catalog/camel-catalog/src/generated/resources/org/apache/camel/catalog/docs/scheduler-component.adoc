...
:description: Generate messages in specified intervals using <code>java.util.concurrent.ScheduledExecutorService.
:since: 2.15
:supportLevel: Stable
...
| *concurrentTasks* (scheduler) | Number of threads used by the scheduling thread pool. <p/> Is by default using a single thread | 1 | int
|===
