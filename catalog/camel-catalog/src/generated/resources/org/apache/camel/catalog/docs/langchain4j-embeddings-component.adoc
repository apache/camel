...
The component supports both single and batch embedding operations:

* **Single**: Send a `String` or `TextSegment` body to compute a single embedding. The result is available via the `CamelLangChain4jEmbeddingsEmbedding` header.
* **Batch**: Send a `List<String>` or `List<TextSegment>` body to embed all items in one API call using `EmbeddingModel.embedAll()`. The resulting `List<Embedding>` is set as the message body and also available via the `CamelLangChain4jEmbeddingsEmbeddings` header.

Batch embedding is significantly more efficient for RAG ingestion pipelines where many text segments need to be embedded at once.
...
// Result: Embedding in the CamelLangChain4jEmbeddingsEmbedding header
----
...
[source,java]
----
from("direct:embed-batch")
    .setBody(constant(List.of("Hello", "World", "Test")))
    .to("langchain4j-embeddings:embed");
// Result: List<Embedding> as body and in the CamelLangChain4jEmbeddingsEmbeddings header
----
