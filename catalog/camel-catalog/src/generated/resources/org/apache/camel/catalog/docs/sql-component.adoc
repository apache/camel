...
key `CamelSqlParameters`. This allows the SQL component to work
...
The producer supports to use a different `javax.sql.DataSource` than the default configured on the component,
to make it possible to dynamic query another database. This requires to specify a header in the `Exchange` that contains
...
NOTE: Remember to remove the header if you use SQL component later during routing and don't want to keep using the custom
data source.
