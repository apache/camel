...
In real-time mode the document changes reported by the listener are buffered until the next poll picks them up. That
buffer is unbounded by default, so a collection changing faster than the route consumes it makes the buffer grow, and
it keeps growing while the route is suspended. Set `maxPendingChanges` to bound it: when the buffer is full the oldest
buffered change is discarded and a warning is logged, which leaves the route with the most recent state of the
collection.
