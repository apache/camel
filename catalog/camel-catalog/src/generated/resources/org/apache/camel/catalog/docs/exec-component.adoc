...
Since Camel 4.20, the `CamelExecCommand*` headers that override URI options are
disabled by default. This prevents untrusted message sources from redirecting
...
When `allowControlHeaders` is `false` (the default), any `CamelExecCommand*`
headers on the exchange are ignored and a WARN is logged. URI parameters such
as `args` continue to work as before.
