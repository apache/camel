= Test Main JUnit6 Component
:doctitle: Test Main JUnit6
:shortname: test-main-junit6
:artifactid: camel-test-main-junit6
...
The `camel-test-main-junit6` module is used for unit testing Camel launched in Standalone mode with Camel Main.

This module proposes two approaches to configure and launch Camel like a Camel Main application for testing purpose.

* *Legacy*: This approach consists of extending the base class `org.apache.camel.test.main.junit6.CamelMainTestSupport` and overriding the appropriate methods to enable or disable a feature.

* *Annotation*: This approach consists of annotating the test classes with `org.apache.camel.test.main.junit6.CamelMainTest` with the appropriate attributes to enable or disable a feature.
...
[source,java]
----
import org.apache.camel.test.main.junit6.ReplaceInRegistry;
...
The test class needs to implement the interface `org.apache.camel.test.main.junit6.DebuggerCallback` to enable the feature.
...
The annotation-based approach supports natively https://junit.org/junit6/docs/current/user-guide/#writing-tests-nested[Nested tests].
