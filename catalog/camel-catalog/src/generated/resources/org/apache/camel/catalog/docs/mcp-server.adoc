...
xref:components::ai-tool-component.adoc[ai-tool] component as tools of a
...
xref:components::openai-component.adoc[camel-openai] MCP client:
...
* *Authentication*: the MCP endpoint is served through the main HTTP server
  ...
  xref:components:others:oauth.adoc[camel-oauth] for resource-server style
