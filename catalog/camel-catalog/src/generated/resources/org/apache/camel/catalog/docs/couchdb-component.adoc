...
* As a producer, can save, update, delete (by using `CamelCouchDbMethod` with `DELETE` value)
documents and get documents by id (by using `CamelCouchDbMethod` with GET value) into CouchDB.
...
ignored. That means, for example, if you set CamelCouchDbId as a header, it
