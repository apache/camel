...
Starting with Camel 4.19, the hybrid signature, hybrid KEM, and xref:dataformats:pqc-dataformat.adoc[PQC DataFormat] wire formats include self-describing algorithm metadata. This makes the binary output portable and verifiable without out-of-band knowledge of which algorithms were used.
