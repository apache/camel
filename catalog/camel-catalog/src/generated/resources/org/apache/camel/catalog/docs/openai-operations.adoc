...
NOTE: The `illicit` and `illicit/violent` categories are only returned by the `omni-moderation-*` models, so they are
absent from the category map when an older moderation model is used.
