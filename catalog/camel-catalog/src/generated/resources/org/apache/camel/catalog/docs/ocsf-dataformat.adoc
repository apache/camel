...
NOTE: The script first tries to load the requested OCSF version from the
bundled `ocsf-json-schema` Python package. If the version is not bundled
yet (typical for the most recent OCSF release), the script automatically
downloads the schema from `https://schema.ocsf.io/\{version}/export/schema`.
