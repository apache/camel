...
| generateHtmlXlsxReport | false | Whether to generate a coverage-report in HTML and XLSX format. When enabled (along with `generateJacocoXmlReport`) it will generate the file: `target/site/site/route-coverage/html/index.html` and the file `target/site/site/route-coverage/xslx/index.xlsx`
|===
