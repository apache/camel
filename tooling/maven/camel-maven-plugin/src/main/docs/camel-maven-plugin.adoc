...
=== Make SonarQube pick up camel-route-coverage

To allow sonarQube to show route-coverage in camel-routes you need to enable `generateJacocoXmlReport` 
(see options below) and tell sonarRunner to pick up the resulting xml-files. When using the sonar-maven-plugin
this is done by setting the following properties in your `pom.xml`

----
<sonar.sources>src/main</sonar.sources>		
<sonar.coverage.jacoco.xmlReportPaths>target/site/jacoco/jacoco.xml,target/site/jacoco/xmlJacoco.xml</sonar.coverage.jacoco.xmlReportPaths>
----
...
| generateJacocoXmlReport | false | *Camel 2.24* Whether to generate a coverage-report in Jacoco XML format, when enabled it will generate a file: `target/site/jacoco/xmlJacoco.xml`
|===
