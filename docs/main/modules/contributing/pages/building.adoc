...
The mojo requires compiled classes and packaged JARs for all reactor modules.
Run `package -DskipTests` first:

[source,bash]
----
# Full reactor — report mode (default, includes test-scope)
./mvnw package -DskipTests -Dlicense.skip eu.maveniverse.maven.plugins:pilot-plugin:0.4.0:dependencies -Pdep-check

# Single module (after a full reactor package)
cd components/camel-ftp
./mvnw eu.maveniverse.maven.plugins:pilot-plugin:0.4.0:dependencies -Pdep-check
----
...
[source,bash]
----
./mvnw package -DskipTests -Dlicense.skip eu.maveniverse.maven.plugins:pilot-plugin:0.4.0:dependencies -Pdep-check -Dpilot.action=check
----

CI also runs this check automatically on pull requests targeting `main` (see the *Dependency Analysis* workflow).