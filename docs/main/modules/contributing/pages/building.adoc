...
Run `compile` first (test-scope analysis is skipped by default, since many modules depend on
`camel-test-spring-junit6` which is not resolvable in a clean checkout without a prior install):
