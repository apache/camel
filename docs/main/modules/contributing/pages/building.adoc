...
The opt-in `-Pdep-check` profile runs `maven-dependency-plugin:analyze-only` to detect *used-but-undeclared* and *unused-but-declared* dependencies.
It is non-blocking by default (`failOnWarning=false`), so it only reports findings without failing the build.

To run it on the full reactor:

[source,bash]
----
mvn verify -Pdep-check -DskipTests -Dquickly
----
...
mvn verify -Pdep-check -DskipTests
----
