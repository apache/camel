...
| formatName |  | String | The name of the format to use, the default value is CSVFormat.DEFAULT. There are 6 enums and the value can be one of: DEFAULT, EXCEL, INFORMIX_UNLOAD, INFORMIX_UNLOAD_CSV, MYSQL, RFC4180
...
|`CamelCsvHeaderRecord` |The header record provided or collected from the CSV.
...
When an empty string array is provided for the header, the format will collect the headers into
a message header called CamelCsvHeaderNames.

[source,java]
--------------------------------------------
CsvDataFormat csv = new CsvDataFormat();
csv.setHeader(new String[0]);

from("direct:start")
  .unmarshal(csv)
  .log("${header[CamelCsvHeaderNames]}");
--------------------------------------------
...
include::{page-component-version}@camel-spring-boot::page$csv-starter.adoc[]
