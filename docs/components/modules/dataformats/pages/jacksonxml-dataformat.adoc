...
| include |  | String | If you want to marshal a pojo to JSON, and the pojo has some fields with null values. And you want to skip these null values, you can set this option to <tt>NON_NULL
...
| enableFeatures |  | String | Set of features to enable on the Jackson <tt>com.fasterxml.jackson.databind.ObjectMapper. <p/> The features should be a name that matches a enum from <tt>com.fasterxml.jackson.databind.SerializationFeature, <tt>com.fasterxml.jackson.databind.DeserializationFeature, or <tt>com.fasterxml.jackson.databind.MapperFeature <p/> Multiple features can be separated by comma
| disableFeatures |  | String | Set of features to disable on the Jackson <tt>com.fasterxml.jackson.databind.ObjectMapper. <p/> The features should be a name that matches a enum from <tt>com.fasterxml.jackson.databind.SerializationFeature, <tt>com.fasterxml.jackson.databind.DeserializationFeature, or <tt>com.fasterxml.jackson.databind.MapperFeature <p/> Multiple features can be separated by comma
| allowUnmarshallType | false | Boolean | If enabled then Jackson is allowed to attempt to use the CamelJacksonUnmarshalType header during the unmarshalling. <p/> This should only be enabled when desired to be used.
