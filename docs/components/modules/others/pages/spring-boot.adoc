...
:docTitle: Spring Boot
:artifactId: camel-spring-boot
:description: Using Camel with Spring Boot
:since: 2.15
...
| *camel.springboot.stream-caching* | *Deprecated* Sets whether stream caching is enabled or not (deprecated use stream-caching-enabled instead). Default is false. | false | Boolean
| *camel.springboot.stream-caching-spool-chiper* | *Deprecated* Sets a stream caching chiper name to use when spooling to disk to write with encryption. By default the data is not encrypted. @deprecated use streamCachingSpoolCipher |  | String
| *camel.springboot.use-m-d-c-logging* | *Deprecated* To turn on MDC logging (deprecated use use-mdc-logging instead) | false | Boolean
|===
