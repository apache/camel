...
| *configurationUri* (producer) | Location of MyBatis xml configuration file. <p/> The default value is: SqlMapConfig.xml loaded from the classpath | SqlMapConfig.xml | String
...
|===
| Name | Description | Default | Type
| *executorType* (producer) | The executor type to be used while executing statements. <ul> <li>simple - executor does nothing special. <li>reuse - executor reuses prepared statements. <li>batch - executor reuses statements and batches updates. There are 3 enums and the value can be one of: SIMPLE, REUSE, BATCH | SIMPLE | ExecutorType
