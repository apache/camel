...
| *customHttpClient* (common) | (Optional) Set custom HTTP client for authenticated access. |  | OkHttpClient
| *destinationServerSide Encryption* (common) | (Optional) Server-side encryption for source object while copy/move objects. |  | ServerSideEncryptionCustomerKey
...
| *objectLock* (common) | (Optional) Set when creating new bucket. | false | boolean
...
| *proxyPort* (common) | (Optional) TCP/IP port number. 80 and 443 are used as defaults for HTTP and HTTPS. |  | Integer
| *region* (common) | (Optional) The region in which Minio client needs to work. When using this parameter, the configuration will expect the lowercase name of the region (for example ap-east-1) You'll need to use the name Region.EU_WEST_1.id() |  | String
| *secure* (common) | (Optional) Flag to indicate to use secure connection to minio service or not. | false | boolean
| *serverSideEncryption* (common) | (Optional) Server-side encryption. |  | ServerSideEncryptionCustomerKey
...
| *destinationObjectName* (consumer) | (Optional) Source object name. |  | String
...
| *length* (consumer) | (Optional) Number of bytes of object data from offset. |  | long
...
| *offset* (consumer) | (Optional) Start byte position of object data. |  | long
| *prefix* (consumer) | (Optional) Object name starts with prefix. |  | String
| *recursive* (consumer) | (Optional) List recursively than directory structure emulation. | false | boolean
...
| *useVersion1* (consumer) | (Optional) when true, version 1 of REST API is used. | false | boolean
...
|===
| Name | Description | Default | Type
| *bucketNameOrArn* | *Required* Qualified url |  | String
|===
