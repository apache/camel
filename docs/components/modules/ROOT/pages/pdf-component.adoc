...
|===
| Name | Description | Default | Type
| *operation* | *Required* Operation type. There are 3 enums and the value can be one of: create, append, extract-text |  | PdfOperation
|===
...
| *textProcessingFactory* (producer) | Text processing to use. autoFormatting: Text is getting sliced by words, then max amount of words that fits in the line will be written into pdf document. With this strategy all words that doesn't fit in the line will be moved to the new line. lineTermination: Builds set of classes for line-termination writing strategy. Text getting sliced by line termination symbol and then it will be written regardless it fits in the line or not. There are 2 enums and the value can be one of: auto-formatting, line-termination | line-termination | TextProcessingFactory
