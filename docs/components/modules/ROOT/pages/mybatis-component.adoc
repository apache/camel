...
| *statementType* (producer) | Mandatory to specify for the producer to control which kind of operation to invoke. There are 8 enums and the value can be one of: select-one, select-list, insert, insert-list, update, update-list, delete, delete-list |  | StatementType
