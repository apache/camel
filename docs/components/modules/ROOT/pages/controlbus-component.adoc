...
|===
| Name | Description | Default | Type
| *action* (producer) | *status*: to get the status of the route as output in the message body. +
*start*, *stop*, or *status*: To either start or stop a route. +
*suspend* and *resume*: (from Camel 2.11.1 onwards) to either suspend or resume a route. +
*stats*: (from Camel 2.11.1 onwards) you can use this to get performance statics returned in XML format; the routeId option can be used to define which route to get the performance stats for, if routeId is not defined, then you get statistics for the entire CamelContext. 
*restart*: this action will restart the route. |  | String
...
performance statistics per route, or for the
CamelContext. For example to get the statistics for
