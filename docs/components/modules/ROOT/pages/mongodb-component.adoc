...
The exchange body will contain the full document of any change, with the operation type specified in the `CamelMongoDbStreamOperationType`
Exchange Header and document id specified in an `_id` header. In the case of delete operations, the exchange body will instead be a Document type
containing only the _id field of the deleted document.
