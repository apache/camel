...
| *operation* (producer) | The operation to perform. It can be listFunctions, getFunction, createFunction, deleteFunction or invokeFunction. There are 18 enums and the value can be one of: list-functions, get-function, create-alias, delete-alias, get-alias, list-aliases, create-function, delete-function, invoke-function, update-function, create-event-source-mapping, delete-event-source-mapping, list-event-source-mapping, list-tags, tag-resource, untag-resource, publish-version, list-versions | invoke-function | LambdaOperations
