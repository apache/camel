...
The pattern option is used for matching. It uses the same rules as xref:{eip-vc}:eips:intercept.adoc[Intercept], which is applied in the following order:
...
Suppose you use the xref:{eip-vc}:eips:split-eip.adoc[Split] EIP in a route; then you can use `weaveByType` to select this EIP.
...
Due to that route has only one xref:{eip-vc}:eips:split-eip.adoc[Split] EIP, you can use `weaveByType` to find this single
...
Here we weave and select the xref:{eip-vc}:eips:split-eip.adoc[Split] EIP and weave in a message transformation, that
...
Given the following route having two branches in the xref:{eip-vc}:eips:content-based-router-eip.adoc[Content Based Router] EIP:
...
Given the following route which has multiple xref:{eip-vc}:eips:filter-eip.adoc[Filter] EIP,
