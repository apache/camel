...
* *No shield against transitive-dependency CVEs that are not reachable
  through a Camel-exposed code path.* If a JAR Camel pulls in has a CVE
  and Camel never reaches the vulnerable code, the report belongs
  upstream. See the third-party-dependency entry in _Out of scope_.
...
* *"Component X depends on a JAR with CVE-Z, therefore Camel is
  ...
  _Out of scope_, unless the report demonstrates a Camel-exposed code
  path that reaches the vulnerable code. Default disposition is to
  forward upstream.
...
* *"The developer console, the backlog debugger, JMX, Jolokia or
  `camel-management` exposes operations that execute code or read
  `Exchange` state."* Management surfaces; the trust boundary is the
