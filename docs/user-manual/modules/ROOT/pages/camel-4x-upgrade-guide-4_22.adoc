...
`camel run`, `camel dev`, and `camel debug` accept `--mcp` to expose JBang dev/diagnostics tools over MCP
streamable HTTP on the management server (`/mcp` by default). The feature is gated as developer-only
(`insecure:dev`) via `camel.jbang.mcp` and `camel.management.mcpEnabled`. It uses `VertxMcpServerEngine`
from `camel-mcp-server` on the management HTTP router — distinct from business `ai-tool` MCP exposed on the
