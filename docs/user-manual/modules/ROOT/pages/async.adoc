...
xref:components::http4-component.adoc[HTTP] protocol that we use every day when we surf the
...
message over xref:components::http4-component.adoc[HTTP] to Camel. The client application will
...
still use xref:components::http4-component.adoc[HTTP] despite http being
...
Suppose we want to call a xref:components::http4-component.adoc[HTTP] service but it is usually
...
then do other stuff while the slow xref:components::http4-component.adoc[HTTP] service is
processing our request. And then a bit later we can use the `Future`
handle to get the response from the xref:components::http4-component.adoc[HTTP] service. Yeah
nice so lets do it:

First we define some routes in Camel. One for the xref:components::http4-component.adoc[HTTP]
...
xref:components::http4-component.adoc[HTTP] service is on route. This allows you to be able to
...
First we define a route in Camel for the xref:components::http4-component.adoc[HTTP] service
...
And then we have the client API where we call the xref:components::http4-component.adoc[HTTP]
...
* xref:async.adoc[Asynchronous Processing]
