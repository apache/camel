...
The value of a Simple expression such as `$\{body}` or `${header.foo}` is passed to the method as-is.
Quotes in the value are kept, and a value that is the text `null` is passed as that text.
Only a parameter written as a quoted String, such as `'World'`, has its quotes removed,
and only the parameter `null` (without quotes) is a `null` value.
