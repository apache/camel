...
The `configure(CamelContext)` method of the ``org.apache.camel.main.MainListener` has been deprecated and will be removed in a future release.  The `afterConfigure(BaseMainSupport)` method should be used instead.
...
- In Camel 3.3.0, camel-jgroups and camel-jgroups-raft are not supported in OSGi anymore.
- In Camel 3.3.0, camel-infinispan is not supported in OSGi anymore.
