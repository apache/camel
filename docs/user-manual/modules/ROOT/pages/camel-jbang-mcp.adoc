...
| List available Camel JBang examples with filtering by name, description, or tag (case-insensitive substring match).
...
| Get the content of a specific file from a Camel JBang example. For bundled examples, returns the file content
...
The assistant calls `camel_catalog_examples` with `filter=rest` and `level=beginner` and returns matching examples
...
The assistant calls `camel_catalog_example_file` with `example=rest-api` and `file=route.camel.yaml` and returns
the file content directly for bundled examples, or a GitHub URL for non-bundled ones.
