...
| `camel_catalog_doc`
| Catalog documentation of a component, data format, language, EIP or built-in bean: description, options (with
  a keyword filter), Maven coordinates, and for a component the rules of its endpoint URI spelled out (which
...
| Validated YAML DSL samples of an EIP or file entry (`onException`, `aggregate`, `split`, `rest`, `beans`) taken
  ...
  construct) or what to do (`read file`, `call service`, `retry`, `batch`). The EIP samples come from the
