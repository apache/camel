...
A new strict JSON Schema variant (`camelYamlDsl-strict.json`) has been added alongside the existing classic
schema (`camelYamlDsl.json`). The strict schema removes all implicit patterns (string shorthands, inline
...
The `YamlValidator` class now accepts a `boolean strict` constructor parameter to validate against the
strict schema.
