...
A new `ErrorRegistry` SPI has been added to `CamelContext`. It captures snapshots of exceptions that occur during
message routing, without retaining references to the original exchange or exception objects.

The registry is disabled by default (opt-in). It can be enabled via:
...
[source,properties]
----
camel.context.error-registry-enabled = true
----

When enabled, error snapshots can be browsed globally or scoped to a specific route. A dev console (`errors`) and
JMX MBean (`ManagedErrorRegistry`) are available for monitoring.
