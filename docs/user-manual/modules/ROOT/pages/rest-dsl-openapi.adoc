...
fragment.
Because `rest-openapi` endpoint URIs are lenient, delegate options may be accepted by
`rest-openapi` even when the selected delegate ignores them. Verify that the selected consumer
