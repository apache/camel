...
In Camel 4.17, you can use `camel cmd send` to send messages directly to infrastructure services that are started with `camel infra run`.
...
The command automatically discovers the running NATS service, reads its connection details from the JSON file (e.g., `~/.camel/infra-nats-{pid}.json`), and constructs the appropriate endpoint URI with the server information.

You can also combine the `--infra` option with a specific endpoint:

[source,bash]
----
$ camel cmd send --endpoint='nats:subject:test' --body=file:payload.json --infra=nats
----

This allows you to send messages to infrastructure services without needing to know or specify the server addresses manually.
