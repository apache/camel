...
NOTE: At the moment, the `load` command is only adding new, so if you load the same a 2nd time, then Camel
will add this as a new duplicate route. This may be improved in the future to make the `load` command
