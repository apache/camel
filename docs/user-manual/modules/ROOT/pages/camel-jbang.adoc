...
|`camel.jbang.info`
| Info console at /observe/info on local HTTP server (port 8080 by default) when running standalone Camel

|`camel.jbang.health`
| Health check at /observe/health on local HTTP server (port 8080 by default) when running standalone Camel
