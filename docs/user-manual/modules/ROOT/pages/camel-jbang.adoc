...
Supported plugins:
 ...
 diagram     diagram     org.apache.camel:camel-jbang-plugin-diagram     Visualize Camel routes using Hawtio
----
...
The `camel diagram` command uses Hawtio to visualize Camel routes.

NOTE: The diagram command is a plugin that must be installed first:

[source,bash]
----
camel plugin add diagram
----

Once installed, you can run route files in the background and open Hawtio in a browser:
...
To attach to an existing integration that is already running:
...
You can export a PNG snapshot using Playwright (requires a Chromium-based browser):

NOTE: The Playwright library is downloaded automatically on first use and cached in your local Maven repository (`~/.m2`).
No manual installation is required — only a Chromium-based browser executable is needed.
...
On macOS with Google Chrome installed, the command looks like:

[source,bash]
----
camel diagram routes.yaml --output=routes.png --playwright-browser-path="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
----

On Linux with Chromium:

[source,bash]
----
camel diagram routes.yaml --output=routes.png --playwright-browser-path=/usr/bin/chromium-browser
----

When using `--output`, Hawtio runs headless and the PNG is captured automatically.

NOTE: PNG export currently supports Chromium-based browsers only (Google Chrome, Chromium, Microsoft Edge).
The `--playwright-browser-path` option (or the `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` environment variable) must point to the browser executable.
