...
When `--dev` is used, Camel also disables `contentCache` on resource-based components that
implement the `org.apache.camel.spi.ContentCacheAware` interface, so that edits to resource files
are picked up on the next message without having to restart the route.

This is driven by `camel.main.routesReloadEnabled` (set automatically by `--dev`). Explicit user
settings on the component, endpoint or URI are always respected: set `contentCache=true` to keep
caching even in dev mode.
