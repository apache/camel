...
|`dumpExchangePropertiesAsXml(nodeId)` |`String`|To dump the properties of the suspended Exchange at the given node in XML format.
...
|`evaluateExpressionAtBreakpoint(nodeId, language, expression)` | `Object`|To evaluate an expression in any supported language (e.g. Simple, CSimple, etc.)
