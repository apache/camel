[[CamelContextAutoConfiguration-CamelContextAutoConfiguration]]
...
1. Configure `CamelContext` (and more, such as components) from xref:properties-component[properties] such as `application.properties|yaml`
...
| MessageHistoryFactory | To use a custom factory for xref:{eip-vc}:eips:message-history.adoc[message history]
...
| ProcessorFactory | To use a custom factory for creating xref:{eip-vc}:eips:enterprise-integration-patterns.adoc[EIP] processors
