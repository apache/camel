...
`contentCache` option on resource-based components (such as `xslt`) whose default is `true`. This
way, edits to resource files are picked up on the next message without having to restart the
route.

User properties (e.g. `camel.component.xslt.contentCache=true`) and explicit endpoint/URI settings
are always respected: set `contentCache=true` to keep caching even when route reload is enabled.
