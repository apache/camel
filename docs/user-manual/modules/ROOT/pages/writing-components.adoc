...
This metadata allows tooling such as IDE plugins, JBang and others to better understand
...
Refer to the xref:endpoint-annotations.adoc[Endpoint Annotations guide for details].
...
TIP: To learn how to build a custom component its recommended to take a look at any of the existing components
from the source code. For a beginner component then look at the log or timer component.