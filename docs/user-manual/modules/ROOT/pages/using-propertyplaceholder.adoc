...
* `aws` - A function to lookup the property from AWS Secrets Manager service.
...
If you want custom properties functions to be easier to use then we recommend making them pluggable.
For example the beer function from above, can be made pluggable simply by adding the `PropertiesFunction` annotations as shown:
...
Then by having the `camel-component-maven-plugin` as part of building the component will
then ensure that this custom properties has necessary source code generated that makes Camel
