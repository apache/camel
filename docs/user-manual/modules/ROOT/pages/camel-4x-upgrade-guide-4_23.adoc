...
=== camel-http, camel-http-common, camel-undertow - property placeholders in the CamelHttpUri header

The HTTP producers no longer resolve property placeholders (`{{...}}`) in the value of the
`CamelHttpUri` header. That header carries message content, while property placeholders are a
route/configuration authoring feature resolved at build time on the endpoint URI written in the
route. This is the same alignment 4.22 applied to `toD` and `enrich`.
...
A `{{...}}` token arriving in the `CamelHttpUri` header is now treated as a literal part of the URI
(and percent-encoded like any other unsafe character in a header-supplied URI) rather than being
expanded. Routes that relied on that expansion must resolve the value before it reaches the header.

This affects `camel-http`, everything built on `camel-http-common` (including `camel-servlet` and
`camel-jetty`) and `camel-undertow`. `camel-http` had a `skipControlHeaders` option that could turn
the header read off entirely; `camel-http-common` and `camel-undertow` did not.
