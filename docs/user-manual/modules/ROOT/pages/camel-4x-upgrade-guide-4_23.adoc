...
* `camel-sql-stored`: the `CamelSqlStoredTemplate` header replaced the endpoint template, and its value was
  resolved through `SqlHelper.resolveQuery`, which resolves `file:` / `http:` / `classpath:` resources.
...
* `camel-sql-stored`: set `allowTemplateFromHeader=true` to honour the `CamelSqlStoredTemplate` header again; a
  header-supplied template is now resolved with property placeholders only, never as a `file:` / `http:` resource.
