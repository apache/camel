...
Re-applying such an option re-creates the component, as it already does when the option changes in a watched
properties file. Only options whose value is a placeholder are affected, and only when running with Camel Main,
Camel Spring Boot or Camel Quarkus. If a component must not be re-created on reload, configure it programmatically
rather than with a placeholder based property.
