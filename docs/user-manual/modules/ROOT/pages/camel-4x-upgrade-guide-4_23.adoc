...
Credentials are no longer sent to a host the endpoint was not configured with. Two paths reached that
...
again for every redirect hop and re-attached `Authorization: Bearer <token>` to whatever host the
`Location` header named. The token is now attached only for the endpoint's own host.
...
Routes that relied on credentials following a redirect to a different host must set `authHost`
explicitly, which continues to take precedence.

`HttpComponent.createHttpClientConfigurer(Map, boolean)` is unchanged and still behaves as before; a new
`createHttpClientConfigurer(Map, boolean, URI)` overload carries the endpoint's target URI, and the
two-argument form delegates to it with a null URI.
