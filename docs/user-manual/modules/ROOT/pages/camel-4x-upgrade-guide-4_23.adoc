...
A new Cache EIP has been added that provides a transparent read-through cache for a block of processing steps.
On cache hit the cached body is returned and the block is skipped; on cache miss the block executes and
the result body is cached. The Cache EIP uses the `KeyValueRepository` SPI as its backing store, with
`MemoryKeyValueRepository` auto-created when no explicit store is configured.

See xref:components:eips:cache-eip.adoc[Cache EIP] for details.
