...
When GenAI observability is enabled but misconfigured, Camel logs a one-time diagnostic per
`CamelContext` on the first affected LLM call: INFO by default, WARN when
`camel.aiObservability.enabled=true` is set explicitly. This covers a missing
`camel-ai-observability` dependency and an `ObservationRegistry` without a Micrometer tracing
handler (add `micrometer-tracing-bridge-otel` or remove the registry bean to fall back to
`camel-opentelemetry2` GenAI CLIENT spans).
