...
and restarts the route to force a clean reconnect. With `cleanSession=true`, the consumer unsubscribes
before disconnecting; with `cleanSession=false`, it keeps the durable subscription. Routes using a
user-provided client are not affected by this change. Configuring Camel's `SupervisingRouteController`
