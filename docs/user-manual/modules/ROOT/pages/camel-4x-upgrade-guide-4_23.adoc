...
`unmarshal` now deserializes into a copy of `defaultInstance` and returns that, so each exchange gets
its own object. `defaultInstance` is left untouched and therefore keeps working as a template: values
preset on it are now visible on every unmarshalled message, where previously they were overwritten by
the first message and lost.
