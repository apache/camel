...
to `CamelIBMCOSCacheControl`, so the header name matches the Cache-Control metadata it carries. Routes that
referenced the previous header name directly (as a string) must use `CamelIBMCOSCacheControl`; routes using the
