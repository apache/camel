...
==== Spring XML / Blueprint

If you define the repository as a Spring or Blueprint bean, replace the single bean with two beans
— a `MemoryKeyValueRepository` backing store and the appropriate adapter:
...
<!-- After -->
<bean id="kvStore" class="org.apache.camel.support.MemoryKeyValueRepository"/>
<bean id="myRepo" class="org.apache.camel.support.KeyValueIdempotentRepository">
    <constructor-arg ref="kvStore"/>
</bean>
----
...
<!-- Before -->
<bean id="myRepo"
      class="org.apache.camel.processor.aggregate.MemoryAggregationRepository"/>

<!-- After -->
<bean id="kvStore" class="org.apache.camel.support.MemoryKeyValueRepository"/>
<bean id="myRepo" class="org.apache.camel.support.KeyValueAggregationRepository">
    <constructor-arg ref="kvStore"/>
</bean>
----

The `idempotentRepository` and `aggregationRepository` attributes in the route DSL remain
unchanged — they still reference the bean by its `id`.
