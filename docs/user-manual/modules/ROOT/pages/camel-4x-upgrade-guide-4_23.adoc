...
the `default` level, which mirrors what Spring Boot's own health indicators expose, and matches how the other
runtimes report a failed check: camel-main's management endpoint includes `error-stacktrace` only when the
caller asks for it with `?stackTrace=true`, and camel-quarkus-microprofile-health never emits one. It also
matches the documented meaning of the levels, where `full` is the level that includes all details from the
invoked health checks. The trace is unchanged in the application log.
