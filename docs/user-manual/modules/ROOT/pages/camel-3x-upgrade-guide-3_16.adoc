...
The route template local beans have renamed bean `property` to `properties`, eg:
For example a local datasource in a route template as follows:
...
Should have `property` renamed to `properties` as shown:
