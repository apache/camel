...
The Camel CLI can either be installed and used together with JBang (step 1a),
or as a pure standalone Camel installation (step 1b).
...
Download the xref:camel-jbang-launcher.adoc[Camel Launcher] .zip distribution for a desired Camel release.

For example to download Camel 4.18.2 from https://repo1.maven.org/maven2/org/apache/camel/camel-launcher/4.18.2/[Maven Central],
then click and download the _.bin.zip_ file, and then unzip the file to a work folder:
...
TIP: You can also unzip the camel-launcher to a shared folder, and add its bin to the `$PATH` environment so you can execute
`camel` from anywhere in your terminal.
...
Make sure to check out the xref:camel-jbang.adoc[Camel CLI] documentation, for more details on the powers
...
TIP: See the `camel wrapper` command which can be used to install and pin Camel CLI to a given version
which is local for a given project/directory (similar to how Maven Wrapper works).
