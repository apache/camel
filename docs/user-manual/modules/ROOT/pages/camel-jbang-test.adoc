...
This means you need to enable the `test` plugin first to use the subcommands in Camel CLI.
...
TIP: The Camel CLI plugin automatically creates a subfolder `test` and adds the test source files into this folder.
...
Now the automated tests from the prototyping phase with Camel CLI are part of the project for further usage in a CI/CD pipeline for instance.
...
Citrus is an Open Source Java test framework that integrates very well with Apache Camel as it provides special test actions to use Camel CLI for instance.
...
The Camel CLI `infra` command enables you to start infrastructure services in your test environment.
...
You are able to apply specific configuration to the Camel CLI process such as `application.properties`, system properties and environment variables.
...
Read more about the https://citrusframework.org/citrus/reference/html/#apache-camel[Camel CLI support in Citrus].
