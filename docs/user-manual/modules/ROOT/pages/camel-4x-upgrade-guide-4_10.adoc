...
However, if the erro handler is configured to perform retries, then the retry would be
...
Suppose the sink kamelet is throwing an exception, then Camel will now perform the redeliver attempt
