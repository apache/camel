...
Running Camel routes on Kubernetes is quite simple with Camel CLI.
In fact, you can develop and test your Camel route locally with Camel CLI and then promote the same source to running it as an integration on Kubernetes.

The Camel CLI Kubernetes functionality is provided as a command plugin.
This means you need to enable the `kubernetes` plugin first to use the subcommands in Camel CLI.
...
The Kubernetes plugin works with the Camel CLI export functionality.
...
TIP: The Camel CLI Kubernetes plugin provides a `run` command that combines these steps (export, container image build, push, deploy) into a single command.
...
The Camel CLI plugin automatically inspects the Camel routes for exposed Http services and adds the service resource when applicable.
...
NOTE: The `knative-service` trait is disabled by default, you need to enable the Knative service trait with `--trait knative-service.enabled=true` option. Otherwise, the Camel CLI export will always create an arbitrary Kubernetes service resource.
...
The Camel CLI Kubernetes plugin provides some autoconfiguration options when connecting with the Knative component.
...
If you export this route with the Camel CLI Kubernetes plugin, you will see a Knative trigger being generated as part of the Kubernetes manifest (kubernetes.yml).
...
The Camel CLI Kubernetes plugin will inspect your code to automatically create a channel subscription as part of the Kubernetes manifest.
...
The Camel CLI Kubernetes plugin export automatically inspects such a route and automatically creates the sink binding resource for us.
...
In addition to creating the sink binding, the Camel CLI plugin also takes care of configuring the Knative Camel component.
...
NOTE: The `--resource` parameter doesn't set the `camel.component.properties.location=<path>` as the `--config`.
