...
This lists the examples as a ladder: quick start first, then run, transform and map, route, fail well, connect
without a service, connect to one service, contracts and security, AI and cloud, with showcase last. Each group
has a one-line introduction, each example says what you will see when it runs and what it teaches, and icons
show whether it is bundled (works offline), requires Docker, has Citrus tests, or needs a local model.
