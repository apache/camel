...
NOTE: The annotations on a `.groovy` file are handled by the Camel CLI only. In a project created with
`camel export` the file is compiled, but the class is not bound as a bean. For a project that is exported, create
the bean with an inline Groovy `script` in a `- beans:` entry, which works in every runtime, or write it in Java.
