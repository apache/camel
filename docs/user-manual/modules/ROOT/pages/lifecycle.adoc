...
Shutdown is the last phase when Camel is shutting down. After shutdown then Camel cannot be started again.
The shutdown happens automatic when a JVM is terminating, such as Spring Boot applications that has been signalled to terminate.
