...
TIP: There is an alternative tracer that captures the messages in a xref:backlog-tracer.adoc[Backlog Tracer] for on-demand tracing. For example tools, Camel CLI, Hawtio and others are using this.
...
NOTE: When using Camel Main or Camel CLI with the `dev` profile (the default), tracing standby is automatically enabled.
