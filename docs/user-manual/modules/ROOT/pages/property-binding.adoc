...
components, endpoints, xref:components:eips:enterprise-integration-patterns.adoc[EIPs], routes, and Camel bootstrap configuration.

Together with xref:using-propertyplaceholder.adoc[property placeholders], then there is plenty of power, but also
...
- _reference new class_ - Values can refer to creating new beans by their class name by prefixing with `#class`, eg `#class:com.foo.MyClassType`.
       ...
       A class that has no public no-arg constructor but a public static `builder()` or `newBuilder()` method
       (Lombok `@Builder`, Immutables, LangChain4j, the AWS SDK v2, protobuf, and many others) is created via its builder:
       `#class:dev.langchain4j.model.ollama.OllamaChatModel` calls `OllamaChatModel.builder()`, and the bean is created
       with the builder's `build()` method. Where properties are declared for the bean (`camel.beans.` in Camel Main,
       `beans` in YAML and XML) they are set on the builder before the bean is built.
...
Property binding is notably used when running Camel in standalone mode such as with Camel Main.
