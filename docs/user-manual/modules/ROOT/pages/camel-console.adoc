...
The Developer Console is easily available when using xref:camel-jbang.adoc[Camel CLI],
by the `--console` argument when running Camel CLI.
...
With POST, parameters can be sent as a JSON request body, which avoids URL length limits
and is more natural for complex or large payloads:
