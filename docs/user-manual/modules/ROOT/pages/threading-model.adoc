...
pattern in a Camel route you can do it using the `executorService`
...
enforces a flat concurrency cap: the maximum number of concurrently executing tasks equals `maxQueueSize`.
Pool sizing parameters (`poolSize`, `maxPoolSize`) are ignored since virtual threads are not pooled.
