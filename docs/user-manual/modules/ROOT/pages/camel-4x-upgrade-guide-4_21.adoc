...
**Header Override Options — secure by default (breaking change)**

The `useHeaderRecipients`, `useHeaderFrom`, `useHeaderSubject`, and `useHeaderReplyTo` options
introduced in CAMEL-23520 now default to `false`. Previously they defaulted to `true`, which
allowed untrusted exchange headers to override the endpoint URI's pre-configured `To`, `From`,
`Subject`, and `Reply-To` values. This was the same injection pattern addressed in
CVE-2025-27636 and CAMEL-23522 (`useJavaMailSessionPropertiesFromHeaders`).

With the new defaults, the endpoint URI configuration always wins. Routes that set recipient or
sender headers intentionally (for example via `.setHeader("To", ...)` in the route DSL) must
now opt in explicitly:

[source,text]
----
smtp://mailserver?to=default@example.com&useHeaderRecipients=true&useHeaderFrom=true&useHeaderSubject=true
----

Or globally for all mail endpoints, add to `application.properties`:
...
Routes that do not set these headers explicitly, or that consume from an untrusted upstream
(such as an HTTP consumer), are now secure by default without any configuration change.
