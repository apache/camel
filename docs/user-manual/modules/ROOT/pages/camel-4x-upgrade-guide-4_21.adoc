...
Because `rest-openapi` endpoint URIs are lenient, pass-through options such as `oauthProfile` are
accepted by the selected consumer delegate but may not appear as first-class `rest-openapi` options in
component metadata or tooling. Verify that the selected delegate supports the pass-through option;
delegates that ignore unknown options can otherwise start without the expected endpoint protection.
