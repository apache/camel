...
The component has been upgraded to Jackson 3, moving all core classes from the legacy `com.fasterxml.jackson` package to `tools.jackson`. Jackson 3 now enforces strict object immutability to ensure optimal runtime thread safety. If you use a custom mapper, you must migrate from the traditional `new ObjectMapper()` constructor to the new `JsonMapper.builder()` syntax.
