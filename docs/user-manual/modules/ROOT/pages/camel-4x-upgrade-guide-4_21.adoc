...
The `DefaultHeaderFilterStrategy` now blocks headers whose names start with `Camel`, `camel`, or
`org.apache.camel` (case-insensitive) by default in both the inbound (external to Camel) and outbound
(Camel to external) directions. The `CAMEL_FILTER_STARTS_WITH` constant has been updated to include
`org.apache.camel`. Previously this required each component to explicitly call
