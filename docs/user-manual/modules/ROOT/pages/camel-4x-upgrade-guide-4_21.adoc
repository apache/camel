...
==== camel-jbang infra restart

A new `camel infra restart` command has been added. It stops any running instance of the named
external service and starts a fresh one, making it easy to recover a service to a clean state
without having to run `camel infra stop` and `camel infra run` separately.
