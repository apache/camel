...
The `EXTRACT_METADATA` operation now populates the `title` and `documentType` fields on the returned
`DocumentMetadata` (and the corresponding `CamelDoclingMetadataTitle` and `CamelDoclingMetadataDocumentType`
headers) when the information can be determined from the document. The title is taken from the document title
detected by Docling, and the document type is derived from the source file extension or MIME type. Previously
these two fields were always empty.
...
The `aws-bedrock-agent` component no longer exposes the `modelId` endpoint option. It was an unused copy
of the option from the sibling `aws-bedrock-agent-runtime` component: none of the operations served by
`aws-bedrock-agent` (`startIngestionJob`, `listIngestionJobs`, `getIngestionJob`) ever read it, yet it
was incorrectly marked as required and listed a stale enum of retired Anthropic Claude model ids. Routes
that previously set `modelId` on an `aws-bedrock-agent` endpoint (where it had no effect) must remove it
from the endpoint URI. The unused `CamelAwsBedrockAgentRuntimeModelId` header constant
(`BedrockAgentConstants.MODEL_ID`) has also been removed. This does not affect `aws-bedrock-agent-runtime`,
where `modelId` remains a valid and functional option.
...
=== camel-http, camel-jetty, camel-servlet, camel-atmosphere-websocket

The HTTP components (which share `camel-http-common`) now apply a Java deserialization filter
(`ObjectInputFilter`) when deserializing `application/x-java-serialized-object` payloads on the opt-in
`allowJavaSerializedObject` / `transferException` path, aligning them with `camel-netty-http`, `camel-jms`
and `camel-vertx-http`. A new `deserializationFilter` component option (same syntax as `jdk.serialFilter`)
can be set to customise it; when unset, the JVM-wide `jdk.serialFilter` is used if present, otherwise a
conservative default filter (denying `java.net.**`, allowing `java.**` / `javax.**` /
`org.apache.camel.**`, with JEP-290 graph-shape limits) is applied. This is a defense-in-depth measure on
an already opt-in path; a serialized payload that was accepted before may now be rejected if it references
a denied class.
...
Some deprecated API usage has been removed from the camel-vertx components to help make the future transition to Vert.x 5 simpler.

==== camel-vertx-websocket clientOptions type changed from HttpClientOptions to WebSocketClientOptions

The `clientOptions` configuration property on the `vertx-websocket` endpoint has changed type from `io.vertx.core.http.HttpClientOptions` to `io.vertx.core.http.WebSocketClientOptions`. 
Both classes share the same base class (`ClientOptionsBase`) so SSL, proxy, and timeout options carry over. 
Applications that set `clientOptions` programmatically must update the type. Endpoint URI options are unaffected.

==== ByteBuf type converters removed

The type converters for Netty `ByteBuf` to Vert.x `Buffer`, `JsonObject`, and `JsonArray` have been removed from `camel-vertx-common`. 
These converters used the `Buffer.buffer(ByteBuf)` API which is deprecated in Vert.x 4.x and removed in Vert.x 5.
