...
The `platform-http` consumer endpoint now has an `oauthProfile` option for opt-in validation of incoming
`Authorization: Bearer` tokens using the public OAuth token validation SPI.
...
such as `at+jwt`.
Requests without a Bearer token or with an invalid token are rejected with HTTP 401.
Static profile configuration is validated at route startup. Token validation infrastructure failures
