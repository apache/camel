...
The services should try to minimize the test execution time and resource usage when running. As such the JUnit `@BeforeAllCallback` and `@AfterAllCallback`
...
[NOTE]
====
Bear in mind that, according to the https://docs.junit.org/6.0.3/extensions/registering-extensions.html#registration-programmatic[JUnit 6 Extension]
