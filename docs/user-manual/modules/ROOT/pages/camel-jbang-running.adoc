...
`--jvm-args` and `--jfr` are passed to the application on all three runtimes. The application logs to a
...
- `--console` is supported with Camel Main (the exported project enables the developer console, health, info
  and Jolokia), but not yet with Spring Boot and Quarkus. The deprecated `--health` and `--metrics` options
