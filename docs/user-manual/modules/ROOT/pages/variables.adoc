...
IMPORTANT: You can also use _exchange properties_ as variables, but the exchange properties are also used internally by Camel,
...
has APIs to browse the current variables. Camel uses this with Camel CLI, and JMX to be able to see the current variables
...
IMPORTANT: The id `exchange`, `header`, `global`, `route`, and `group` are reserved by Camel internally and should not be used as id for custom repositories.
...
You can also set group scoped variables, using `group.` as prefix. The dot is used to separate
...
When data is received into the same variable again, the header variables from the previous time are removed first,
so the header variables are always the headers of the last received message. This is done for exchange, global, route
and group variables. A custom variable repository that cannot be browsed keeps the old header variables.
Note that the header variables are matched by name prefix, so receiving into `a` also removes the header variables of a variable named `a.b`
(`header:a.b.x` is both the header `b.x` of `a` and the header `x` of `a.b`).
