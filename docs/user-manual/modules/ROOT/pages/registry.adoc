...
whether you run Camel on Spring Boot, Quarkus, Standalone, Camel JBang or something else.
...
IMPORTANT: The binding API will bind the bean to Camels internal registry, and not in the runtime (Spring Boot etc).
...
Registry is mostly used for looking up beans by their IDs, or by type. This is frequently used
...
which may not always be the case. Therefore, it is recommended to use the Spring Boot or Quarkus annotations.
