...
that runtime puts it: `/q/dev` with Camel Main, `/actuator/camel` with Spring Boot (the `camel` actuator
endpoint; with `--observe` it moves to the management port under `/observe/camel`), and
`/q/camel/dev-console` with Quarkus. The exported Quarkus project exposes the console in prod mode
(`quarkus.camel.console.exposure-mode=ALL`) as `camel run` without `--dev` runs via `quarkus:run`. The
console has no authentication on any runtime, so it is meant for local development only.
