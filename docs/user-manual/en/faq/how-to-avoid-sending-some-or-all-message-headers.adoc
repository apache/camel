...
private data" since the header has only been filtered out from the communication,
but not removed from the Exchange.
...
can be reversed so only certain headers are accepted throw the filterOnMatch
property.
