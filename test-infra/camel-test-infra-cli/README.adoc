...
= Camel CLI CLI test infrastructure

== Overview

This module provides test infrastructure for Camel CLI (Camel CLI) integration tests.
Two service implementations are available:

* *Container-based* (`CliLocalContainerService`) -- builds and runs a Docker container based on {image-name} image with Camel CLI. This is the default.
