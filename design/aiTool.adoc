...
* Provide `AiToolExecutor`, a shared utility that resolves the route processor, validates
  ...
  and also wraps them in an `AiToolArguments` variable for programmatic access. It then
...
4. Also wrap arguments in an `AiToolArguments` object and set it as an exchange variable
   for programmatic access
5. Invoke the route processor
6. Return the result body as a string
