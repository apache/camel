...
| *camel.debug.singleStepFirst* | In single step mode, then when the exchange is created, then simulate a breakpoint as first, that allows to suspend and watch the incoming exchange at the route (you can see message body as response, failed exception etc). | false | boolean
| *camel.debug.singleStepLast* | In single step mode, then when the exchange is complete, then simulate a breakpoint as last, that allows to suspend and watch the exchange when complete (you can see message body as response, failed exception etc). | false | boolean
