...
| `camel.resilience4j.slidingWindowSynchronizationStrategy` | Configures the synchronization strategy for the sliding window. LOCK_FREE uses a CAS-based lock-free algorithm for better performance under high concurrency. SYNCHRONIZED uses blocking locks with lower memory allocation. | SYNCHRONIZED | String
...
| `camel.errorRegistry.maximumEntriesPerKind` | The maximum number of error entries of the same kind (same route, node, exception type and message) to keep, so a storm of one failure does not evict all the other errors. The counter of that kind keeps rising even when its older entries are evicted. | 3 | int
...
(Lombok `@Builder`, Immutables, LangChain4j, Spring AI, the AWS SDK v2, Jackson, protobuf, the JDK `HttpClient`
and many others) is created via its builder: Camel calls the `builder()` method, sets the properties on the
returned builder, and creates the bean with its `build()` method. A LangChain4j chat model is then declared like
any other bean, with the property names of the builder's fluent setters:
...
If the builder has no `build()` method, Camel uses its single public no-arg method that returns the type.
A class that has a public no-arg constructor is created with the constructor as usual, even if it also has a `builder()` method;
a class whose builder is a separate class, or has several methods that return the type, is declared in YAML or XML
with `builderClass` and `builderMethod` (see the xref:components:others:yaml-dsl.adoc[YAML DSL]).
