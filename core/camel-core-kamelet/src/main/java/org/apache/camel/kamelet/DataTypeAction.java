...
@KameletSpec(name = "data-type-action",
             ...
             properties = {
                     ...
                     @KameletProperty(
                                      ...
                                      description = "Defines the data type that will be applied by this action. Apache Camel and the "
                                                    +
                                                    "Kamelet catalog support different data types and performs automatic message conversion according to the given type.")
             },
...
public class DataTypeAction extends KameletRouteTemplate {

    @Override
    protected void template(RouteTemplateDefinition template) {
        template.templateBean("dataTypeProcessor", "#class:org.apache.camel.processor.transformer.DataTypeProcessor");

        template.from(kameletSource())
                .choice()
                    .when(simple("'{{?scheme}}' != ''"))
                        .setProperty("CamelDataType", simple("{{?scheme}}:{{format}}"))
                    .otherwise()
                        .setProperty("CamelDataType", simple("{{format}}"))
                .end()
                .bean("{{dataTypeProcessor}}", "process");
    }
}
