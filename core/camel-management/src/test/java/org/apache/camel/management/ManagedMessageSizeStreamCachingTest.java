...
public class ManagedMessageSizeStreamCachingTest extends ManagementTestSupport {
    ...
    @Test
    public void testStreamCacheBodyReadableAfterSizeComputation() throws Exception {
        getMockEndpoint("mock:result").expectedMessageCount(1);

        String body = "Hello Stream Cached World";
        template.sendBody("seda:start", new ByteArrayInputStream(body.getBytes(StandardCharsets.UTF_8)));

        assertMockEndpointsSatisfied();

        // verify that the body was still readable after size computation (stream cache was reset)
        String receivedBody = getMockEndpoint("mock:result").getExchanges().get(0)
