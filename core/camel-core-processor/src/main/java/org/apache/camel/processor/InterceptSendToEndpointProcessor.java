...
public class InterceptSendToEndpointProcessor extends DefaultAsyncProducer {
    ...
    private boolean callback(Exchange exchange, AsyncCallback callback, boolean doneSync) {
        ...
        if (endpoint.getAfter() != null) {
            ...
            whenMatches = (Boolean) exchange.getProperty(Exchange.INTERCEPT_SEND_TO_ENDPOINT_WHEN_MATCHED);
        } else {
            // remove property as its not longer needed
            whenMatches = (Boolean) exchange.removeProperty(Exchange.INTERCEPT_SEND_TO_ENDPOINT_WHEN_MATCHED);
        }
        if (whenMatches != null) {
    ...
    protected void doInit() throws Exception {
        ...
        if (endpoint.getBefore() != null || endpoint.getAfter() != null) {
            ...
            Predicate predicate = exchange -> {
                Boolean whenMatches
                        = (Boolean) exchange.removeProperty(Exchange.INTERCEPT_SEND_TO_ENDPOINT_WHEN_MATCHED);
