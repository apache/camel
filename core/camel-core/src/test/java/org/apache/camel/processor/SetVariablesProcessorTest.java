...
public class SetVariablesProcessorTest extends ContextTestSupport {
    ...
    private final Map<String, Expression> varMap = new LinkedHashMap<>();
    ...
    public void testUseXpathLanguage() throws Exception {
        expected.message(0).variable("name").isEqualTo("Jane");
        expected.message(0).variable("age").isEqualTo(10);
    ...
    public void testUseMap() throws Exception {
        context.addRoutes(new RouteBuilder() {
            public void configure() throws Exception {
                from("direct:startMap").setVariables(varMap).to("mock:result");
            }
        });
        expected.message(0).variable("foo").isEqualTo("ABC");
        expected.message(0).variable("bar").isEqualTo("XYZ");
    ...
    public void testUseMethod() throws Exception {
        ...
        expected.message(0).variable("mapTest").isEqualTo(new VarBean().getVariables(hdrInBody));
        assertMockEndpointsSatisfied();
    }

    @Test
    public void testSetOneVariableFromAnother() throws Exception {
        expected.message(0).variable("foo").isEqualTo(15);
        expected.message(0).variable("bar").isEqualTo(true);
    ...
    public void setUp() throws Exception {
        super.setUp();
        varMap.put("foo", ConstantLanguage.constant("ABC"));
        varMap.put("bar", ConstantLanguage.constant("XYZ"));
