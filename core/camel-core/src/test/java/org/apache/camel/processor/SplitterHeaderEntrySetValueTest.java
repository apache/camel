...
/**
 * A split sub-message that changes a header value through the entry set of its headers does not change the header of
...
public class SplitterHeaderEntrySetValueTest extends ContextTestSupport {
    ...
    public void testEntrySetValueIsolated() throws Exception {
        getMockEndpoint("mock:split").expectedHeaderValuesReceivedInAnyOrder("foo", "A", "B", "C");
            ...
            public void configure() {
                from("direct:start")
                        ...
                        .process(e -> {
                            ...
                            for (Map.Entry<String, Object> entry : e.getMessage().getHeaders().entrySet()) {
