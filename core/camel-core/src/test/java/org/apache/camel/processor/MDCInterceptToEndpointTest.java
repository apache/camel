...
public class MDCInterceptToEndpointTest extends ContextTestSupport {
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                ...
                interceptSendToEndpoint("direct:cheese").process(new Processor() {
                ...
                from("direct:cheese").routeId("route-cheese").step("step-cheese").process(new Processor() {
                    public void process(Exchange exchange) {
                        assertEquals("route-cheese", MDC.get("camel.routeId"));
                        assertEquals(exchange.getExchangeId(), MDC.get("camel.exchangeId"));
                        assertEquals(exchange.getIn().getMessageId(), MDC.get("camel.messageId"));
                        assertEquals("step-cheese", MDC.get("camel.stepId"));
                        assertEquals("1", MDC.get("custom.id"));
                    }
                });
