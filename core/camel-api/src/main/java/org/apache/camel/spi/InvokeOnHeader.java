 ...
 * <p/>
 * A method is only asynchronously executed if the method has {@link AsyncCallback} as a parameter.
 * In this situation then the method should not return a value (void). And its the responsible
 * of the method to invoke <pre>callback.done(false)</pre> when to continue routing.
 * <p/>
 * Synchronous methods can either be void or return a value. If a value is returned then the value
 ...
 * </ul>
 * Component implementation producers should extend org.apache.camel.support.HeaderSelectorProducer.
 * And use Camel maven tooling (camel-package-maven-plugin) to generate java source code
