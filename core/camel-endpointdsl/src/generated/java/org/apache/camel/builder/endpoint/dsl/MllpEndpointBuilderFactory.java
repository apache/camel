 ...
 * other systems using the MLLP protocol. NOTE: MLLP payloads are not logged
 * unless the logging level is set to DEBUG or TRACE to avoid introducing PHI
...
public interface MllpEndpointBuilderFactory {
    ...
    public interface MllpBuilders {
         ...
         * communicate with other systems using the MLLP protocol. NOTE: MLLP
         * payloads are not logged unless the logging level is set to DEBUG or
         * TRACE to avoid introducing PHI into the log files. Logging of PHI can
         * be globally disabled by setting the org.apache.camel.mllp.logPHI
     ...
     * with other systems using the MLLP protocol. NOTE: MLLP payloads are not
     * logged unless the logging level is set to DEBUG or TRACE to avoid
     * introducing PHI into the log files. Logging of PHI can be globally
     * disabled by setting the org.apache.camel.mllp.logPHI system property to
     * false.
     * 
     * Category: mllp
     * Since: 2.17
