...
public interface BlockingTask extends Task {
     ...
     * @param  predicate    the task as a predicate. The result of the predicate is used to check if the task has
     *                      completed or not. The predicate must return true if the execution has completed or false
     *                      otherwise. Failures on the task should be handled on the predicate using the payload as
     *                      wrapper for In/Out if necessary
     ...
     * @return              true if the task has completed successfully or false if: 1) the budget is exhausted or 2)
     ...
     *                      the task was interrupted.
     */
    boolean run(CamelContext camelContext, BooleanSupplier supplier);
