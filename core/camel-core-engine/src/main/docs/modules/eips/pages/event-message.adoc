...
the xref:manual:ROOT:exchange-pattern.adoc[Exchange Pattern] for a *consumer* endpoint using
...
You can specify the
xref:manual:ROOT:exchange-pattern.adoc[Exchange Pattern] using `setExchangePattern` in the DSL.
...
When using `setExchangePattern` then the xref:manual:ROOT:exchange-pattern.adoc[Exchange Pattern]
on the xref:manual:ROOT:exchange.adoc[Exchange] is changed from this point onwards in the route.
...
NOTE: Using `setExchangePattern` to change the xref:manual:ROOT:exchange-pattern.adoc[Exchange Pattern]
