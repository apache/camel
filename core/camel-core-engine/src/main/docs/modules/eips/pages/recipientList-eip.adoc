...
The following properties are set on each Exchange that are sent by the recipient list:
...
The Recipient List EIP allows to route *the same* message to a number of xref:latest@manual:ROOT:endpoint.adoc[endpoints]
...
The following example demonstrates how to create a dynamic recipient list using an xref:latest@manual:ROOT:expression.adoc[Expression]
...
The Recipient List now supports `ignoreInvalidEndpoints` (like xref:routingSlip-eip.adoc[Routing Slip] EIP).
