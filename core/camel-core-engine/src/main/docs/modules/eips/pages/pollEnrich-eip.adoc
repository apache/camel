...
You then use the `AggregationStrategy` with the `pollEnrich` in the following example,
where Camel will poll a file (timeout 10 seconds). The `AggregationStrategy` is then used to merge the file with the existing `Exchange`.
