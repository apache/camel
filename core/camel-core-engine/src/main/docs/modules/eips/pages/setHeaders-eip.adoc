...
:description: Allows to set multiple headers on the message st the same time.
:since: 
:supportlevel: Stable
:tabs-sync-option:
...
If the order in which the headers shoud be set is important, use a `LinkedHashMap`.
====
Java::
+
[source,java]
