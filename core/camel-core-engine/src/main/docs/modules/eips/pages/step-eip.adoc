...
This allows capturing metrics at a group level which can make management and monitoring
of Camel routes easier by using higher-level abstractions. You can also
think this as a middle-level between the route and each processor in the routes.
...
Each Step EIP is registered in JMX under the `type=steps` tree, which allows monitoring
