...
If the message body is stream based, then logging the message body may cause the message body to be empty afterward. For streamed messages, you can use xref:manual::stream-caching.adoc[Stream caching] to allow logging the message body and be able to read the message body afterward again.
...
Java::
+
In Java DSL the Log EIP has overloaded methods to set the logging level and/or name as well.
+
[source,java]
----
from("direct:start")
...
<route>
    <from uri="direct:start"/>
    <log logName="com.mycompany.MyCoolRoute" message="Processing ${id}" loggingLevel="DEBUG"/>
    <to uri="bean:foo"/>
</route>
----
...
The Log EIP can also be used to log the file name being processed if you consume files.
...
TIP: If you enable `sourceLocationEnabled=true` on `CamelContext` then Camel will use source _sourceFileName:lineNumber_ as logger name,
...
In addition to name-based masking, `DefaultMaskingFormatter` also masks two value shapes that have no
sensitive key name: URI userinfo passwords (`scheme://user:password@host`) and PEM private-key blocks
(`-----BEGIN … PRIVATE KEY-----`). Certificates and public keys are left unchanged.
