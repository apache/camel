...
xref:manual:ROOT:exchange.adoc[Exchange] is about to be sent to the given xref:message-endpoint.adoc[endpoint].
...
* xref:manual:ROOT:predicate.adoc[Predicate] using `when` to only trigger the interceptor in certain conditions
...
If you only want to intercept "sometimes", then you can use a xref:manual:ROOT:predicate.adoc[predicate].
...
You can combine both a xref:manual:ROOT:predicate.adoc[predicate] and skip sending to the original endpoint.
