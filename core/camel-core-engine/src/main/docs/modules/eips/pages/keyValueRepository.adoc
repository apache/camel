...
| `JdbcKeyValueRepository`
| ✗
| Optimistic INSERT + unique-key constraint catch

| `JpaKeyValueRepository`
| ✗
| Optimistic INSERT + constraint catch (provider-dependent)

| `CassandraKeyValueRepository`
| ✗
| `INSERT … IF NOT EXISTS` (Paxos, not linearizable across restarts)
