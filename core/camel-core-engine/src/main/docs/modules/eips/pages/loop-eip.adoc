...
| Exchange.LOOP_SIZE | Total number of loops. This is not available if running the loop in while loop mode.
...
The xref:latest@manual:ROOT:expression.adoc[Expression] *must* evaluate to an `int`,
