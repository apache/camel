...
NOTE: When multiple `.cache()` blocks auto-discover the same `KeyValueRepository`, they share a single key namespace.
If two unrelated blocks can produce the same cache key value, they will collide and return each other's cached data.
To avoid this, either give each block its own named `KeyValueRepository`, or design cache keys that are inherently
unique across blocks (e.g. include the target service name in the key expression:
