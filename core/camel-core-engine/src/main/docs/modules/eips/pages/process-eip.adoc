...
is used for processing message xref:latest@manual:ROOT:exchange.adoc[Exchanges].
...
the in message is reused. At the end of a route, the xref:latest@manual:ROOT:exchange-pattern.adoc[Messageg Exchange Pattern] (MEP)
...
has been enlisted in the xref:latest@manual:ROOT:registry.adoc[Registry], such as with the id `myProcessor`:
