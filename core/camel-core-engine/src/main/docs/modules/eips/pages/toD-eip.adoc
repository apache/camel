...
The xref:to-eip.adoc[To] and ToD EIPs are the most common patterns to use in Camel xref:latest@manual:ROOT:routes.adoc[routes].
...
The dynamic endpoint is evaluated _on-demand_ by an xref:latest@manual:ROOT:expression.adoc[Expression].
...
You can also prefix the uri with a value because the endpoint xref:latest@manual:ROOT:uris.adoc[URI] is
...
You can also use other languages such as xref:components:languages:xpath-language.adoc[XPath].
...
. The dynamic uri in `toD` must provide the component name as either static or resolved via xref:latest@manual:ROOT:using-propertyplaceholder.adoc[property placeholders].
