...
|`!` | The `not` operator negates the function that follows it, which must evaluate to `true` or `false`.
|====
...
In an expression a `!` is ordinary text, so a message such as `Hello ${body}!` is unaffected. Note that `!` negates a
function, not a comparison: write `${body} != 'x'` rather than `!${body} == 'x'`, and the negated operators
