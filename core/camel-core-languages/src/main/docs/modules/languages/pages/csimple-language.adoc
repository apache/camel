...
[WARNING]
====
The CSimple (compiled simple) language is *deprecated* (since Camel 4.19) and will be removed in a future release.
...
|variableAsIndex(_key_, _type_, _index_) |Type | To be used for collecting a variable from an existing `Collection`, `Map` or array (lookup by the index)
and then converting the variable to the given type determined by its classname. The converted variable can be null.
...
NOTE: The ternary operator (`? :`) IS supported in csimple. The limitation above refers only to the null-safe operator used in OGNL expressions (e.g., `${body?.address}`).
