...
The simple language is designed to cover almost all the common use cases
when little need for scripting in your Camel routes.

However, for much more complex use cases, a more powerful language is recommended such as:
...
You can also use the alternative syntax which uses `$simple{ }` as placeholders.
This approach is helpful in avoiding conflicts, such as when you are using Spring property placeholders alongside with Camel.
====
...
|exception |Object |refer to the exception object on the exchange, which will be *null* if no exception is set on exchange; when no exception is found, it will fall back to retrieve any caught exceptions (`Exchange.EXCEPTION_CAUGHT`) present on the Exchange.

|exception.*OGNL* |Object |refer to the exchange exception invoked using a Camel OGNL
expression object

|exception.message |String |refer to the message of the exception on the exchange, which will be *null* if no exception is set on exchange;
when no exception is found, it will fall back to retrieve any (`Exchange.EXCEPTION_CAUGHT`) present on the Exchange.

|exception.stacktrace |String |refer to the stack trace object of the exchange, which will be *null* if no exception is set on exchange;
when no exception is found, it will fall back to retrieve any caught exceptions (`Exchange.EXCEPTION_CAUGHT`) present on the Exchange.
...
Exchange is being routed. Not all routes has a group assigned, so this may be null.
...
|substring(num1,num2,exp) |String |replace all the string values in the given expression.
If the number is positive then the returned string is clipped from the beginning.
If the number is negative then the returned string is clipped from the ending.
...
true, otherwise the value of `falseExp` is returned.
