...
|`average(val1,val2,...)` | `long` | Returns the average number from all the values.
...
|`max(val1,val2,...)` | `long` | Returns the maximum number from all the values.
|`min(val1,val2,...)` | `long` | Returns the minimum number from all the values.
...
|`pad(exp,width,separator)` | `String` | Pads the expression with extra padding if necessary, according the the total width The separator is by default a space. If the width is negative then padding to the right, otherwise to the left.
...
Yes you can use the full power of the simple language and all its functions, for example:
