...
|`listAdd(source,fun)` | `List` | Adds the result of the function to the message body (or expression) as a list object.
|`listRemove(source,fun)` | `List` | Removes the result of the function (use int to remove by position index) from the message body (or expression) as a list object.
|`map(key1,value1,...)` | `Map` | The map function creates a `java.util.LinkedHashMap` with the given set of pairs.
|`mapAdd(source,key,fun)` | `List` | Adds the result of the function to the message body (or expression) as a map object.
...
There are also _pretty_ alternatives so when calling `toPrettyJson` will output: TODO:
