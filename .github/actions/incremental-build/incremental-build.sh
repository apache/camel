...
runScalpelDetection() {
  ...
  # Temporarily inject Scalpel extension and snapshot repository into .mvn/extensions.xml
  # Scalpel 0.1.0-SNAPSHOT is deployed to Sonatype Central Portal snapshots.
  ...
  # Create a settings file with the Sonatype Central Portal snapshot repository
  # so Maven can resolve the Scalpel extension during initialization.
  cat > /tmp/scalpel-settings.xml << 'SETTINGS_EOF'
<settings>
  <profiles>
    <profile>
      <id>scalpel-snapshots</id>
      <repositories>
        <repository>
  ...
  <activeProfiles>
    <activeProfile>scalpel-snapshots</activeProfile>
  </activeProfiles>
</settings>
SETTINGS_EOF
  ...
  # - fullBuildTriggers="": override .mvn/** default (we modified extensions.xml)
  ...
  ./mvnw -B -q validate -s /tmp/scalpel-settings.xml $scalpel_args -l /tmp/scalpel-validate.log 2>/dev/null || {
    echo "  WARNING: Scalpel detection failed (exit $?), skipping"
    grep -i "scalpel" /tmp/scalpel-validate.log 2>/dev/null | head -5 || true
    return
  }

  # Restore extensions.xml immediately (don't wait for function return)
