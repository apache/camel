...
Scalpel is resolved as a `0.1.0-SNAPSHOT` from the [Sonatype Central Portal snapshots repository](https://central.sonatype.com/repository/maven-snapshots/). A temporary `settings.xml` is created to enable snapshot resolution. The `mvn validate` with report mode adds ~60-90 seconds.
