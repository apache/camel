...
public class SjmsBatchEndpoint extends DefaultEndpoint implements HeaderFilterStrategyAware {
     ...
     * SjmsBatchConsumer will attempt to create a new session if it sees an IllegalStateException during message
     * consumption. This delay value allows you to pause between attempts to prevent spamming the logs. If this is a
     * negative value (default is -1) then the SjmsBatchConsumer will behave as it always has before - that is it will
     * bail out and the route will shut down if it sees an IllegalStateException.
     */
