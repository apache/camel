...
| *fileExist* (producer) a|
What to do if a file with the same name already exists. The value can be one of:
...
- Move - Moves any existing files before writing the target file. Requires the moveExisting option to be configured as well. The option eagerDeleteTargetFile can be used to control what to do if there is already a file with the same name when moving the file, possibly causing the move operation to fail.
- TryRename - (Only applicable if the tempFileName option is in use) allows to try renaming the file from the temporary name to the actual name without doing any exists checks. This operation may be faster on some file systems and especially FTP servers.
...
| *readLock* (lock) a|
Consumer strategy to check for exclusive read-lock before reading a file (i.e. wait until the file is finished and not still being written). The value can be one of:

- none - No read lock strategy.
- markerFile - Creates a marker file (fileName.camelLock) and then holds a lock on it. This option is not available for the FTP component.
- changed - Uses file length/modification timestamp to detect whether the file is currently being copied or not. Will at least use 1 sec to determine this, so this option cannot consume files as fast as the others, but can be more reliable as the JDK IO API cannot always determine whether a file is currently being used by another process. The option readLockCheckInterval can be used to set the check frequency.
- fileLock - Uses java.nio.channels.FileLock. This option is not available for Windows OS and the FTP component. This approach should be avoided when accessing a remote file system via a mount/share unless that file system supports distributed file locks.
- rename - Tries to rename the file as a test to obtain exclusive read-lock.
- idempotent - (Only for file component) Uses an idempotentRepository as the read-lock. This allows to use read locks that supports clustering if the idempotent repository implementation supports that.
- idempotent-changed - (Only for file component) Uses an idempotentRepository and changed as the combined read-lock. This allows to use read locks that supports clustering if the idempotent repository implementation supports that.
- idempotent-rename - (Only for file component) Uses an idempotentRepository and rename as the combined read-lock. This allows to use read locks that supports clustering if the idempotent repository implementation supports that.

Notice: Not all read lock strategies are suited to work in clustered mode, where concurrent consumers on different nodes compete for the same files on a shared file system. The markerFile strategy uses a close to atomic operation to create the empty marker file but it is not guaranteed to work in a cluster. The fileLock option may work better but then the file system needs to support distributed file locks, and so on. Using the idempotent read lock can support clustering if the idempotent repository supports clustering, such as Hazelcast Component or Infinispan.
| none | String
