...
can be different depending on the OS platform as well. This could lead to
that Camel thinks the file is not locked by another process and start
consuming it. Therefore, you have to do your own investigation what suites
your environment. To help with this Camel provides different `readLock`
options and `doneFileName` option that you can use. See also
...
By default, it will override any existing file if one exists with the same name.
====
...
[NOTE]
====
There is a sample <<File2-ReadingFromADirectoryAndTheDefaultMoveOperation,showing reading from a directory and the default move operation>> below.
====
...
Though, if you use an expression such as xref:languages:file-language.adoc[File Language],
or xref:languages:simple-language.adoc[Simple], then the result of the expression
evaluation is the file name to be used.
E.g., if you set:
...
Then that's using the xref:languages:file-language.adoc[File Language] which we
 use to return the file name to be used.
This can be either relative or absolute.
If relative, the directory is created as a subfolder from within the folder where the file was consumed.
...
The `move` and `preMove` options are Expression-based, so we have the full power of
...
into a xref:languages:file-language.adoc[File Language] expression.
So, when we use `move=.done` Camel will convert this into:
`${file:parent}/.done/${file:onlyname}`.

This is only done if Camel detects that you have not provided a `${ }` in the option value
yourself.
...
choice.
For example, to move the files in an error folder with a timestamp you can use
...
When reading or writing files that contain characters that cannot be
mapped in the configured charset, Camel throws an
`UnmappableCharacterException` by default. You can control this
behavior with the `charsetUnmappable` option:

* `REPORT` (default) — throws an exception when an unmappable character is encountered.
* `IGNORE` — silently drops unmappable characters.
* `REPLACE` — substitutes unmappable characters with the charset's replacement character (typically `?`).

For example, to silently ignore unmappable characters when reading files:
...
Or to replace them with a substitution character when writing:

[source,java]
----
from("file:inbox?charset=utf-8")
  .to("file:outbox?charset=iso-8859-1&charsetUnmappable=REPLACE");
----
...
another folder and, after writing, move the file in the drop folder.
...
If you want only to consume files when a _done file_ exists, then you can
...
It will only consume files from the _bar_ folder if a _done file_ exists in
the same directory as the target files. Camel will automatically delete
the _done file_ when it's done consuming the files.

[NOTE]
====
Camel does not delete automatically the _done file_ if `noop=true` is configured.
====
...
----
+
In this example, the files will only be polled if there exists a _done file_
...
----
+
You can also use a prefix for the _done file_, such as:
...
After you have written a file, you may want to write an additional _done file_ as a kind of marker,
to indicate to others that the file is finished and has been written.
To do that, you can use the `doneFileName` option on the file producer endpoint.
...
This will create a file named `done` in the same directory as the
target file.

However, it is more common to have one _done file_ per target file. This
means there is a 1:1 correlation. To do this, you must use dynamic
placeholders in the `doneFileName` option. Currently, Camel supports the
following two dynamic tokens: `file:name` and `file:name.noext`.
...
----
+
This will, for example, create a file named `done-foo.txt` if the target file
...
----
+
This will, for example, create a file named `foo.txt.done` if the target file
was `foo.txt` in the same directory as the target file.
...
consumed files.
It uses the least recently used cache holding up to 1000
entries. You can plug in your own implementation of this store by using
...
of the in-memory one that is used as default.
This repository uses a first level cache to avoid reading the file
repository. It will only use the file repository to store the content of
the first level cache. Thereby, the repository can survive server restarts.
It will load the content of the file into the first level cache upon
startup. The file structure is basic as it stores the key in
...
rebuilding the content by flushing the first level cache into a fresh
empty file.
...
`org.apache.camel.processor.idempotent.jpa.MessageProcessed` as the model.
...
There is also a `isDirectory` method on `GenericFile` to inform whether the file is a directory.
This allows you to filter unwanted directories, to avoid traversing down unwanted directories.

===== Using the `GenericFilter`

You can then configure the endpoint with such a filter to skip certain files being processed.
...
This strategy it to use the build in `java.util.Comparator` in Java. You can then configure the
...
And then we want to group by name as a second option, so files with the same
modification time are sorted by name:
...
Well, as we have the true power of xref:languages:file-language.adoc[File Language],
we can use its date command that supports patterns.
...
For instance, let's assume a system writes a file in a folder you should
...
case the `begin` operation returned `false`, for example, to clean up
resources etc.
...
[IMPORTANT]
====
When using bridgeErrorHandler, then `interceptors`, `OnCompletions`
do *not* apply.
The Exchange is processed directly by the Camel Error Handler, and does not allow
