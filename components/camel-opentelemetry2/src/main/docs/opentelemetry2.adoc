...
Baggage entries are propagated to downstream spans and services.
They can be managed via `OpenTelemetrySpanAdapter`:
...
public void process(Exchange exchange) throws Exception {
    ...
    // ...
    String value = span.getBaggageEntry("tenant.id");
}
----
