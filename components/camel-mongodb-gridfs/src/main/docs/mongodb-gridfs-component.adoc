...
| *queryStrategy* (consumer) | Sets the QueryStrategy that is used for polling for new files. Default is Timestamp. There are 5 enums and the value can be one of: time-stamp, persistent-timestamp, file-attribute, time-stamp-and-file-attribute, persistent-timestamp-and-file-attribute | time-stamp | QueryStrategy
