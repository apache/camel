...
|===
| Name | Description | Default | Type
| *camel.component.netty4-http.configuration.allow-default-codec* | null |  | boolean
...
| *camel.component.netty4-http.configuration.disable-stream-cache* | Determines whether or not the raw input stream from Netty
 ...
 stream, because Netty has closed it. | false | boolean
...
| *camel.component.netty4-http.configuration.max-header-size* | The maximum length of all headers. If the sum of the length of each
 ...
 {@link io.netty.handler.codec.TooLongFrameException} will be raised. | 8192 | int
...
| *camel.component.netty4-http.configuration.port* | The port number. Is default 80 for http and 443 for https. |  | int
...
| *camel.component.netty4-http.security-configuration.role-class-name* | null |  | String
