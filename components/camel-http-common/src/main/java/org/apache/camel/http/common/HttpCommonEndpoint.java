...
public abstract class HttpCommonEndpoint extends DefaultEndpoint
    ...
    URI httpUri;
    @UriParam(label = "common,advanced", description = "To use a custom HeaderFilterStrategy to filter header to and from Camel message.")
    ...
    @UriParam(label = "producer", description = "If the option is true, HttpProducer will ignore the Exchange.HTTP_URI header, and use the endpoint's URI for request."
    ...
    @UriParam(label = "producer,advanced", description = "If the option is true, HttpProducer will set the Host header to the value contained in the current exchange Host header, "
    ...
    @UriParam(label = "common", description = "If enabled and an Exchange failed processing on the consumer side, and if the caused Exception was send back serialized"
    ...
    @UriParam(label = "consumer", description = "If enabled and an Exchange failed processing on the consumer side the exception's stack trace will be logged"
    ...
    @UriParam(label = "producer,advanced", description = "If this option is true, The http producer won't read response body and cache the input stream")
    boolean ignoreResponseBody;
    @UriParam(label = "producer,advanced", defaultValue = "true", description = "If this option is true then IN exchange headers will be copied to OUT exchange headers according to copy strategy."
    ...
    @UriParam(label = "consumer,advanced", description = "Whether to eager check whether the HTTP requests has content if the content-length header is 0 or not present."
            + " This can be turned on in case HTTP clients do not send streamed data.")
    boolean eagerCheckContentAvailable;
    @UriParam(label = "consumer,advanced", defaultValue = "true", description = "If this option is true then IN exchange Body of the exchange will be mapped to HTTP body."
            + " Setting this to false will avoid the HTTP mapping.")
    boolean mapHttpMessageBody = true;
    @UriParam(label = "consumer,advanced", defaultValue = "true", description = "If this option is true then IN exchange Headers of the exchange will be mapped to HTTP headers."
            + " Setting this to false will avoid the HTTP Headers mapping.")
    boolean mapHttpMessageHeaders = true;
    @UriParam(label = "consumer,advanced", defaultValue = "true", description = "If this option is true then IN exchange Form Encoded body of the exchange will be mapped to HTTP."
            + " Setting this to false will avoid the HTTP Form Encoded body mapping.")
    boolean mapHttpMessageFormUrlEncodedBody = true;
    @UriParam(label = "producer,advanced", defaultValue = "200-299", description = "The status codes which are considered a success response. The values are inclusive. Multiple ranges can be"
    ...
    @UriParam(label = "producer,security", enums = "Basic,Bearer,Digest,NTLM", description = "Authentication methods allowed to use as a comma separated list of values Basic, Bearer, Digest or NTLM. (NTLM is deprecated)")
    private String authMethod;
    @UriParam(label = "producer,security", enums = "Basic,Bearer,Digest,NTLM", description = "Which authentication method to prioritize to use, either as Basic, Bearer, Digest or NTLM. (NTLM is deprecated)")
    ...
    @UriParam(label = "producer,security", defaultValue = "5", description = "Amount of time which is deducted from OAuth2 tokens expiry time to compensate for the time it takes OAuth2 Token Endpoint to send the token over http, in seconds. "
            +
    ...
    /**
     * If the option is true, HttpProducer will ignore the Exchange.HTTP_URI header,
     * and use the endpoint's URI for
     * request. You may also set the option throwExceptionOnFailure to be false to
     * let the HttpProducer send all the
     * fault response back.
     */
    ...
    /**
     * If the option is true, HttpProducer will set the Host header to the value
     * contained in the current exchange Host
     * header, useful in reverse proxy applications where you want the Host header
     * received by the downstream server to
     * reflect the URL called by the upstream client, this allows applications which
     * use the Host header to generate
     ...
     * the URI prefix if no exact match is
     * found.
     * <p/>
     * See more details at:
     ...
     * (Camel will read the stream into a
     * in memory/overflow to file, Stream caching) cache. By default Camel will
     * cache the Servlet input stream to
     * support reading it multiple times to ensure it Camel can retrieve all data
     * from the stream. However you can set
     * this option to true when you for example need to access the raw stream, such
     * as streaming it directly to a file
     * or other persistent store. DefaultHttpBinding will copy the request input
     * stream into a stream cache and put it
     * into message body if this option is false to support reading the stream
     * multiple times. If you use Servlet to
     * bridge/proxy an endpoint then consider enabling this option to improve
     * performance, in case you do not need to
     * read the message payload multiple times. + The http producer will by default
     * cache the response body stream. If
     * setting this option to true, + then the producers will not cache the response
     * body stream but use the response
    ...
    /**
     * If this option is false Servlet will disable the HTTP streaming and set the
    ...
    /**
     * If this option is true, the producer will add a Connection Close header to
     * HTTP Request
     */
    ...
    /**
     * If enabled and an Exchange failed processing on the consumer side the
     * exception's stack trace will be logged when
    ...
    /**
     * If this option is true, The http producer won't read response body and cache
     * the input stream.
     */
     ...
     * inclusive. Multiple ranges can be
     * defined, separated by comma, e.g. <tt>200-204,209,301-304</tt>. Each range
     * must be a single number or from-to
    ...
    /**
     * If this option is true then IN exchange Form Encoded body will be mapped to
     * HTTP
     */
    ...
    /**
     * Authentication methods allowed to use as a comma separated list of values
    ...
    /**
     * Which authentication method to prioritize to use, either as Basic, Digest or
     * NTLM.
     */
    ...
    /**
     * Default expiration time for cached OAuth2 tokens, in seconds. Used if token
    ...
    /**
     * Amount of time which is deducted from OAuth2 tokens expiry time to compensate
     * for the time it takes OAuth2 Token
     * Endpoint to send the token over http, in seconds. Set this parameter to high
     * value if you OAuth2 Token Endpoint
     * answers slowly or you tokens expire quickly. If you set this parameter to too
     * small value, you can get 4xx http
     * errors because camel will think that the received token is still valid, while
     * in reality the token is expired for
