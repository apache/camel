...
._Java-only: minimum setup for MicrometerObservabilityTracer in standalone Camel_
```java
...
._Java-only: configuring the OpenTelemetry bridge with Micrometer tracing_
```java
import org.apache.camel.micrometer.observability.MicrometerObservabilityTracer;
...
._Java-only: creating a custom span within a Camel processor_
[source,java]
----
...
._Java-only: setting baggage properties and retrieving them via Micrometer API_
[source,java]
----
                from("direct:start")
