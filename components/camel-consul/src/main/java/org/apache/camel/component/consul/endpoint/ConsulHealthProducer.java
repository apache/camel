...
public final class ConsulHealthProducer extends AbstractConsulProducer<HealthClient> {
    ...
    protected void nodeChecks(Message message) throws Exception {
        processConsulResponse(message,
                              getClient().getNodeChecks(getMandatoryHeader(message, ConsulConstants.CONSUL_NODE, String.class), buildQueryOptions(message, getConfiguration())));
    }

    @InvokeOnHeader(ConsulHealthActions.SERVICE_CHECKS)
    protected void serviceChecks(Message message) throws Exception {
        processConsulResponse(message, getClient().getServiceChecks(getMandatoryHeader(message, ConsulConstants.CONSUL_SERVICE, String.class),
    ...
    protected void serviceInstances(Message message) throws Exception {
        ...
        if (healthyOnly) {
            processConsulResponse(message, getClient().getHealthyServiceInstances(getMandatoryHeader(message, ConsulConstants.CONSUL_SERVICE, String.class),
                                                                                  buildQueryOptions(message, getConfiguration())));
        } else {
            processConsulResponse(message, getClient().getAllServiceInstances(getMandatoryHeader(message, ConsulConstants.CONSUL_SERVICE, String.class),
                                                                              buildQueryOptions(message, getConfiguration())));
        }
    }
    ...
    protected void checks(Message message) throws Exception {
        processConsulResponse(message, getClient().getChecksByState(getMandatoryHeader(message, ConsulConstants.CONSUL_HEALTHY_STATE, State.class),
                                                                    buildQueryOptions(message, getConfiguration())));
    }
}
