...
[NOTE]
====
To allow the Bot to receive all messages exchanged within a group or channel (not just
...
It can also be used in *reactive chatbot mode* (to consume, then produce messages).
...
`OutgoingTextMessage` has the property `ReplyMarkup` which can be used for such a thing.
