...
| *acceptTimeout* (timeout) | Timeout (in milliseconds) while waiting for a TCP connection <p/> TCP Server Only | 60000 | int
...
| *connectTimeout* (timeout) | Timeout (in milliseconds) for establishing for a TCP connection <p/> TCP Client only | 30000 | int
