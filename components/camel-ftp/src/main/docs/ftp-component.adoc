...
| *resumeDownload* (consumer) | Configures whether resume download is enabled. This requires support from the FTP server (which almost all FTP server does). In addition the option localWorkDirectory must be configured so downloaded files are stored in a local directory which is required for support resume downloads. | false | boolean
