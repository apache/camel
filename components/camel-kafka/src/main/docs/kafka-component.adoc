...
NOTE: The `KafkaIdempotentRepository` bean must be registered in the Camel registry before it can be referenced by the route.
...
The exchange carrying the batch also exposes the record metadata headers that every record in the batch agrees on:
`CamelKafkaTopic` and `CamelKafkaPartition`. This makes it possible to read them on the batch itself, before
splitting it. When the batch is split, each child exchange carries the body and the headers of its own record:
...
A header is only set on the batch when *every* record in the batch carries the same value for it. If the batch spans
multiple topics or partitions, the corresponding header is not set, since no single value would be correct for the
batch as a whole. Per-record headers that naturally differ, such as `CamelKafkaOffset`, are never set on the batch —
...
TIP: In a production scenario, applications should send failed records to a separate topic (dead-letter queue) or fix the condition that led to the failure.
...
TIP: In XML and YAML, register the `GroupedExchangeAggregationStrategy` as a bean named `groupedExchange` in the registry.
...
NOTE: The `FileStateRepository` bean must be registered in the Camel registry before it can be referenced by the route.
