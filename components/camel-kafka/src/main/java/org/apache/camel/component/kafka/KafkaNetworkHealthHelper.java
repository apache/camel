...
final class KafkaNetworkHealthHelper {
    ...
    static boolean consumerHasReadyNodes(Consumer consumer) {
        ...
        try {
            ...
            // Classic consumer (group.protocol=classic)
            try {
            ...
            // Async consumer (group.protocol=consumer, KIP-848)
            if ("AsyncKafkaConsumer".equals(delegate.getClass().getSimpleName())) {
                ...
                LOG.trace("Health-Check calling KafkaClient.hasReadyNodes on AsyncKafkaConsumer");
    ...
    static boolean producerHasReadyNodes(org.apache.kafka.clients.producer.Producer<?, ?> producer) {
        ...
        try {
            ...
            LOG.debug("Cannot check hasReadyNodes on KafkaProducer due to: {}. Treating as not ready.", e.getMessage(), e);
            return false;
        }
    }
}
