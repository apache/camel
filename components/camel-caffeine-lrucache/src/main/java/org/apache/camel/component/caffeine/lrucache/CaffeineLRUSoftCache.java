 ...
 * <p/>
 * This implementation uses {@link java.lang.ref.SoftReference} for stored values in the cache, to support the JVM when
 * it wants to reclaim objects when it's running out of memory. Therefore this implementation does not support
 ...
 * <p/>
 * Notice that if the JVM reclaims memory, the content of this cache may be garbage collected without any eviction
 * notifications.
 * <p/>
...
public class CaffeineLRUSoftCache<K, V> extends CaffeineLRUCache<K, V> {
    ...
    public CaffeineLRUSoftCache(int initialCapacity, int maximumCacheSize, boolean stopOnEviction) {
