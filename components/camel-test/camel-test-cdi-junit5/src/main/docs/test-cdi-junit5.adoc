= Test CDI JUnit5 Component (deprecated)
...
There are a number of supported approaches gqfor testing with CDI in
Camel:
...
* JUnit 5
 ...
 a|
The Camel CDI test module (`camel-test-cdi-junit5`) provides a JUnit extension
...
With this approach, your test classes use the JUnit extension provided in
Camel CDI test. This extension manages the lifecycle of a standalone CDI
...
JUnit 5 provides out of the box the `@Order` annotation that you can use to
...
control how the extension instantiates the test class, either one test class
...
alternative bean in your tests. If you use the `CamelCdiExtension` JUnit
extension, you can do that with the `@Beans` annotation provided by the
...
[[CDITesting-RoutesadvisingwithAdviceRoute]]
== Routes advising with `@AdviceRoute`

The annotation `@AdviceRoute` can be used to _advice_ an existing route
before its being tested. It allows adding http://camel.apache.org/intercept.html[Intercept]
or _weave_ routes for testing purpose, for example using
the xref:ROOT:mock-component.adoc[Mock] component.

The extension detects automatically all classes of type `AdviceWithRouteBuilder` annotated with
`@AdviceRoute`, instantiates them and delays the Camel context start-up to ensure that the target
routes are advised properly.

In the next example, the `AdviceWithRouteBuilder` of type `ReplaceDirectWithMockBuilder` advises the
