= Test Spring JUnit6 Component
:doctitle: Test Spring JUnit6
:shortname: test-spring-junit6
:artifactid: camel-test-spring-junit6
...
The `camel-test-spring-junit6` module is used for testing Camel with Spring;
both the classic Spring XML files or Spring Boot.

== Testing Spring Boot

The recommended approach is to annotate the test class with `org.apache.camel.test.spring.junit6.CamelSpringBootTest`.
...
This approach provides feature parity with `org.apache.camel.test.junit6.CamelTestSupport` but does not support Spring annotations
...
However, it does NOT have feature parity with `org.apache.camel.test.junit6.CamelTestSupport`.

== Camel test annotations

The following annotations can be used with `camel-spring-junit6` unit testing.
...
| `@EnableRouteDiagramDump` | Enable dumping route diagrams into the specified folder. This requires having camel-diagram on the classpath.
...
Find below some hints to help in migrating Camel Spring tests from JUnit 4 to JUnit 6.

Projects using `camel-test-spring` would need to use `camel-test-spring-junit6`. For instance, maven users would update their pom.xml file as below:
...
TIP: It's possible to run JUnit 5 & JUnit 6 based Camel Spring tests side by side including the following dependencies `camel-test-spring-junit5`,
`camel-test-spring-junit6` and `junit-vintage-engine`. This configuration allows migrating Camel tests one by one.

=== Migration steps

* Migration steps from camel-test-junit6 should have been applied first
* Imports of `org.apache.camel.test.spring.\*` should be replaced with `org.apache.camel.test.spring.junit6.*`
