...
Alternatively, the scheduler can be started and configured entirely through component options - no Java code - by setting
`camel.component.pqc.*` properties (for example in `application.properties`). Bind a `KeyLifecycleManager` in the
...
When `keyRotationSchedulerEnabled` is `true`, the `pqc` component builds a `KeyRotationScheduler` around the configured
`keyLifecycleManager` and starts and stops it with the component. The same options are available in the YAML DSL and as
`PQCComponent` setters, which makes the scheduler usable from Spring Boot and Quarkus without any Java code.
