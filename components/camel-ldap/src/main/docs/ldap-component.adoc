...
IMPORTANT: The camel-ldap component uses the message body to filter the search results.
...
If no specific filter is required (for example, you need to look
up a single entry), specify a wildcard filter expression.
