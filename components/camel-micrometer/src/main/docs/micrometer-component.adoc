...
| *decrement* (producer) | Decrement value when using counter type |  | Double
| *increment* (producer) | Increment value when using counter type |  | Double
| *value* (producer) | Value value when using histogram type |  | Double
|===
...
By default the Camel Micrometer component uses a `SimpleMeterRegistry` instance.
...
The meter name defined in URI can be overridden by using a header with name `CamelMetricsName`.
The meter tags defined as URI parameters can be augmented by using a header with name `CamelMetricsTags`.
...
tag `dynamic-key` with value `dynamic-value` in addition to the tag `static-key` with value `static-value`.
