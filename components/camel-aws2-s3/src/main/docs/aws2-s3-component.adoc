...
| *moveAfterRead* (consumer) | Move objects from S3 bucket to a different bucket after they have been retrieved. The copy bucket operation is only performed if the Exchange is committed. If a rollback occurs, the object is not moved. | false | boolean
