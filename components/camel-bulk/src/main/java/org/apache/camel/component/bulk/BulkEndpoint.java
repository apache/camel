...
/**
 * Process collections of items in structured bulk operations with chunking, error thresholds, and watermark tracking.
 */
@UriEndpoint(firstVersion = "4.19.0", scheme = "bulk", title = "Bulk",
             syntax = "bulk:jobName", producerOnly = true,
             remote = false, category = { Category.CORE, Category.SCHEDULING },
             headersClass = BulkConstants.class)
public class BulkEndpoint extends DefaultEndpoint {

    @UriPath(description = "The bulk job name")
    ...
    @UriParam(defaultValue = "1.0", description = "Fraction of failed items (0.0-1.0) before aborting the bulk operation."
                                                  + " 1.0 means never abort. 0.1 means abort after 10% failures.")
    private double errorThreshold = 1.0;

    @UriParam(defaultValue = "-1", description = "Maximum number of failed records before aborting the bulk operation."
    ...
    @UriParam(description = "Comma-separated list of endpoint URIs or bean references for multi-step bulk processing."
    ...
    private BulkAcceptPolicy acceptPolicy = BulkAcceptPolicy.ALL;

    @UriParam(description = "Optional bean reference to an AggregationStrategy for collecting results")
    private String aggregationStrategy;

    @UriParam(description = "Optional endpoint URI or bean reference called after the bulk operation completes,"
                            + " regardless of success or failure. Receives the BulkResult as the exchange body.")
    ...
    @UriParam(description = "A Simple expression evaluated on each successfully processed item to extract a watermark value."
                            ...
                            + " and the current watermark value is set as a CamelBulkWatermarkValue header instead.")
    ...
    public Producer createProducer() throws Exception {
        ...
        if (maxFailedRecords < -1) {
            throw new IllegalArgumentException("maxFailedRecords must be >= -1, was: " + maxFailedRecords);
        }
        ...
        return new BulkProducer(this);
    }

    @Override
    public Consumer createConsumer(Processor processor) throws Exception {
        throw new UnsupportedOperationException("The bulk component does not support consumers");
    }
    ...
    public BulkAcceptPolicy getAcceptPolicy() {
        return acceptPolicy;
    }

    public void setAcceptPolicy(BulkAcceptPolicy acceptPolicy) {
