...
public class SmbDefaultMoveExistingFileStrategy implements FileMoveExistingStrategy {

    private static final Logger LOG = LoggerFactory.getLogger(SmbDefaultMoveExistingFileStrategy.class);
    ...
    public boolean moveExistingFile(GenericFileEndpoint<?> endpoint, GenericFileOperations<?> operations, String fileName)
        ...
        dummy.getIn().setHeader(SmbConstants.FILE_NAME, fileName);
        dummy.getIn().setHeader(SmbConstants.FILE_NAME_ONLY, onlyName);
        dummy.getIn().setHeader(SmbConstants.FILE_PARENT, parent);
        ...
        if (operations.existsFile(to)) {
            if (endpoint.isEagerDeleteTargetFile()) {
                ...
                boolean result;
                result = operations.deleteFile(to);
                if (!result) {
