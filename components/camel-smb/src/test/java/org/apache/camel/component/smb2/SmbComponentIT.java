    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            private void process(Exchange exchange) throws IOException {
                final Smb2File file = exchange.getMessage().getBody(Smb2File.class);
            ...
            public void configure() {
                ...
                fromF("direct:smbSendFile")
                        .to("smb2:%s/%s?username=%s&password=%s&path=/&smbConfig=#smbConfig")
                        .to("smb2:%s/%s?username=%s&password=%s&path=/")
                        .to("mock:result");
