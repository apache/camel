...
public class KubernetesDeploymentsConsumerNamespaceIT extends KubernetesConsumerTestSupport {
    @Test
    public void namespaceTest() {
        createDeployment(ns1, "d1", null);
        createDeployment(ns2, "d2", null);

        Awaitility.await().atMost(5, TimeUnit.SECONDS).untilAsserted(() -> {
            final List<String> list = result.getExchanges().stream()
                    .map(ex -> ex.getIn().getBody(String.class))
                    .toList();
            assertThat(list, allOf(hasItem(containsString("d2")), not(hasItem(containsString("d1")))));
        });
    }
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                fromF("kubernetes-deployments://%s?oauthToken=%s&namespace=%s", host, authToken, ns2)
