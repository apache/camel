...
/**
 * Handler for OpenID Connect authentication ({@code type=openIdConnect}). On the producer side, acquires a token via
 * the {@code camel-oauth} SPI. On the consumer side, validates the Bearer token via
 * {@link OAuthHelper#validateOAuthToken} for full JWT/introspection-based validation.
 */
public class OpenIdConnectSchemeHandler implements A2ASecuritySchemeHandler {

    private static final Logger LOG = LoggerFactory.getLogger(OpenIdConnectSchemeHandler.class);
    ...
    public void applyCredentials(Exchange exchange, SecurityScheme scheme, A2AConfiguration config, CamelContext context) {
        if (config.getOauthProfile() == null) {
            LOG.warn("OpenID Connect scheme selected but no oauthProfile configured");
            return;
        }
        try {
            ...
            throw new RuntimeCamelException(
                    "Failed to acquire OIDC token for profile '"
    ...
    public A2AUserProfile validateCredentials(Exchange exchange, SecurityScheme scheme, A2AConfiguration config) {
        ...
        return BearerTokenExtractor.validateBearerToken(exchange, token, config, "openIdConnect");
    }
}
