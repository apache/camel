...
/**
 * Implementation of the GetTask A2A operation. Infrastructure-only operation for retrieving task state from the task
 * store without dispatching to the user's route.
 */
public class TaskGetOperation implements A2AOperation {

    @Override
    public A2AOperations getOperationType() {
        return A2AOperations.TASK_GET;
    }
    ...
    public void parseRequest(Exchange exchange, Object request) {
        TaskGetRequest taskGetRequest = (TaskGetRequest) request;

        exchange.getMessage().setHeader(A2AConstants.TASK_ID, taskGetRequest.getId());
        exchange.getMessage().setHeader(A2AConstants.OPERATION, methodName());

        if (taskGetRequest.getHistoryLength() != null) {
            exchange.getMessage().setHeader(A2AConstants.HISTORY_LENGTH, taskGetRequest.getHistoryLength());
        }
    }

    @Override
    public Object buildResponse(Exchange exchange) {
        // Return the task from the exchange body
