...
components (default is `true` when a tracing or metrics backend is present). Camel Main exposes the same setting
via `camel.ai.observability.enabled` in application.properties or programmatically with
