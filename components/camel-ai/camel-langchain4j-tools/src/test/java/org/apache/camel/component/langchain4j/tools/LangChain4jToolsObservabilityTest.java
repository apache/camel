...
class LangChain4jToolsObservabilityTest extends CamelTestSupport {
    ...
    protected CamelContext createCamelContext() throws Exception {
        ...
        component.getConfiguration().setChatModel(new StubChatModel());
        return context;
    }
    ...
    protected RoutesBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:start")
                        .to("langchain4j-tools:invoke?tags=observability")
                        .to("mock:result");

                from("langchain4j-tools:dummy?tags=observability&description=Dummy tool")
                        .setBody(constant("tool-result"));
            }
        };
    }

    @Test
    void shouldEmitGenAiSpanFromToolsProducer() throws Exception {
        MockEndpoint mock = getMockEndpoint("mock:result");
        mock.expectedMessageCount(1);

        template.sendBody("direct:start", List.of(UserMessage.from("Hello")));
        ...
        assertThat(tags.get(GenAiAttributes.CAMEL_COMPONENT)).isEqualTo("langchain4j-tools");
    }
