...
def handle(inputs: Input):
    try:
        ...
        # Extract generated text (default to first item)
        generated_text = result[0]['generated_text'] if result and isinstance(result, list) and len(result) > 0 and 'generated_text' in result[0] else ''
        outputs = Output()
        outputs.add(generated_text, "data")
        return outputs
