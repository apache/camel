...
public class SentenceEmbeddingsPredictor extends AbstractTaskPredictor {
    ...
    public SentenceEmbeddingsPredictor(HuggingFaceConfiguration config) {
        super(config);
    }
        ...
        return """
                ...
                def handle(inputs: Input):
                    try:
                        ...
                        input_list = json.loads(input_str) # Expecting a JSON list of strings

                        torch.manual_seed(42)
                        embeddings = model.encode(input_list, convert_to_numpy=True)
                        ...
                        return outputs
                                """.formatted(config.getDevice(), config.getModelId());
    }
