...
class OpenAIAudioTranscriptionObservabilityTest extends CamelTestSupport {

    @RegisterExtension
    static OpenAIMock openAIMock = new OpenAIMock().builder()
            .whenTranscription()
            .replyWithTranscription("Observed transcription")
            .end()
            .build();
    ...
    @Test
    void shouldEmitTranscriptionGenAiSpan() {
        Exchange result = template.request("direct:transcribe",
                e -> e.getIn().setBody(new byte[] { 0x00, 0x01 }));

        assertThat(result.getException()).isNull();
        assertThat(result.getMessage().getBody(String.class)).isEqualTo("Observed transcription");
        ...
        assertThat(tags.get(GenAiAttributes.OPERATION_NAME)).isEqualTo("transcription");
        assertThat(tags.get(GenAiAttributes.SYSTEM)).isEqualTo("openai");
        assertThat(tags.get(GenAiAttributes.REQUEST_MODEL)).isEqualTo("whisper-1");
