...
class OpenAIAudioTranslationObservabilityTest extends CamelTestSupport {

    @RegisterExtension
    static OpenAIMock openAIMock = new OpenAIMock().builder()
            .whenTranslation()
            .replyWithTranslation("Observed translation")
            .end()
            .build();
    ...
    @Test
    void shouldEmitTranslationGenAiSpan() {
        Exchange result = template.request("direct:translate",
                e -> e.getIn().setBody(new byte[] { 0x00, 0x01 }));

        assertThat(result.getException()).isNull();
        assertThat(result.getMessage().getBody(String.class)).isEqualTo("Observed translation");
        ...
        assertThat(tags.get(GenAiAttributes.OPERATION_NAME)).isEqualTo("translation");
