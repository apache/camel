...
* **`toolExecutionErrorStrategy`** (default `repromptModel`): controls what happens when a tool
  ...
  - `failExchange` -- propagates the exception to the Camel exchange so that standard Camel error
    handling (`onException`, dead-letter channel) can process it.
...
| Tool execution throws an exception | Governed by `toolExecutionErrorStrategy`: `repromptModel` (default) catches the error, logs it as WARN, and sends it as tool result text to the model; `failExchange` propagates the exception to the exchange
