...
The `CamelDoclingOutputFilePath` header, which selects the CLI `--output` directory, is normalized the same way
and is confined to `outputBaseDirectory` when one is set. Leaving `outputBaseDirectory` unset keeps the previous
behavior, where the header value is used as given after normalization.
