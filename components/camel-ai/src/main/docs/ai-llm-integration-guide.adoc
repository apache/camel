...
For LangChain4j Chat, use `CHAT_SINGLE_MESSAGE_WITH_PROMPT` with `{{variable}}` placeholders — see xref:langchain4j-chat-component.adoc#_send_a_prompt_with_variables[Send a prompt with variables].
