...
* You use prompt templates with `{{variable}}` substitution (see <<prompt-templates>>)
...
See xref:openai-component.adoc#Structured Output with JSON Schema[Structured Output with JSON Schema] for header-based and inline schema examples.
...
- to:
    ...
    parameters:
      model: gpt-4o-mini
      additionalBodyProperty.temperature: "0.1"
      additionalBodyProperty.seed: "42"
----

NOTE: Prefer the dedicated `temperature` option when available — it is clearer and validated by the component. Use `additionalBodyProperty` for vendor-specific knobs (reasoning effort, provider routing, etc.).

=== LangChain4j Chat

Configure temperature on the `ChatModel` bean (Spring Boot properties or programmatic builder). See xref:langchain4j-chat-component.adoc#Using a specific Chat Model[LangChain4j Chat — Using a specific Chat Model].
...
See xref:openai-component.adoc#Basic Chat Completion with String Input[OpenAI — file-based prompts].
...
For LangChain4j Chat, use `CHAT_SINGLE_MESSAGE_WITH_PROMPT` with `{{variable}}` placeholders — see xref:langchain4j-chat-component.adoc#Send a prompt with variables[Send a prompt with variables].
...
from("platform-http:/chat/stream?httpMethodRestrict=POST")
    ...
    .split(body().isInstanceOf(Iterator.class)).streaming()
        .process(exchange -> {
...
| You only proxy an upstream SSE stream verbatim (consider xref:a2a-producer.adoc#Raw Passthrough (RAW mode)[A2A RAW passthrough])
