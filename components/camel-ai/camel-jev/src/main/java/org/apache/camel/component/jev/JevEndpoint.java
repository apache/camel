...
@UriEndpoint(firstVersion = "4.23.0", scheme = "jev", title = "Jev", syntax = "jev:name",
             producerOnly = true, category = { Category.AI })
public class JevEndpoint extends DefaultEndpoint {
    @UriPath
    @Metadata(required = true)
    private String name;
    @UriParam
    private JevConfiguration configuration;
    private JsonObject configuredQuestions;
    private Expression stateExpression;

    private volatile JevClient client;

    public JevEndpoint(String uri, JevComponent component, String name, JevConfiguration configuration) {
        super(uri, component);
        this.name = name;
    ...
    public Producer createProducer() {
        return new JevProducer(this);
    }

    @Override
    public Consumer createConsumer(Processor processor) {
        throw new UnsupportedOperationException("Jev is producer only");
    }
    ...
    public JevConfiguration getConfiguration() {
        return configuration;
    }

    /** The Jev API configuration. */
    public void setConfiguration(JevConfiguration configuration) {
        this.configuration = configuration;
    }
