...
/**
 * The MCP engine accepts clients before the routes start, so all tools and resources must be published before any route
 * consumer starts, otherwise an early {@code tools/list} returns a partial list.
 */
class McpServerBridgeStartupOrderTest extends CamelTestSupport {
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            public void configure() {
                // declared first: its consumer starts before the ai-tool and ai-resource consumers
    ...
    void toolsAndResourcesArePublishedBeforeTheFirstRouteConsumerStarts() {
        assertThat(seenByFirstConsumer).containsExactlyInAnyOrder("query_db", "camel:///config/app.json");
    }
