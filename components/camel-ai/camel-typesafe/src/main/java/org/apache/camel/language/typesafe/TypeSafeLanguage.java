 ...
 */
package org.apache.camel.language.typesafe;
...
import org.apache.camel.component.typesafe.TypeSafeConfiguration;
import org.apache.camel.component.typesafe.TypeSafeEndpoint;
import org.apache.camel.component.typesafe.TypeSafeUncertainResultException;
...
/** Evaluates a plain-text Noul question against selected exchange state through a managed TypeSafe endpoint. */
@Language(value = "typesafe", modelName = "language")
@Metadata(title = "TypeSafe", description = "Evaluate a Noul question with the TypeSafe AI decision API",
          label = "language,ai", firstVersion = "4.23.0")
public class TypeSafeLanguage extends LanguageSupport {
    public static final String RESULT = "CamelTypeSafeResult";
    ...
    private String endpoint = "typesafe:default";
    ...
    public boolean validateExpression(String expression) {
        if (expression == null || expression.isBlank()) {
            throw new IllegalArgumentException("TypeSafe question must not be null or blank");
        }
        return true;
    }
    ...
    /** Managed endpoint URI. Defaults to typesafe:default, inheriting camel.component.typesafe settings. */
    ...
    private static final class Evaluation extends ExpressionAdapter {
        ...
        private TypeSafeEndpoint endpoint;
        ...
        public void init(CamelContext context) {
            super.init(context);
            if (endpointUri == null || !endpointUri.startsWith("typesafe:")) {
                throw new IllegalArgumentException("TypeSafe language endpoint must be a typesafe: URI");
            }
            // Register once with Camel, including when no producer uses this endpoint.
            endpoint = context.getEndpoint(endpointUri, TypeSafeEndpoint.class);
            TypeSafeConfiguration configuration = endpoint.getConfiguration();
            ...
            if (selection instanceof Expression expression) {
                ...
                throw new IllegalArgumentException("TypeSafe state must be an Expression or a nonblank Simple string");
            }
            state.init(context);
        }
        ...
        public Object evaluate(Exchange exchange) {
            ...
            try {
                if (endpoint == null) {
                    throw new IllegalStateException("TypeSafe expression must be initialized");
                }
                Object selected = state.evaluate(exchange, Object.class);
                if (selected == null) {
                    throw new IllegalArgumentException("TypeSafe state must not be null");
                }
                ...
                if (uncertainty > 0 && probability >= threshold - uncertainty && probability <= threshold + uncertainty) {
                    if (uncertaintyPolicy == UncertaintyPolicy.Fail) {
                        throw new TypeSafeUncertainResultException(probability);
                    }
                    return false;
                }
        ...
        public String toString() {
            ...
            return "typesafe[endpoint=" + (endpointUri == null ? "" : endpointUri.split("\\?", 2)[0])
                   + ", threshold=" + threshold + "]";
        }
    }
}
