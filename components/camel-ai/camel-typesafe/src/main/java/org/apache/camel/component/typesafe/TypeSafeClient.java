...
final class TypeSafeClient implements AutoCloseable {
    ...
    TypeSafeClient(TypeSafeConfiguration configuration) {
    ...
    JsonObject evaluate(Map<String, Object> input) throws Exception {
        synchronized (this) {
            if (closed) {
                throw new IllegalStateException("TypeSafe client is stopped");
            }
            if (activeRequests >= maxConcurrentRequests) {
                throw new RejectedExecutionException("TypeSafe maxConcurrentRequests limit reached: " + maxConcurrentRequests);
            }
            activeRequests++;
        }
        try {
    ...
    private JsonObject send(Map<String, Object> input) throws Exception {
        JsonObject request = TypeSafeJson.request(input, model);
        ...
        try {
            ...
            return TypeSafeJson.response(response.body(), request);
            ...
            if (e.getCause() instanceof HttpTimeoutException cause) {
                TimeoutException timeoutException = new TimeoutException("TypeSafe request timed out");
