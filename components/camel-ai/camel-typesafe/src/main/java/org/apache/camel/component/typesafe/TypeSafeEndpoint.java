...
@UriEndpoint(firstVersion = "4.23.0", scheme = "typesafe", title = "TypeSafe", syntax = "typesafe:name",
             producerOnly = true, category = { Category.AI })
public class TypeSafeEndpoint extends DefaultEndpoint {
    @UriPath
    ...
    public TypeSafeEndpoint(String uri, TypeSafeComponent component, String name, TypeSafeConfiguration configuration) {
        super(uri, component);
        this.name = name;
    ...
    public Producer createProducer() {
        return new TypeSafeProducer(this);
    }

    @Override
    public Consumer createConsumer(Processor processor) {
        throw new UnsupportedOperationException("TypeSafe is producer only");
    }
    ...
    protected void doStart() throws Exception {
        ...
        if (configuration.getQuestions() != null) {
            configuredQuestions = TypeSafeJson.questions(configuration.getQuestions());
            stateExpression = createStateExpression();
        }
        client = new TypeSafeClient(configuration);
    }

    @Override
    protected void doStop() throws Exception {
        TypeSafeClient current = client;
        client = null;
        if (current != null) {
            current.close();
        }
    ...
    public JsonObject evaluate(Map<String, Object> request) throws Exception {
        TypeSafeClient current = client;
        if (current == null) {
            throw new IllegalStateException("TypeSafe endpoint is not started");
        }
    ...
    public TypeSafeConfiguration getConfiguration() {
        return configuration;
    }

    /** The TypeSafe API configuration. */
    public void setConfiguration(TypeSafeConfiguration configuration) {
        this.configuration = configuration;
    }
