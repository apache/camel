...
public class LangChain4jChatConfiguration implements Cloneable {
     ...
     * apiKey, temperature, timeout, and provider-specific model.* properties) instead of a ChatModel bean. The
     * LangChain4j module of the provider (dev.langchain4j:langchain4j-ollama, ...) must be on the classpath; Camel
     * JBang downloads it. Ignored when a ChatModel is configured. For a provider not listed, set customProvider
     * instead.
     */
