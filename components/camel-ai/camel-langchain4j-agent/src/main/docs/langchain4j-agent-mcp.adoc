...
When MCP clients are configured at the endpoint level, the same `withMcpToolProviderFilter(...)` predicate configured on the endpoint's `agentConfiguration` (or on a registry `AbstractAgent` bean) is applied to those clients as well. This keeps filtering behavior consistent regardless of whether MCP clients are declared on the agent or on the endpoint URI.
