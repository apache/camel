...
==== Using the responseType endpoint option (Java Class-based Schema)

As an alternative to providing a JSON schema directly, you can use the `responseType` endpoint option to specify a Java class. The Camel LangChain4j agent will automatically:

1. Derive the JSON schema from the class structure
2. Configure the AI model to return JSON matching that schema
3. Unmarshal the JSON response directly to an instance of your Java class
...
Then configure the endpoint to use this class as the response type. Use the fully qualified class name (FQCN) directly — no registry binding needed:
...
from("direct:extract-person-info")
    .setBody(constant("Extract information about John Smith, a 35-year-old software engineer"))
    .to("langchain4j-agent:structured?agentConfiguration=#myAgentConfig"
            + "&responseType=com.example.PersonInfo")
    .process(exchange -> {
        // Response is automatically unmarshalled to PersonInfo
...
[NOTE]
====
* Both `jsonSchema` and `responseType` only work when using `agentConfiguration` (inline agent creation mode)
...
* Complex nested objects and collections are fully supported with `responseType`
====
