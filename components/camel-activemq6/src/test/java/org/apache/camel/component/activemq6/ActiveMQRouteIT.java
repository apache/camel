    ...
    private static RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            public void configure() {
                from("activemq6:queue:jmsQueue").to("log:routing").to("mock:result");

                from("activemq6:queue:inOut").setBody().constant("response");

                from("activemq6:topic:jmsTopic").to("log:routing").to("mock:result");

                from("activemq6:topic:jmsTopic").to("log:routing").to("mock:result");

                from("activemq6:queue:wildcard.>").to("log:routing").to("mock:result");

                from("activemq6:queue:uriEndpoint").to("log:routing").to("mock:result");
            }
        };
    }
}
