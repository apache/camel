...
| *operation* (producer) | Queue service operation hint to the producer. There are 8 enums and the value can be one of: listQueues, createQueue, deleteQueue, addMessage, retrieveMessage, peekMessage, updateMessage, deleteMessage | listQueues | QueueServiceOperations
...
include::{page-component-version}@camel-spring-boot::page$azure-starter.adoc[]
