...
----
azure-files://account[.file.core.windows.net][:port]/share/directoryname?token[&options]
----
...
Consequently:
  ...
  - For the `changed` read lock, only the last modified
    can be relied on, furthermore a precision better than
    5 seconds might be problematic.
