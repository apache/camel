...
1. **Cloud Validation with Cache (`validateEventSource` and `validateEventType`)**:
   - **Validated Cache Update Workflow**: When validation is enabled, the component calls `listEventBuses` and `listRules` on Alibaba Cloud.
   - Verifies that:
     1. The target `eventBusName` exists on Alibaba Cloud.
     2. Each configured `allowedEventSource` is registered for that bus.
     3. Each configured `allowedEventType` is defined in the rule `filterPattern`s for that source on Alibaba Cloud.
   - **Only after validation succeeds** is the verified entry cached in `EventSourceCache` for `eventSourceCacheTtl` ms (default: 5 minutes) for fast runtime comparisons.

2. **Whitelist Validation (`allowedEventSources`)**:
...
* *Event Type Mapping*: Arrow (`->`) or equals (`=`) maps a source to its allowed comma-separated event types.
