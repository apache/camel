...
class DeleteRowTest extends CamelTestSupport {
    ...
    protected RouteBuilder createRouteBuilder() {
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:deleteRow")
                        .to("alibaba-ots:deleteRow"
    ...
    void testDeleteRow() throws Exception {
        DeleteRowResponse response = new DeleteRowResponse(
                new Response("req-delete-row"),
                null,
                new ConsumedCapacity(new CapacityUnit(0, 1)));

        when(otsClient.deleteRow(any(DeleteRowRequest.class))).thenReturn(response);

        MockEndpoint mock = getMockEndpoint("mock:result");
        mock.expectedMinimumMessageCount(1);

        DeleteRowRequest request = new DeleteRowRequest();
        template.sendBody("direct:deleteRow", request);
        ...
        assertThat(body)
                .containsEntry("requestId", "req-delete-row")
                .containsKey("consumedCapacity");

        verify(otsClient).deleteRow(any(DeleteRowRequest.class));
    }
}
