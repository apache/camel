...
public class JsonPathChoiceReaderTest extends CamelTestSupport {
    ...
    public void testCounterStringReader() throws Exception {
        ...
        template.sendBody("direct:start", new StringReader("{\"counter\": 1}"));
        template.sendBody("direct:start", new StringReader("{\"counter\": 0}"));
        template.sendBody("direct:start", new StringReader("{\"counter\": 2}"));
    ...
    public void testCounterReaderCache() throws Exception {
        getMockEndpoint("mock:positive").expectedBodiesReceived("{\"counter\": 4}", "{\"counter\": 3}");
        getMockEndpoint("mock:zero").expectedBodiesReceived("{\"counter\": 0}");
