...
For event ingestion (producer) use cases, consider migrating to xref:splunk-hec-component.adoc[Splunk HEC],
which uses the standard HTTP Event Collector and has no dependency on the unmaintained Splunk Java SDK.
Note that Splunk HEC is producer-only and does not support consumer (search) operations.
====
