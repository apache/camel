...
public interface InfinispanConstants {
    ...
    @Metadata(
            label = "producer",
            description = "A Map to use in case of `CamelInfinispanOperationPutAll` operation",
            javaType = "Map")
    ...
    @Metadata(
            label = "producer",
            description =
                    "The QueryBuilder to use for QUERY command, if not present the command defaults to InifinispanConfiguration's one",
            javaType = "org.apache.camel.component.infinispan.InfinispanQueryBuilder")
    String QUERY_BUILDER = "CamelInfinispanQueryBuilder";

    @Metadata(
            label = "consumer",
            description = "Provides access to the version of the created cache entry.",
            javaType = "long",
