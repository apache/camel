...
public class InfinispanTestContainersPutIfAbsentProducerTest extends InfinispanTestContainerSupport {
    ...
    public void testUriCommandOption() throws Exception {
        ...
        template.send("direct:putIfAbsent", new Processor() {
            ...
            public void process(Exchange exchange) throws Exception {
                exchange.getIn().setHeader(InfinispanConstants.KEY, COMMAND_KEY);
                exchange.getIn().setHeader(InfinispanConstants.VALUE, "CommandValue2");
            }
        });
        ...
        assertEquals(COMMAND_VALUE, result.getExchanges().get(0).getMessage().getBody(String.class));
    }
        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                ...
                from("direct:putIfAbsent")
                        .toF("infinispan:mycache?hosts=%s&operation=PUTIFABSENT&username=%s&password=%s&secure=true"
                             + "&saslMechanism=RAW(DIGEST-MD5)&securityRealm=default&securityServerName=infinispan",
                                getInfispanUrl(), service.username(), service.password());
                from("direct:get")
