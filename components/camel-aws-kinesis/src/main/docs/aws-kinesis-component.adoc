...
| *iteratorType* (consumer) | Defines where in the Kinesis stream to start getting records. There are 5 enums and the value can be one of: at-sequence-number, after-sequence-number, trim-horizon, latest, at-timestamp | trim-horizon | ShardIteratorType
