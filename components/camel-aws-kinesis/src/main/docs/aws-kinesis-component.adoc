...
$reference/components/aws-kinesis.adoc[opts=optional]
//Manually maintained attributes
:group: AWS
:camel-spring-boot-name: aws-kinesis
...
| *iteratorType* (consumer) | Defines where in the Kinesis stream to start getting records. There are 5 enums and the value can be one of: AT_SEQUENCE_NUMBER, AFTER_SEQUENCE_NUMBER, TRIM_HORIZON, LATEST, AT_TIMESTAMP | TRIM_HORIZON | ShardIteratorType
