...
public class EKS2ComponentVerifierExtension extends DefaultComponentVerifierExtension {
    ...
    protected Result verifyConnectivity(Map<String, Object> parameters) {
        ...
        try {
            ...
            EksClient client = clientBuilder.credentialsProvider(credentialsProvider).region(Region.of(configuration.getRegion())).build();
            client.listClusters(ListClustersRequest.builder().build());
        } catch (SdkClientException e) {
            ResultErrorBuilder errorBuilder = ResultErrorBuilder.withCodeAndDescription(VerificationError.StandardCode.AUTHENTICATION, e.getMessage())
                .detail("aws_eks_exception_message", e.getMessage()).detail(VerificationError.ExceptionAttribute.EXCEPTION_CLASS, e.getClass().getName())
