...
public class HttpAuthenticationTest extends BaseHttpTest {
    ...
    public void basicAuthenticationShouldSuccess() throws Exception {
        Exchange exchange = template.request("http://" + getHostName() + ":" + getPort() + "/search?username=" + user + "&password=" + password, new Processor() {
    ...
    public void basicAuthenticationShouldFailWithoutCreds() throws Exception {
        Exchange exchange = template.request("http://" + getHostName() + ":" + getPort() + "/search?throwExceptionOnFailure=false", new Processor() {
            public void process(Exchange exchange) throws Exception {
            }
        });
    ...
    public void basicAuthenticationShouldFailWithWrongCreds() throws Exception {
        Exchange exchange = template.request("http://" + getHostName() + ":" + getPort() + "/search?throwExceptionOnFailure=false&username=camel&password=wrong", new Processor() {
            public void process(Exchange exchange) throws Exception {
            }
        });
