...
public class HttpQueryTest extends BaseHttpTest {
    ...
    public void httpQueryHeader() {
        Exchange exchange = template.request(
                baseUrl + "/", exchange1 -> exchange1.getIn().setHeader(Exchange.HTTP_QUERY, "hl=en&q=camel"));
    ...
    public void httpQueryWithPasswordContainingNonAsciiCharacterAsQueryParams() {
        Exchange exchange = template.request(baseUrl + "/user/passwd", exchange1 -> exchange1
                .getIn()
                .setHeader(
                        Exchange.HTTP_QUERY,
                        "password='PasswordWithCharsThatNeedEscaping!≥≤!'&username=NotFromTheUSofA"));
    ...
    public void httpDanishCharactersAcceptedInBaseURL() {
        Exchange exchange = template.request(
                baseUrl + "/danish-accepted?characters='" + DANISH_CHARACTERS_UNICODE + "'", exchange1 -> {});
    ...
    public void httpDanishCharactersAcceptedAsQueryParams() {
        Exchange exchange = template.request(baseUrl + "/danish-accepted", exchange1 -> exchange1
                .getIn()
                .setHeader(Exchange.HTTP_QUERY, "characters='" + DANISH_CHARACTERS_UNICODE + "'"));
