...
xref:others:test-junit5.adoc[Test] endpoints
...
The `<package>` and `<packageScan>` will skip all classes that Spring has already
...
similar cross-cutting functionalities defined in the `<camelContext>`. In
...
to allow this. It provides two methods (`excludeRoute` and
