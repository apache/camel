...
._Java-only: uses `Consumer<String>` callback and `UserMessage` SDK type_
[source,java]
--------------------------------------------------------------------------------
...
NOTE: Requires the `camel-vertx-websocket` dependency. See the xref:vertx-websocket-component.adoc[Vert.x WebSocket component documentation] for more details.
...
._Java-only: uses `List.of()` for batch input and `process` lambda for typed result handling_
[source,java]
--------------------------------------------------------------------------------
from("direct:embed")
...
._Java-only: uses Qdrant SDK types (`Points.PointStruct`, `VectorsFactory`, etc.) for vector construction_
[source,java]
--------------------------------------------------------------------------------
...
NOTE: This example requires the `camel-qdrant` dependency. See the xref:qdrant-component.adoc[Qdrant component documentation] for more details.

=== Rerank Documents

Rerank documents by relevance to a query:

._Java-only: uses `process` lambda with `List.of()` for document input_
[source,java]
--------------------------------------------------------------------------------
from("direct:rerank")
  .process(exchange -> {
...
._Java-only: uses `List.of()` for document input_
[source,java]
--------------------------------------------------------------------------------
...
._Java-only: uses watsonx.ai SDK detector types (`Pii`, `Hap`)_
[source,java]
--------------------------------------------------------------------------------
...
._Java-only: illustrative snippets showing various file source integrations_
[source,java]
--------------------------------------------------------------------------------
...
._Java-only: complex async polling pattern with `loopDoWhile`, `delay`, `exchangeProperty`, and `choice`_
[source,java]
--------------------------------------------------------------------------------
...
._Java-only: uses watsonx.ai SDK types (`InputSchema`, `ForecastData`) for time series data construction_
[source,java]
--------------------------------------------------------------------------------
...
._Java-only: uses `process` lambda with `Map.of()` for tool input and config_
[source,java]
--------------------------------------------------------------------------------
from("direct:runTool")
  .process(exchange -> {
