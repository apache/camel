...
For more dynamic implementation, we can create the implementation class as follows:
...
@WebService(name = "TextService", serviceName = "TextService", targetNamespace = "http://server.soap.cxf.component.camel.apache.org/")
...
Then we can use https://camel.apache.org/components/next/bean-component.html[Bean] (don't forget to add `camel-bean` dependency to your project) invocation based on requested operation name:
...
----
    from("cxf:textServiceResponseFromImpl?serviceClass=org.apache.camel.component.cxf.soap.server.TextService&address=/text-impl")
                .process(exchange -> exchange.getIn().setHeader(BeanConstants.BEAN_METHOD_NAME, exchange.getIn().getHeader(CxfConstants.OPERATION_NAME)))
