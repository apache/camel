...
public class LRASagaRoutesTest {
    ...
    void testParseQuerySuccessUnencoded()
        ...
        Assertions.assertEquals(
                "direct://saga1_participant1_compensate",
                testResult.get("Camel-Saga-Compensate"),
        ...
        Assertions.assertEquals(
                "direct://saga1_participant1_complete",
                testResult.get("Camel-Saga-Complete"),
    ...
    @Test
    void testParseQueryInvalidUnencoded()
            throws InvocationTargetException, IllegalAccessException, NoSuchMethodException {

        Map<String, String> testResult =
                (Map<String, String>) getParseQueryMethod().invoke(new LRASagaRoutes(null), "key1=value1&key2");

        LOG.debug("Parsed query: {}", testResult);

        Assertions.assertNotNull(testResult, "pared query must not be null");
        Assertions.assertEquals(2, testResult.size(), "query parameter count must be two");
        Assertions.assertNotNull(testResult.get("key1"), "query parameter value for name 'key1' must not be null");
        Assertions.assertEquals(
                "value1", testResult.get("key1"), "query parameter value for name 'key1' has unexpected content");
        Assertions.assertNotNull(testResult.get("key2"), "query parameter value for name 'key2' must not be null");
        Assertions.assertEquals(
                "", testResult.get("key2"), "query parameter value for name 'key2' has unexpected content");
    }

    @DisplayName("Tests parseQuery() to handle incorrect query string (no value is given)")
    @Test
    void testParseQuerySuccessEncoded()
            throws InvocationTargetException, IllegalAccessException, NoSuchMethodException {
