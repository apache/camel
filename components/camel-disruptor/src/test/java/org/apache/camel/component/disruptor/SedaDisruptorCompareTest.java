...
public class SedaDisruptorCompareTest extends CamelTestSupport {
    ...
    private static final long[] LATENCY_HISTOGRAM_BOUNDS = new long[] {1, 2, 5, 10, 20, 50, 100, 200, 500,
            1000, 2000, 5000};
    ...
    public static Collection<Object[]> parameters() {
        ...
        if (SIZE_PARAMETER_VALUE == 0) {
            parameters.add(new Object[] {"SEDA LONG {P=1, C=1, CCT=1, SIZE=0}",
            ...
            parameters.add(new Object[] {"SEDA LONG {P=1, C=1, CCT=1, SIZE=" + SIZE_PARAMETER_VALUE + "}",
                    "seda:speedtest?concurrentConsumers=1&waitForTaskToComplete=IfReplyExpected&timeout=30000&multipleConsumers=false&limitConcurrentConsumers=true&blockWhenFull=true&size="
                            + SIZE_PARAMETER_VALUE, singleProducer(), singleConsumer(),
    ...
    private static void addParameterPair(final List<Object[]> parameters, final int producers,
        ...
        final String concurrentConsumerOptions = (parallelConsumerThreads > 1 ? "concurrentConsumers="
        ...
        parameters
                .add(new Object[] {"Disruptor" + testDescription, "disruptor:speedtest" + options, producers,
    ...
    private void outputExchangeAwaitersResult(final long start) throws InterruptedException {
        for (final ExchangeAwaiter exchangeAwaiter : exchangeAwaiters) {
            ...
            System.out.printf("%-45s time spent = %5d ms. Latency (ms): %s %n", componentName, (stop - start),
                    histogram.toString());
        }
    }
    ...
    private static final class ExchangeAwaiter implements Processor {
        ...
        public boolean awaitMessagesReceived(final long timeout, final TimeUnit unit)
