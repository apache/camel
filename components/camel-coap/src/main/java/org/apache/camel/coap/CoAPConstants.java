...
public interface CoAPConstants {
    ...
    @Metadata(
            description = "The request method that the CoAP producer should use when calling the target CoAP\n"
                    + "server URI. Valid options are DELETE, GET, PING, POST & PUT.",
            javaType = "String")
    String COAP_METHOD = "CamelCoapMethod";

    @Metadata(
            description = "The CoAP response code sent by the external server. See RFC 7252 for details\n"
