...
|===
| Name | Description | Default | Type
| *url* | *Required* Hostname and port for the solr server. Multiple hosts can be specified, separated with a comma.In that case, the scheme decides between CloudSolrClient and LBHttpSolrClient. |  | String
|===
...
| *autoCommit* (producer) | If true, each producer operation will be committed automatically | false | boolean
| *connectionTimeout* (producer) | connectionTimeout on the SolrClient |  | Integer
...
| *soTimeout* (producer) | Read timeout on the SolrClient |  | Integer
...
| *httpClient* (httpClient) | Uses provided http client to generate the solr client |  | HttpClient
...
| *solrClient* (solrClient) | Uses provided solr client to connect to solr |  | SolrClient
| *collection* (solrCloud) | Set the default collection for SolrCloud |  | String
| *zkChroot* (solrCloud) | Set the chroot of the zookeeper connections |  | String
| *zkHost* (solrCloud) | Set the ZooKeeper host urls which the CloudSolrClient uses, e.g. zkhost=localhost:8123,localhost:8124. Optionally add the chroot, e.g. zkhost=localhost:8123,localhost:8124/mysolr. |  | String
|===
