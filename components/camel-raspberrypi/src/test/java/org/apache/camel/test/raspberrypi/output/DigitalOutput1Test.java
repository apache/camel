        ...
        return new RouteBuilder() {
            public void configure() {
                from("timer://foo?repeatCount=1").id("rbpi-route").to("log:org.apache.camel.component.raspberrypi?showAll=true&multiline=true")
                    .to("raspberrypi://pin:7?mode=DIGITAL_OUTPUT&state=LOW&action=TOGGLE").to("raspberrypi://pin:8?mode=DIGITAL_OUTPUT&state=HIGH&action=LOW")
                    .to("raspberrypi://pin:9?mode=DIGITAL_OUTPUT&state=LOW&action=HIGH").to("mock:result");
