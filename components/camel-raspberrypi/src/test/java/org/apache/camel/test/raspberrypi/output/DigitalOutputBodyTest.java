 ...
 */
package org.apache.camel.test.raspberrypi.output;
...
import org.apache.camel.component.raspberrypi.mock.RaspiGpioProviderMock;
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() {
                from("timer://foo?repeatCount=1").id("rbpi-route").to("log:org.apache.camel.component.raspberrypi?showAll=true&multiline=true").transform().constant(true)
                    .to("raspberrypi://pin:5?mode=DIGITAL_OUTPUT").transform().simple("${body} == false").to("raspberrypi://pin:6?mode=DIGITAL_OUTPUT&state=HIGH").to("mock:result");
            }
        };
    }
}
