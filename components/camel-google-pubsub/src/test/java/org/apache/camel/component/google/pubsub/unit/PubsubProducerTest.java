...
public class PubsubProducerTest extends PubsubTestSupport {
    ...
    private static final String testTopicName="test-topic-name";

    @EndpointInject(uri = "google-pubsub:{{project.id}}:"+testTopicName)
    private Endpoint to;
    ...
    public void testProducerConfiguration() throws Exception {
        ...
        Endpoint endpoint = context.hasEndpoint(String.format("google-pubsub:%s:%s:1", PROJECT_ID, testTopicName));
        assertNotNull(String.format("Endpoint 'google-pubsub:%s:$s' is not found in Camel Context",
                                    PROJECT_ID,
                                    testTopicName), endpoint);
