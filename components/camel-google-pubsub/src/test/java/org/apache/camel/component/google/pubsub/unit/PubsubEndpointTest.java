...
public class PubsubEndpointTest extends PubsubTestSupport {
    ...
    private static final String testSubscriptionName = "test-sub-name";

    // For testing purposes the URI params need to be aligned in alphabetical order
    private static final String subscriptionURI = testSubscriptionName +
            "?ackMode=NONE" +
            "&concurrentConsumers=5" +
            "&maxMessagesPerPoll=2";

    @EndpointInject(uri = "google-pubsub://{{project.id}}:" + subscriptionURI)
    ...
    public void testEndpointConfiguration() throws Exception {
        ...
        Endpoint endpoint = context.hasEndpoint(String.format("google-pubsub:%s:%s:1", PROJECT_ID, subscriptionURI));
        assertNotNull(String.format("Endpoint 'google-pubsub:%s:%s' is not found in Camel Context",
                                    PROJECT_ID, subscriptionURI), endpoint);
        ...
        assertEquals("google-pubsub://" + PROJECT_ID + ":" + subscriptionURI, pubsubEndpoint.getEndpointUri());

        assertEquals(PROJECT_ID, pubsubEndpoint.getProjectId());
        assertEquals(testSubscriptionName, pubsubEndpoint.getDestinationName());
