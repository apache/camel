...
| *pubsubEndpoint* (producer) | Pub/Sub endpoint to use when message ordering is enabled. Sending messages to the same region ensures they are received in order even when multiple publishers are used |  | String
...
A String set as message header `GooglePubsubConstants.ORDERING_KEY` will be set as the PubSub orderingKey.
