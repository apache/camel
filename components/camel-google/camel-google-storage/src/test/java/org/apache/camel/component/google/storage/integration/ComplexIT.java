        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                ...
                from("google-storage://" + bucket1 + "?serviceAccountKey=file:" + serviceAccountKeyFile
                                ...
                                + "&includeBody=true")
                        .log(
                                "consuming: ${header.CamelGoogleCloudStorageBucketName}/${header.CamelGoogleCloudStorageObjectName}")
                        .to("direct:processed")
                        .to("mock:processed");
                ...
                from("direct:processed")
                        ...
                        .process(exchange -> {
                            exchange.getIn()
                                    .setHeader(
                                            GoogleCloudStorageConstants.DOWNLOAD_LINK_EXPIRATION_TIME,
                                            86400000L); // 1 day
                        })
                        .to("google-storage://" + bucket2 + "?serviceAccountKey=file:" + serviceAccountKeyFile
                                + "&operation=createDownloadLink")
                        .log(
                                "URL for ${header.CamelGoogleCloudStorageBucketName}/${header.CamelGoogleCloudStorageObjectName} =${body}")
                        .to("mock:bucket2");
                ...
                from("timer:timer1?repeatCount=1&fixedRate=true&period=10000")
                        ...
                        .log("splitted: ${body}")
                        .multicast()
                        .to("direct:moreinfo", "direct:copy");
                ...
                from("direct:moreinfo")
                        ...
                        .to("google-storage://" + bucket2 + "?serviceAccountKey=file:" + serviceAccountKeyFile
                                + "&operation=getObject")
                        .log(
                                "get object bucket:${header.CamelGoogleCloudStorageBucketName} object:${header.CamelGoogleCloudStorageObjectName}, body:${body}");
                ...
                from("direct:copy")
                        .process(exchange -> {
                            ...
                            exchange.getIn()
                                    .setHeader(GoogleCloudStorageConstants.DESTINATION_OBJECT_NAME, copyFileName);
                        })
                        .to("google-storage://" + bucket2 + "?serviceAccountKey=file:" + serviceAccountKeyFile
