...
/**
 * Reads a response body with an explicit byte cap, so that a remote peer cannot exhaust the heap by returning an
 * unbounded document.
 */
public final class BoundedInputStreamReader {
    ...
    /**
     * Reads the stream fully, failing as soon as more than {@code maxBytes} have been seen.
     *
     * @param  input       the stream to read
     * @param  maxBytes    the inclusive maximum number of bytes to accept
     * @param  description what is being read, used in the failure message
     * @return             the bytes read
     * @throws IOException if the stream fails, or yields more than {@code maxBytes}
     */
