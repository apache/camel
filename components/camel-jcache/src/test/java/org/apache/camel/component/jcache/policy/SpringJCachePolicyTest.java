...
public class SpringJCachePolicyTest extends CamelSpringTestSupport {
    ...
    public void testXmlDslValueGetsCached() throws Exception {
        ...
        Object responseBody = this.template().requestBody("direct:policy-spring", key);
        ...
        responseBody = this.template().requestBody("direct:policy-spring", key);
    ...
    //Basic test to verify if we call the route with different keys, both gets cached
    @Test
    public void testXmlDslDifferent() throws Exception {
        ...
        //Send first, key is not in cache
        Object responseBody = this.template().requestBody("direct:policy-spring", key1);
        ...
        responseBody = this.template().requestBody("direct:policy-spring", key2);
