...
public class CasperEventHandler implements EventHandler {
	...
	public static final Logger LOG = LoggerFactory.getLogger(CasperEventHandler.class);
	...
	public void onOpen() throws Exception {
		LOG.info("The event stream has been opened");
	}

	@Override
	public void onClosed() throws Exception {
		LOG.info("The event stream has been closed");
	}
	...
	public void onMessage(String evt, MessageEvent messageEvent) throws Exception {
		...
		case BLOCK_ADDED:
			if (firstJsonPropertyKey.equals("BlockAdded")) {
				LOG.debug("received event of type :  BlockAdded");
		...
		case DEPLOY_PROCESSED:
			if (firstJsonPropertyKey.equals("DeployProcessed")) {
				LOG.debug("received event of type :  DeployProcessed");
		...
		case DEPLOY_ACCEPTED:
			if (firstJsonPropertyKey.equals("DeployAccepted")) {
				LOG.debug("received event of type :  DeployAccepted");
		...
		case FINALITY_SIGNATURE:
			if (firstJsonPropertyKey.equals("FinalitySignature")) {
				LOG.debug("received event of type :  FinalitySignature");
		...
		case STEP:
			if (firstJsonPropertyKey.equals("Step")) {
				LOG.debug("received event of type :  Step");
		...
		case FAULT:
			if (firstJsonPropertyKey.equals("Fault")) {
				LOG.debug("received event of type :  Fault");
		...
		case DEPLOY_EXPIRED:
			if (firstJsonPropertyKey.equals("DeployExpired")) {
				LOG.debug("received event of type :  DeployExpired");
	...
	public void onError(Throwable t) {
		LOG.error("an error occured when connecting to the event stream", t);
	}
	...
	private void processMessage(ConsumerEvent event, JSONObject data) {
		LOG.debug("processing message for event: {}", event);
