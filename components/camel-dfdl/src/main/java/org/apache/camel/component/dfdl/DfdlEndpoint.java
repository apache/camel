...
public class DfdlEndpoint extends ProcessorEndpoint {
    ...
    @UriParam(description = "The root element of the schema to use. If not specified, the first root element in the schema will be used.",
              defaultValue = "")
    private String rootElement = "";

    @UriParam(description = "The root namespace of the schema to use.", defaultValue = "")
    ...
    protected void onExchange(Exchange exchange) throws Exception {
        if (getParseDirection() == ParseDirection.UNPARSE) {
            ...
            UnparseResult result = getDaffodilProcessor().unparse(inputter, Channels.newChannel(bos));
            if (result.isError()) {
            ...
            ParseResult result = getDaffodilProcessor().parse(inputStream, outputter);
            if (result.isError()) {
    ...
    public DataProcessor getDaffodilProcessor() throws IOException {
        if (daffodilProcessor == null) {
            ...
            if (getRootElement() != null && !getRootElement().isEmpty() &&
                ...
                processorFactory = Daffodil.compiler().compileResource(getSchemaUri());
            }
            ...
            daffodilProcessor = processorFactory.onPath("/");
        }
        return daffodilProcessor;
    }
