...
class InfinispanStateStorePropertiesIT extends CamelMainTestSupport {
    ...
    private RemoteCacheManager testCacheManager;

    @AfterEach
    void cleanUp() {
        if (testCacheManager != null) {
            testCacheManager.stop();
    ...
    private RemoteCacheManager createCacheManager() {
        ...
        testCacheManager = new RemoteCacheManager(builder.build(), true);
    ...
    void testBackendConfiguredViaProperties() {
        ...
        assertThat(backend).isNotNull().isInstanceOf(InfinispanStateStoreBackend.class);
        assertThat(((InfinispanStateStoreBackend) backend).getCacheName()).isEqualTo("state-store-props-test");
    }
