...
|===
| Name | Description | Default | Type
| *artifactId* (producer) | The artifact id identifies Zoo inference model |  | String
| *model* (producer) | Custom Deep Learning Model bean id. Bean should implement ai.djl.Model |  | String
| *translator* (producer) | Custom Translator for pre-processing and postprocessing bean id. Bean should implement ai.djl.translate.Translator |  | String
|===
