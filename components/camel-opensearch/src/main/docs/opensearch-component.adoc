...
DeleteIndex operation using a custom OpenSearchClient that uses AWS SDK transport. When both restClient and openSearchClient are present in registry, openSearchClient takes precedence over restClient.
...
OpenSearchTransport transport = new AwsSdk2Transport(httpClient, host, Region.of(region),AwsSdk2TransportOptions.builder().setCredentials(buildCredentialsProvider()).setMapper(new JacksonJsonpMapper(mapper)).build());
