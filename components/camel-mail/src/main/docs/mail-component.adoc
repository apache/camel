...
[source,java]
----
.to("smtp://relay@mailhost?to=safe@mycompany.com&from=noreply@mycompany.com"
  + "&useHeaderRecipients=false&useHeaderFrom=false");
----

NOTE: These options do *not* strip the headers from the exchange. See the xref:manual::security-model.adoc#_deployment_hardening[Camel Security Model]
