...
public abstract class DefaultSqlEndpoint extends DefaultPollingEndpoint implements EndpointServiceLocation {
    ...
    @UriParam(
            label = "consumer",
            description =
                    "After processing each row then this query can be executed, if the Exchange was processed successfully, for example to mark the row as processed. The query can have"
                            + " parameter.")
    ...
    @UriParam(
            label = "advanced",
            description =
                    "If set greater than zero, then Camel will use this count value of parameters to replace instead of"
    ...
    @UriParam(
            description =
                    "Store the query result in a header instead of the message body. By default, outputHeader == null and the query result is stored"
    ...
    @UriParam(label = "advanced", description = "Factory for creating RowMapper")
    ...
    public ResultSetIterator queryForStreamList(Connection connection, Statement statement, ResultSet rs)
            throws SQLException {
        if (outputClass == null) {
            RowMapper<?> rowMapper = rowMapperFactory.newColumnRowMapper();
