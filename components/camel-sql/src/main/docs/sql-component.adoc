...
(e.g. by a vault provider), the component evicts stale connections from the component-level
DataSource's connection pool so that new connections are created with the updated credentials.

Currently only HikariCP pools are supported for active eviction via `softEvictConnections()`.
Other pool implementations will pick up the new credentials when existing connections expire naturally.
