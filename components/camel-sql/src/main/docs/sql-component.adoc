...
A query that spells a named parameter without the `#`, as `:name` or `:${body.name}`, has no named parameter in
it as far as Camel is concerned: it is sent to the database as it stands, and the database rejects it with a
complaint about its own syntax. Camel logs a warning naming the offending text and the supported form when the
endpoint is created.
