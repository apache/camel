...
| *queryTimeout* (producer) | Sets the query timeout property on statements used for query processing. This will be overridden by the remaining transaction timeout when executing within a transaction that has a timeout specified at the transaction level. | -1 | int
|===
...
=== Query Timeout
Sets the query timeout property on statements used for query processing. This will be overridden by the remaining transaction timeout when executing within a transaction that has a timeout specified at the transaction level.
