        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:testHealthCheck")
                        .to("aws2-msk://test?operation=listClusters&region=l&useDefaultCredentialsProvider=true");
            }
        };
    }
    ...
    public void testConnectivity() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            boolean containsAws2AthenaHealthCheck =
                    res2.stream().anyMatch(result -> result.getCheck().getId().startsWith("producer:aws2-msk"));
            boolean hasRegionMessage =
                    res2.stream().anyMatch(r -> r.getMessage().stream().anyMatch(msg -> msg.contains("region")));
            Assertions.assertTrue(down, "liveness check");
            Assertions.assertTrue(containsAws2AthenaHealthCheck, "aws2-msk check");
            Assertions.assertTrue(hasRegionMessage, "aws2-msk check error message");
        });
    }
}
