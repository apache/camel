...
public class RedshiftData2ProducerHealthCheckStaticCredsTest extends CamelTestSupport {
        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:listDatabases")
                        .to("aws2-redshiftdata://test?operation=listDatabases&region=l&secretKey=l&accessKey=k");
            }
        };
    }
    ...
    public void testConnectivity() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            boolean containsAws2RedshiftDataHealthCheck = res2.stream()
                    .anyMatch(result -> result.getCheck().getId().startsWith("producer:aws2-redshiftdata"));
            boolean hasRegionMessage = res2.stream()
                    .anyMatch(r -> r.getMessage().stream().anyMatch(msg -> msg.contains("region")));
            Assertions.assertTrue(down, "liveness check");
            Assertions.assertTrue(containsAws2RedshiftDataHealthCheck, "aws2-redshiftdata check");
            Assertions.assertTrue(hasRegionMessage, "aws2-redshiftdata check error message");
        });
