...
The operation must match the model type. Text-generation models are used with `invokeTextModel`, image-generation
models (for example `amazon.titan-image-generator-v1`, `amazon.nova-canvas-v1:0`, the `stability.*` models) with
`invokeImageModel`, and embedding models (for example `amazon.titan-embed-text-v1`, `cohere.embed-english-v3`) with
`invokeEmbeddingsModel`. Invoking `invokeTextModel` with an image, embedding, rerank or video/speech model fails fast
with an `IllegalArgumentException` that names the model and points to the operation to use instead.
