...
=== Reporting the AWS credentials source

Set `useAutoDetectCredentialsProvider=true` to have the component detect which credentials source applies to the
runtime, select the matching AWS credentials provider, and log the detected source at INFO:

* environment variables (`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`)
* web identity / IRSA (`AWS_WEB_IDENTITY_TOKEN_FILE`) - requires `software.amazon.awssdk:sts` on the classpath
* shared profile (`AWS_PROFILE` or `~/.aws/credentials`)
...
from the SDK - the value it adds is making the resolved source visible in the logs. When no source is recognised
(including a plain EC2 instance, whose metadata is deliberately not probed here) it falls back to the SDK default
credentials provider chain. The option is opt-in and takes precedence over the other credential options.
