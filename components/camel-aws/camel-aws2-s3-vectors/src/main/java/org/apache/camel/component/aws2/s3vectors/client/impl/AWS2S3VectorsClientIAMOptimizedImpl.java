...
/**
 * Manage an AWS S3 Vectors client for all users to use (enabling temporary creds). This implementation is for remote
 * instances to manage the credentials on their own (eliminating credential rotations)
 */
public class AWS2S3VectorsClientIAMOptimizedImpl implements AWS2CamelS3VectorsInternalClient {
    private static final Logger LOG = LoggerFactory.getLogger(AWS2S3VectorsClientIAMOptimizedImpl.class);
    private AWS2S3VectorsConfiguration configuration;

    /**
     * Constructor that uses the config file.
     */
    public AWS2S3VectorsClientIAMOptimizedImpl(AWS2S3VectorsConfiguration configuration) {
        LOG.trace("Creating an AWS S3 Vectors client for an ec2 instance with IAM temporary credentials (normal for ec2s).");
