...
public class AWS2S3VectorsComponent extends HealthCheckComponent {
    @Metadata
    private AWS2S3VectorsConfiguration configuration = new AWS2S3VectorsConfiguration();

    public AWS2S3VectorsComponent() {
        this(null);
    }

    public AWS2S3VectorsComponent(CamelContext context) {
        super(context);
    }
    ...
    protected Endpoint createEndpoint(String uri, String remaining, Map<String, Object> parameters) throws Exception {
        ...
        final AWS2S3VectorsConfiguration configuration
                = ObjectHelper.isNotEmpty(this.configuration) ? this.configuration.copy() : new AWS2S3VectorsConfiguration();
        configuration.setVectorBucketName(remaining);
        AWS2S3VectorsEndpoint endpoint = new AWS2S3VectorsEndpoint(uri, this, configuration);
        ...
        if (Boolean.FALSE.equals(configuration.isUseDefaultCredentialsProvider())
                ...
                && configuration.getS3VectorsClient() == null
                && (configuration.getAccessKey() == null || configuration.getSecretKey() == null)) {
            throw new IllegalArgumentException(
                    "useDefaultCredentialsProvider is set to false, useProfileCredentialsProvider is set to false, useSessionCredentials is set to false, S3VectorsClient or accessKey and secretKey must be specified");
        }
