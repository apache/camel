...
The vector in the message body is a `List` of numbers, a `float[]` or a `double[]`.
The YAML DSL has no way to write such a list as a constant, so the YAML examples build it with a Groovy expression
