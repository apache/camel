        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:listDatabases")
                        .to("aws2-redshift-data://test?operation=listDatabases&region=l&useDefaultCredentialsProvider=true");
            }
        };
    }
    ...
    public void testConnectivity() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            boolean containsAws2RedshiftDataHealthCheck = res2.stream()
                    .anyMatch(result -> result.getCheck().getId().startsWith("producer:aws2-redshift-data"));
            boolean hasRegionMessage = res2.stream()
                    .anyMatch(r -> r.getMessage().stream().anyMatch(msg -> msg.contains("region")));
            Assertions.assertTrue(down, "liveness check");
            Assertions.assertTrue(containsAws2RedshiftDataHealthCheck, "aws2-redshift-data check");
            Assertions.assertTrue(hasRegionMessage, "aws2-redshift-data check error message");
        });
