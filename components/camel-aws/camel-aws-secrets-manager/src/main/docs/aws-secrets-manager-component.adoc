...
| *operation* (producer) | *Required* The operation to perform. There are 7 enums and the value can be one of: listSecrets, createSecret, getSecret, describeSecret, deleteSecret, rotateSecret, updateSecret |  | SecretsManagerOperations
