...
public class AWS2BatchExecuteStatementRuleIT extends Aws2DDBBase {

    @EndpointInject("direct:start")
    private ProducerTemplate template;

    private final String attributeName = "clave";
    private final String tableName = "TestTableBatchPartiQL";

    @Test
    public void batchExecuteStatement() {
        List<BatchStatementRequest> statements = Arrays.asList(
                BatchStatementRequest.builder()
                        .statement("INSERT INTO \"" + tableName + "\" VALUE {'" + attributeName + "': 'key1', 'data': 'val1'}")
                        .build(),
                BatchStatementRequest.builder()
                        .statement("INSERT INTO \"" + tableName + "\" VALUE {'" + attributeName + "': 'key2', 'data': 'val2'}")
                        .build());
        ...
        Exchange result = template.send("direct:start", new Processor() {
            public void process(Exchange exchange) {
                ...
                exchange.getIn().setHeader(Ddb2Constants.BATCH_STATEMENTS, statements);
            }
        });

        @SuppressWarnings("unchecked")
        List<BatchStatementResponse> responses
                = result.getIn().getHeader(Ddb2Constants.BATCH_STATEMENT_RESPONSE, List.class);
