...
public class Rekognition2ProducerHealthCheckProfileCredsTest extends CamelTestSupport {
        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:listCollections")
                        .to("aws2-rekognition://test?operation=listCollections&region=l&useDefaultCredentialsProvider=true");
            }
        };
    }
    ...
    public void testConnectivity() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            boolean containsAws2RekognitionHealthCheck = res2.stream()
                    .anyMatch(result -> result.getCheck().getId().startsWith("producer:aws2-rekognition"));
            boolean hasRegionMessage = res2.stream()
                    .anyMatch(r -> r.getMessage().stream().anyMatch(msg -> msg.contains("region")));
            Assertions.assertTrue(down, "liveness check");
            Assertions.assertTrue(containsAws2RekognitionHealthCheck, "aws2-rekognition check");
            Assertions.assertTrue(hasRegionMessage, "aws2-rekognition check error message");
        });
