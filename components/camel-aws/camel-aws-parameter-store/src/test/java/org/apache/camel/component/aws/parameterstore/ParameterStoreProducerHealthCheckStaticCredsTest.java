...
public class ParameterStoreProducerHealthCheckStaticCredsTest extends CamelTestSupport {
        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                from("direct:describeParameters")
                        .to("aws-parameter-store://test?operation=describeParameters&region=l&secretKey=l&accessKey=k");
            }
        };
    }
    ...
    public void testConnectivity() {
        ...
        await().atMost(20, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            boolean containsAwsParameterStoreHealthCheck = res2.stream()
                    .anyMatch(result -> result.getCheck().getId().startsWith("producer:aws-parameter-store"));
            boolean hasRegionMessage = res2.stream()
                    .anyMatch(r -> r.getMessage().stream().anyMatch(msg -> msg.contains("region")));
            Assertions.assertTrue(down, "liveness check");
            Assertions.assertTrue(containsAwsParameterStoreHealthCheck, "aws-parameter-store check");
            Assertions.assertTrue(hasRegionMessage, "aws-parameter-store check error message");
        });
