...
|`sendMessage`| Sends a message with a given time-to-live and a timeout period where the message is invisible in the queue.
...
=== Consumer Examples
To consume a queue into a file component, this can be done like this:
