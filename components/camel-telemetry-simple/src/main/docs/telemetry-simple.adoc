= Temetry Simple Component
:doctitle: Temetry Simple
:shortname: telemetry-simple
:artifactid: camel-telemetry-simple
:description: Basic implementation of Camel Telemetry
:since: 4.11
...
|`camel.telemetrySimple.enabled`| false | Turn the tracing on/off.
|`camel.telemetrySimple.traceFormat`| default | The format used to trace in the log (default, tree, json).
|`camel.telemetrySimple.traceProcessors`| false | Trace inner processors.
|`camel.telemetrySimple.excludePatterns` |  | Sets exclude pattern(s) that will disable tracing for Camel
