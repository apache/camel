...
@JdkService("telemetry-simple-tracer")
@Configurer
@ManagedResource(description = "TelemetrySimpleTracer")
public class TelemetrySimpleTracer extends Tracer {

    private static final Logger LOG = LoggerFactory.getLogger(TelemetrySimpleTracer.class);
    ...
    protected void initTracer() {
        this.setSpanLifecycleManager(new SimpleSpanLifecycleManager(traceFormat));
    }
    ...
    protected void doStart() throws Exception {
        super.doStart();
        LOG.warn(
                "TelemetrySimpleTracer enabled. This is a simple tracer and you should avoid using it in production workflows.");
    }

    private class SimpleSpanLifecycleManager implements SpanLifecycleManager {

        private final InMemoryCollector inMemoryCollector = new InMemoryCollector();
        private SimpleTraceFormat stf;

        private SimpleSpanLifecycleManager(String traceFormat) {
            if (traceFormat == null) {
                traceFormat = "default";
            }
            switch (traceFormat.toLowerCase()) {
                case "tree":
                    this.stf = new SimpleTraceFormatTree();
                    break;
                case "json":
                    this.stf = new SimpleTraceFormatJson();
                    break;
                case "default":
                    this.stf = new SimpleTraceFormatDefault();
                    break;
                default:
        ...
        public Span create(String spanName, Span parent, SpanContextPropagationExtractor extractor) {
            Span span = SimpleSpanAdapter.buildSpan(spanName);
        ...
        public void activate(Span span) {
            ...
            SimpleSpanAdapter ssa = (SimpleSpanAdapter) span;
        ...
        public void close(Span span) {
            ...
            SimpleSpanAdapter ssa = (SimpleSpanAdapter) span;
            SimpleTrace trace = inMemoryCollector.get(ssa.getTag("traceid"));
            if (trace != null) {
