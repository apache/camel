...
queue or message queue on a remote system. For every entry on the queue, 
a new `Exchange` is sent with the entry's data in the _In_ message's body. 
For message queues, the data is always formatted as a `String`. For data
queues, it is formatted either as a `String` or a `byte[]`, depending on
the format.
