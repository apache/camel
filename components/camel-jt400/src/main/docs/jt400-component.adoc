...
| *bridgeErrorHandler* (consumer) | Allows for bridging the consumer to the Camel routing Error Handler, which mean any exceptions occurred while the consumer is trying to pickup incoming messages, or the likes, will now be processed as a message and handled by the routing Error Handler. By default the consumer will use the org.apache.camel.spi.ExceptionHandler to deal with exceptions, that will be logged at WARN or ERROR level and ignored. | false | boolean
| *messageAction* (consumer) | Action to be taken on messages when read from a message queue. The value can be one of: OLD (mark as old), REMOVE (remove from the queue), SAME (neither remove nor mark as old) | OLD | MessageAction
...
When configured as a data queue consumer endpoint, the endpoint will poll
a data queue or message queue on a remote system. For every entry on the
queue, a new `Exchange` is sent with the entry's data in the _In_ message's
body. For message queues, the data is always formatted as a `String`. For
data queues, it is formatted either as a `String` or a `byte[]`, depending on
the format.

For a message queue provider endpoint, the _In_ message body contents are
presumed to be text and sent to the queue as an impromptu message. For a
data queue provider endpoint, the _In_ message body contents will be put on
...
|===
| Header constant                          | Header value                      | Type    | Description
| Jt400Endpoint.MESSAGE_ID                 | "jt400.MESSAGE_ID"                | String  | The message identifier
| Jt400Endpoint.MESSAGE_FILE               | "jt400.MESSAGE_FILE"              | String  | The message file name
| Jt400Endpoint.MESSAGE_TYPE               | "jt400.MESSAGE_TYPE               | Integer | The message type (corresponds to constants defined in the AS400Message class)
