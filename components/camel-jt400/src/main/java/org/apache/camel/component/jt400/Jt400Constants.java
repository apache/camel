...
public interface Jt400Constants {
    ...
    @Metadata(
            label = "consumer",
            description =
                    "*Data queues:* Returns the sender information for this data queue entry, or an empty string if not available."
    ...
    @Metadata(
            label = "consumer",
            description = "The message type (corresponds to constants defined in the AS400Message class)",
            javaType = "Integer")
    ...
    @Metadata(
            label = "consumer",
            description = "The default message reply, when the message is an inquiry message",
            javaType = "String")
    String MESSAGE_DFT_RPY = "CamelJt400MessageDefaultReply";

    @Metadata(
            description =
                    "*Consumer:* The key of the message that will be replied to (if the `sendingReply` parameter is set to `true`). "
