...
/**
 * SFTP consumer using Apache MINA SSHD.
 */
public class MinaSftpConsumer extends RemoteFileConsumer<SftpRemoteFile> {

    private static final Logger LOG = LoggerFactory.getLogger(MinaSftpConsumer.class);

    private String endpointPath;

    private transient String sftpConsumerToString;

    public MinaSftpConsumer(RemoteFileEndpoint<SftpRemoteFile> endpoint, Processor processor,
    ...
    protected boolean pollSubDirectory(
            Exchange dynamic, String absolutePath, String dirName, List<GenericFile<SftpRemoteFile>> fileList, int depth) {
    ...
    protected boolean ignoreCannotRetrieveFile(String name, Exchange exchange, Exception cause) {
        if (getEndpoint().getConfiguration().isIgnoreFileNotFoundOrPermissionError()) {
            ...
            if (sftp != null) {
                return sftp.getStatus() == SftpConstants.SSH_FX_NO_SUCH_FILE
    ...
    protected void updateFileHeaders(GenericFile<SftpRemoteFile> file, Message message) {
        SftpRemoteFile sftpFile = file.getFile();
        if (sftpFile == null) {
            return;
        }
        Object rf = sftpFile.getRemoteFile();
        if (rf != null) {
            SftpClient.DirEntry e = (SftpClient.DirEntry) rf;
            long length = e.getAttributes().getSize();
            long modified = e.getAttributes().getModifyTime() != null
                    ? e.getAttributes().getModifyTime().toMillis()
                    : 0L;
    ...
    public String toString() {
        if (sftpConsumerToString == null) {
            sftpConsumerToString = "MinaSftpConsumer[" + URISupport.sanitizeUri(getEndpoint().getEndpointUri()) + "]";
        }
