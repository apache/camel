...
/**
 * SFTP Component using Apache MINA SSHD.
 */
@Component("mina-sftp")
@ManagedResource(description = "Managed MINA SFTP Component")
public class MinaSftpComponent extends RemoteFileComponent<SftpRemoteFile> {

    public MinaSftpComponent() {
    }

    public MinaSftpComponent(CamelContext context) {
        super(context);
    }
    ...
    protected GenericFileEndpoint<SftpRemoteFile> buildFileEndpoint(
        ...
        MinaSftpConfiguration config = new MinaSftpConfiguration(new URI(baseUri));

        FtpUtils.ensureRelativeFtpDirectory(this, config);

        return new MinaSftpEndpoint(uri, this, config);
    }
