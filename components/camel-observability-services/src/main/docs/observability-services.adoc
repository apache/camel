...
You can even change `camel.metrics.logMetricsOnShutdown` and `camel.metrics.logMetricsOnShutdownFilters` in order to qualify better this behavior, which defaults to `true` and exchanges, application and jvm memory metrics by default.
...
provided in xref:others:opentelemetry2.adoc#OpenTelemetry-JavaAgent[OpenTelemetry Java Agent configuration].
