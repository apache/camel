...
public class CompositeApiIntegrationTest extends AbstractSalesforceTestBase {
    ...
    public CompositeApiIntegrationTest(final String format, final String version) {
        ...
        batchuri = "salesforce:composite?format=" + format;
    }
    ...
    public void shouldSupportObjectCreation() {
        ...
        assertNotNull(((HashMap<?, ?>)response.getCompositeResponse().get(0).getBody()).get("id"));
    }
    ...
    public void shouldSupportObjectRetrieval() {
        ...
        assertEquals("Composite API Batch", ((HashMap<?, ?>)response.getCompositeResponse().get(0).getBody()).get("Name"));
    }
    ...
    public void shouldSupportQuery() {
        ...
        assertNotNull(((HashMap<?, ?>)response.getCompositeResponse().get(0).getBody()).get("totalSize"));
    }
    ...
    public void shouldSupportRelatedObjectRetrieval() throws IOException {
        ...
        assertEquals(loginConfig.getUserName(), ((HashMap<?, ?>)response.getCompositeResponse().get(0).getBody()).get("Username"));
    }
    ...
    SObjectCompositeResponse testComposite(final SObjectComposite batch) {
        final SObjectCompositeResponse response = template.requestBody(batchuri, batch, SObjectCompositeResponse.class);

        assertNotNull("Response should be provided", response);

        assertTrue("Received errors in: " + response, response.getCompositeResponse().stream()
