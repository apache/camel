...
* *bulk2GetQueryJobResults* - Get bulk query job results. `jobId` parameter is required. Accepts
`maxRecords` and `locator` parameters. Response message headers include `Sforce-NumberOfRecords` and
