...
[IMPORTANT]
====
Starting with Camel 4.14.8, when these parameters are supplied as *message headers*, the header names
...
the value change. See the Camel 4.14 upgrade guide for the full list of renamed header names.
====
...
When using the <<MavenPlugin,Camel Salesforce Maven Plugin>>, binary fields are automatically generated for blob fields:
