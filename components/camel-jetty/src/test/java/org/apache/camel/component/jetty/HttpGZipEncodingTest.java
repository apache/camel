...
public class HttpGZipEncodingTest extends BaseJettyTest {
    ...
    public void testHttpProducerWithGzip() {
        String response = template.requestBodyAndHeader(
                "http://localhost:" + port1 + "/gzip?httpClientConfigurer=#configurer",
    ...
    public void testGzipProxy() {
        String response = template.requestBodyAndHeader(
                "http://localhost:" + port2 + "/route?httpClientConfigurer=#configurer",
                new ByteArrayInputStream("<Hello>World</Hello>".getBytes()),
                Exchange.CONTENT_ENCODING,
                "gzip",
                String.class);
    ...
    public void testGzipGet() {
        String response = template.requestBodyAndHeader(
                "http://localhost:" + port1 + "/gzip", null, "Accept-Encoding", "gzip", String.class);
        ...
        return new RouteBuilder() {
            public void configure() {
                ...
                from("direct:gzip")
                        ...
                        .to("http://localhost:" + port1 + "/gzip?httpClientConfigurer=#configurer")
                        .unmarshal()
                        .gzipDeflater();
