...
NOTE: This component does not support JDK 22 or later due to a Hadoop incompatibility (https://issues.apache.org/jira/browse/HADOOP-19486[HADOOP-19486]).
Hadoop relies on the deprecated `javax.security.auth.Subject.getSubject()` method, which was removed in JDK 22 (JEP 411).
This limitation will be resolved once an updated Hadoop version with a fix is available.
