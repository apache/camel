 ...
 * <li>Then {@link #signatureValueValidationFailed(SignatureValue)} is called if the signature validation fails.
 * <li>Then, for each reference in the signed info whose validation fails {@link #referenceValidationFailed(Reference)}
 * is called.
 * <li>Then, for each reference in the manifests whose validation fails, the method
 * {@link #manifestReferenceValidationFailed(Reference)} is called.
 * <li>Then, the method {@link #ignoreCoreValidationFailure()} is called where you can finally decide whether the
 * processing should go on or be interrupted.
 * <li>It is ensured that the method {@link #end()} is called at the end of the validation, even if the methods called
 * before have thrown an exception. This allows you to hold state between the start and end of the validation handling
 * process.
 * </ul>
 * If you throw an exception then the validation checking is interrupted and after that only the {@link #end()} method
...
public interface ValidationFailedHandler {
    ...
    /**
     * If <tt>true</tt> is returned then the verifier will go-on as if there was no validation failure. If
     * <tt>false</tt> is returned than the verifier will throw an {@link XmlSignatureInvalidException}.
     * <p>
