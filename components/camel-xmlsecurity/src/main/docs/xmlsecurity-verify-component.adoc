...
----
+
A same-document reference is matched against the document element's own id — an `Id`, `ID` or
`id` attribute, an `xml:id` attribute, or an attribute a DTD or schema declared to be of type ID.
A namespaced id from another convention, notably WS-Security's `wsu:Id`, is deliberately not
matched by attribute name (matching it across any namespace would let an attacker put a matching
id on their wrapper element), so select the signed content with an `outputNodeSearch` for those.
