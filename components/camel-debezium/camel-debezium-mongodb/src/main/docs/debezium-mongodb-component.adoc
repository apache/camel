...
The Debezium MongoDB Connector component supports 57 options, which are listed below.
...
| *mongodbConnectTimeoutMs* (mongodb) | The connection timeout in milliseconds | 10s | int
...
| *mongodbPollIntervalMs* (mongodb) | Frequency in milliseconds to look for new, removed, or changed replica sets. Defaults to 30000 milliseconds. | 30s | long
| *mongodbServerSelectionTimeout{zwsp}Ms* (mongodb) | The server selection timeout in milliseconds | 30s | int
| *mongodbSocketTimeoutMs* (mongodb) | The socket timeout in milliseconds | 0ms | int
