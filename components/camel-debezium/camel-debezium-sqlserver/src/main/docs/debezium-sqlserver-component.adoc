...
| *databaseDbname* (sqlserver) | The name of the database the connector should be monitoring |  | String
...
| *retriableRestartConnectorWait{zwsp}Ms* (sqlserver) | Time to wait before restarting connector after retriable exception occurs. Defaults to 10000ms. | 10s | long
...
| *skippedOperations* (sqlserver) | The comma-separated list of operations to skip during streaming, defined as: 'c' for inserts/create; 'u' for updates; 'd' for deletes. By default, no operations will be skipped. |  | String
...
include::{page-component-version}@camel-spring-boot::page$debezium-sqlserver-starter.adoc[]
