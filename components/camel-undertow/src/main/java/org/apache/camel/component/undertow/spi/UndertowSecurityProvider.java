...
/**
 * SPI interface. Camel-undertow component will locate all available providers and will use first of then which returns
 * true in method acceptConfiguration.
 *
 * To implement this interface you have to advertise SPI class of type UndertowSecurityProvider.
 *
 * Instance of that provider is initialized by call of acceptConfiguration and then will be used to authenticate
 * requests.
 */
public interface UndertowSecurityProvider {
    ...
    /**
     * Method to handle incoming request for security purposes. Method returns status code with result (typically 200 if
     * request is authenticated, 403 if not, some security provider needs to return more specific codes)
     *
    ...
    /**
     * Initialization of securityProvider from configuration. Object passed to camel-undertow as 'securityConfiguration'
    ...
    /**
     * Security provider may need for its functionality also working servlet context. This feature could be used for
     * example in case of execution of servletFilters for security reasons.
     */
