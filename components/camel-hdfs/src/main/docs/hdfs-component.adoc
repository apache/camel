...
In a HA setup, there will be multiple nodes (configured through the *namedNodes* parameter).
The "hostname" and "port" portion of the endpoint uri will no longer have the "host" meaning, but it will represent the name given to the cluster.
You can choose whatever name you want for it (the name should follow [a-zA-Z0-9] convention).
This name will be mapped to the HA filesystem with a coresponding proxy, with failover, and the work:

[source,java]
------------------------------------------------------------------------------------------------------

from("hdfs://cluster1/dir1/dir2?namedNodes=node1:8020,node2:8020").routeId(...)
...
------------------------------------------------------------------------------------------------------
