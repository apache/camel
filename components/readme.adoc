...
`azure-blob:containerOrBlobUri` |  | The azure-blob component is used for storing and retrieving blobs from Azure Storage Blob Service.
...
`kafka:brokers` | 2.13 | The kafka component allows messages to be sent to (or consumed from) Apache Kafka brokers.
...
`olingo2:apiName/methodName` | 2.14 | Communicates with OData 2.0 and 3.0 services using Apache Olingo.
...
| link:camel-servletlistener/src/main/docs/servletlistener.adoc[Servlet Listener] (camel-servletlistener) | 2.11 | Bootstrapping Camel using Servet Listener
