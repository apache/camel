...
You can also provide an optional JSON property `resumeToken` to resume the change stream
from a previously stored resume token:

[source,java]
-------------
from("mongodb:myDb?consumerType=changeStreams&database=flights&collection=tickets&resumeToken={'_data':'8269DE5... }")
    .to("mock:test");
-------------
