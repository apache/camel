...
MongoDB is a very popular NoSQL solution. The camel-mongodb component
...
Since mongo java driver 3, the WriteConcern and readPreference options have not been dynamically modifiable.
They are defined in the mongoClient object:
...
`Bson`., and
operation will return the number of documents matching the criteria.
...
Map, etc.) on the `CamelMongoDbFieldsProjection` header.
...
There is often a requirement to fetch the min/max record from a
...
Insert a new object into the MongoDB collection, taken from the IN
...
OID(s) of the inserted record(s) are stored in the
message header under `CamelMongoOid` key. The value stored is `org.bson.types.ObjectId` for single
...
The Camel insert operation return the document or list of documents inserted. Note that each document is updated by a new OID if needed.
...
1. performing a query to retrieve the entire object first along with all
its fields (may not be efficient), alter it inside Camel and then save
it.
2. using the update operation with
...
Update one or multiple records in the collection. Requires a filter query and
a update rules.

You can define the filter using `CamelMongoDbCriteria` header as `Bson`
...
While defining the filter by using `CamelMongoDbCriteria` header as `Bson`
to query mongodb before you do update, you should notice you need to remove it from the resulting Camel exchange
during aggregation if you use the enrich pattern with an aggregation strategy and then apply mongodb update.
If you don't remove this header during aggregation and/or redefine `CamelMongoDbCriteria` header before sending
...
A header with key `CamelMongoDbRecordsAffected` will be returned with the number of
...
A header with key `CamelMongoDbRecordsAffected` is returned with type `int`,
...
Perform an aggregation with the given pipeline contained in the
body.
...
outputType=MongoIterable. The next Processor will see an iterable in the message body allowing it to step through the results one by one.
Thus, setting a batch size and returning an iterable allows for efficient retrieval and processing of the result.
...
Run the body as a command on the database. Useful for admin operation as
...
`CamelMongoDbOperation` header. The values supported are determined by the MongoDbOperation enumeration
...
for you. You will need to provide the key to some field in your
...
container sometimes. However, your last value would then be lost and
