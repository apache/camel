...
| persistRecords | tail | 100 | int | Sets the number of tailed records after which the tail tracking data is persisted to MongoDB.
...
Here is an example of a Java based DSL route demonstrating how you can use the component to track the oplog
collection. In this specific case we are filtering the events which affect a collection *customers* in
database *optlog_test*:
