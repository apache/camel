...
public Map<String, Boolean> checkMultipleResources(String accessToken, List<String> resourceIds) {
...
onException(AuthorizationDeniedException.class)
    ...
    .transform().constant("{\"error\": \"Permission denied\", \"message\": \"User lacks required permissions\"}");
...
2. **Create Resources**:
   ...
   - Create resources representing protected entities (e.g., "documents", "reports")
