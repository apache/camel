...
the `camel-opentelemetry-starter` dependency, and turn on OpenTelemetry by annotating
...
WARNING: the dependencies used by Camel core are not in sync with the one provided by Spring Boot BOM. If you're using this component be mindful of the https://issues.apache.org/jira/browse/CAMEL-21460[4.8.x Opentelemetry configuration] required.
