...
A compiled class with class-level annotations is handed to the registered `org.apache.camel.spi.CompilePostProcessor`s
with a new instance, as the Java DSL does for `.java` sources. With Camel CLI (`camel run`) that binds a
`@BindToRegistry` class in the Registry and registers a `@Converter` class as type converters (the Spring
and Quarkus annotations are supported as well), so a Groovy source can provide a bean such as a custom
xref:languages:simple-advanced.adoc[simple function]. Plain classes without annotations are not instantiated.
