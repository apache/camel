...
public class ClickUpMockRoutes extends RouteBuilder {
    ...
    public ClickUpMockRoutes addEndpoint(String path, String method, Class<?> returnType, String... responseBodies) {
        this.mocks.put(path, new MockProcessor<>(method, path, returnType, responseBodies));
        return this;
    }

    public ClickUpMockRoutes addErrorEndpoint(String path, String method, int errorCode) {
    ...
    public void configure() {
        ...
        mocks.forEach((path, processor) -> {
            from("netty-http:http://localhost:" + port + "/clickup-api-mock/" + path + "?httpMethodRestrict=" + processor.method)
                    .process(processor);
        });
    ...
    public static class MockProcessor<T> implements Processor {
            ...
            synchronized (lock) {
                ...
                if (errorCode > 0) {
                    ...
                    if (returnType == byte[].class) {
                        ...
                        @SuppressWarnings("unchecked") final T body
        ...
        public List<String> getResponseBodies() {
            synchronized (lock) {
                assert responseBodies != null;

                return Arrays.asList(responseBodies);
            }
        }

        public List<String> awaitResponseBodies(int count, long timeoutMillis) {
            return Awaitility.await()
                    .atMost(timeoutMillis, TimeUnit.MILLISECONDS)
                    .until(this::getResponseBodies, msgs -> msgs.size() >= count);
        }
