...
public class RabbitMQMessageConverter {
    ...
    private AMQP.BasicProperties.Builder buildBasicAmqpProperties(
        ...
        Object priority = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.PRIORITY);
        if (priority != null) {
            properties.priority(Integer.parseInt(priority.toString()));
        }

        Object messageId = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.MESSAGE_ID);
        if (messageId != null) {
            properties.messageId(messageId.toString());
        }

        Object clusterId = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.CLUSTERID);
        if (clusterId != null) {
            properties.clusterId(clusterId.toString());
        }

        Object replyTo = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.REPLY_TO);
        if (replyTo != null) {
            properties.replyTo(replyTo.toString());
        }

        Object correlationId = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.CORRELATIONID);
        if (correlationId != null) {
            properties.correlationId(correlationId.toString());
        }

        Object deliveryMode = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.DELIVERY_MODE);
        if (deliveryMode != null) {
            properties.deliveryMode(Integer.parseInt(deliveryMode.toString()));
        }

        Object userId = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.USERID);
        if (userId != null) {
            properties.userId(userId.toString());
        }

        Object type = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.TYPE);
        if (type != null) {
            properties.type(type.toString());
        }

        Object contentEncoding = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.CONTENT_ENCODING);
        if (contentEncoding != null) {
            properties.contentEncoding(contentEncoding.toString());
        }

        Object expiration = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.EXPIRATION);
        if (expiration != null) {
            properties.expiration(expiration.toString());
        }

        Object appId = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.APP_ID);
        if (appId != null) {
            properties.appId(appId.toString());
        }

        Object timestamp = getBasicAmqpProperty(exchangeProperties, msg, RabbitMQConstants.TIMESTAMP);
        if (timestamp != null) {
