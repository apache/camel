...
public class RabbitMQProducer extends DefaultAsyncProducer {
    ...
    protected boolean processInOut(final Exchange exchange, final AsyncCallback callback) throws Exception {
        ...
        final long timeout
                = exchange.getIn().getHeader(RabbitMQConstants.REQUEST_TIMEOUT.key(), getEndpoint().getRequestTimeout(),
                        long.class);

        final String originalCorrelationId = in.getHeader(RabbitMQConstants.CORRELATIONID.key(), String.class);

        // we append the 'Camel-' prefix to know it was generated by us
        String correlationId
                = GENERATED_CORRELATION_ID_PREFIX + getEndpoint().getCamelContext().getUuidGenerator().generateUuid();
        in.setHeader(RabbitMQConstants.CORRELATIONID.key(), correlationId);

        in.setHeader(RabbitMQConstants.REPLY_TO.key(), replyManager.getReplyTo());

        String exchangeName = (String) exchange.getIn().getHeader(RabbitMQConstants.EXCHANGE_OVERRIDE_NAME.key());
        ...
        String key = in.getHeader(RabbitMQConstants.ROUTING_KEY.key(), String.class);
    ...
    private boolean processInOnly(Exchange exchange, AsyncCallback callback) throws Exception {
        ...
        String key = exchange.getIn().getHeader(RabbitMQConstants.ROUTING_KEY.key(), String.class);
        // we just need to make sure RoutingKey option take effect if it is not
        // BridgeEndpoint
        if (key == null || getEndpoint().isBridgeEndpoint()) {
            key = getEndpoint().getRoutingKey() == null ? "" : getEndpoint().getRoutingKey();
                ...
                try {
                    ...
                    if (getEndpoint().getReplyToType() != null) {
                        ...
                        if (getEndpoint().getReplyTo() != null
                                && getEndpoint().getReplyToType().equals(ReplyToType.Temporary.name())) {
                            throw new IllegalArgumentException(
                                    "ReplyToType " + ReplyToType.Temporary + " is not supported when replyTo "
                                                               + getEndpoint().getReplyTo()
                                                               + " is also configured.");
                        }
                    }
                    ...
                    if (getEndpoint().getReplyTo() != null) {
                        ...
                        throw new IllegalArgumentException(
                                "Specifying replyTo " + getEndpoint().getReplyTo() + " is currently not supported.");
                    } else {
