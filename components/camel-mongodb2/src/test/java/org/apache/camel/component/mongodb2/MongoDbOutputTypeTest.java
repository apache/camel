...
public class MongoDbOutputTypeTest extends AbstractMongoDbTest {
    ...
    public void testFindAllDBCursor() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            assertTrue("Result is not of type DBCursor", result instanceof Iterable);

            Iterable<BasicDBObject> resultCursor = (Iterable<BasicDBObject>) result;
        ...
        try {
            RouteBuilder taillableRouteBuilder = new RouteBuilder() {
                ...
                public void configure() throws Exception {
                    from("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.testCollection}}&operation=findById&dynamicity=true&outputType=DBCursor").to("mock:dummy");
                }
            };
    ...
    public void testInitTailWithWrongOutputType() {
        try {
            RouteBuilder taillableRouteBuilder = new RouteBuilder() {
                @Override
                public void configure() throws Exception {
                    from("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.cappedTestCollection}}&tailTrackIncreasingField=increasing&outputType=DBCursor")
        ...
        return new RouteBuilder() {
            public void configure() {
                ...
                from("direct:findAllDBCursor")
                        .to("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.testCollection}}&operation=findAll&dynamicity=true&outputType=DBCursor")
                        .to("mock:resultFindAllDBCursor");
                from("direct:findAllDBObjectList")
                        .to("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.testCollection}}&operation=findAll&outputType=DBObjectList")
