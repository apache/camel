...
public class MongoDbHeaderHandlingTest extends AbstractMongoDbTest {
    ...
    public void testInHeadersTransferredToOutOnInsert() {
        ...
        assertEquals("An input header was not returned", "def", result.getOut().getHeader("abc"));
        DBObject b = testCollection.findOne(new BasicDBObject("_id", "testInsertString"));
    ...
    public void testWriteResultAsHeaderWithWriteOp() {
        ...
        final DBObject record1 = testCollection.findOne(new BasicDBObject("_id", "testSave1"));
        ...
        DBObject record2 = testCollection.findOne(new BasicDBObject("_id", "testSave1"));
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() {
                ...
                // tested routes
                from("direct:count").to("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.testCollection}}&operation=count&dynamicity=true");
                from("direct:save").to("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.testCollection}}&operation=save&writeConcern=SAFE&writeResultAsHeader=true");
                from("direct:getDbStats").to("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.testCollection}}&operation=getDbStats&writeResultAsHeader=true");

                // supporting routes
                from("direct:insert").to("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.testCollection}}&operation=insert&writeConcern=SAFE");
