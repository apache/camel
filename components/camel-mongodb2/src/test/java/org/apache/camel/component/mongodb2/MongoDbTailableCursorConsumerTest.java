...
public class MongoDbTailableCursorConsumerTest extends AbstractMongoDbTest {
    ...
    public void testNoRecords() throws Exception {
        ...
        DBObject options = BasicDBObjectBuilder.start().add("capped", true).add("size", 1000000000).add("max", 1000).get();
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < 5000; i++) {
                    ...
                    cappedTestCollection.insert(new BasicDBObject("increasing", i).append("string", "value" + i));
                }
    ...
    public void testPersistentTailTrackIncreasingDateField() throws Exception {
        ...
        // create a capped collection with max = 1000
        DBObject option = BasicDBObjectBuilder.start().add("capped", true).add("size", 1000000000).add("max", 1000).get();
        db.createCollection(cappedTestCollectionName,
                option);
        cappedTestCollection = db.getCollection(cappedTestCollectionName);
        addTestRoutes();
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                for (int i = 1; i <= 300; i++) {
                    ...
                    cappedTestCollection.insert(new BasicDBObject("increasing", c.getTime()).append("string", "value" + i));
                }
            }
        });
    ...
    protected void addTestRoutes() throws Exception {
        context.addRoutes(new RouteBuilder() {
            ...
            public void configure() throws Exception {
                ...
                from("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.cappedTestCollection}}&tailTrackIncreasingField=increasing")
                    .id("tailableCursorConsumer1")
                    .autoStartup(false)
                    .to("mock:test");
                
                from("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.cappedTestCollection}}&tailTrackIncreasingField=increasing&persistentTailTracking=true&persistentId=darwin")
                ...
                from("mongodb2:myDb?database={{mongodb.testDb}}&collection={{mongodb.cappedTestCollection}}&tailTrackIncreasingField=increasing&readPreference=primary")
