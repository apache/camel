    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() {
                ...
                from("direct:noWriteConcern").to("mongodb2:myDb?database=test&collection=camelTest&operation=insert");
                from("direct:writeConcernParam").to("mongodb2:myDb?database=test&collection=camelTest&operation=insert&writeConcern=SAFE");
                //from("direct:writeConcernRef").to("mongodb2:myDb?database=test&collection=camelTest&operation=insert&writeConcernRef=customWriteConcern");
                from("direct:noWriteConcernWithCallGetLastError").to("mongodb2:myDb?database=test&collection=camelTest&operation=insert");
