...
|base.path |String |To set up the base path where the REST services are available.
...
which will be `protocol://host:port/base.path`. If the base.path option is not set, context-path will be used.
