...
class HiveMQ311SendDynamicAwareTest {

    private DefaultCamelContext camelContext;
    private HiveMQ311SendDynamicAware sendDynamicAware;

    @BeforeEach
    void setUp() {
        camelContext = new DefaultCamelContext();
        sendDynamicAware = new HiveMQ311SendDynamicAware();
        sendDynamicAware.setCamelContext(camelContext);
        sendDynamicAware.setScheme("hivemq311");
    }

    @Test
    @DisplayName("SendDynamic annotation is present for SPI discovery")
    void testSendDynamicAnnotationPresent() {
        SendDynamic annotation = HiveMQ311SendDynamicAware.class.getAnnotation(SendDynamic.class);
        assertThat(annotation).isNotNull();
        assertThat(annotation.value()).isEqualTo("hivemq311");
    }
    ...
    void testPrepareInjectsHeader() throws Exception {
        ...
        DynamicAwareEntry entry = new DynamicAwareEntry(
                "hivemq311:dynamic/sensors/temperature",
                "hivemq311:${header.topic}",
                null,
                null);
        ...
        assertThat(exchange.getMessage().getHeader(HiveMQ311Constants.OVERRIDE_TOPIC))
    ...
    void testResolveStaticUri() throws Exception {
        Exchange exchange = new DefaultExchange(camelContext);
        DynamicAwareEntry entry = new DynamicAwareEntry(
                "hivemq311:devices/device-42/telemetry?host=localhost&port=1883",
                "hivemq311:devices/${header.deviceId}/telemetry?host=localhost&port=1883",
                null,
                null);

        String staticUri = sendDynamicAware.resolveStaticUri(exchange, entry);

        assertThat(staticUri).isEqualTo("hivemq311:devices/${header.deviceId}/telemetry?host=localhost&port=1883");
    }
}
