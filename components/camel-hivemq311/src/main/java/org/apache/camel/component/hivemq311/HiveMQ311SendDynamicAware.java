 ...
 * names are provided via {@link HiveMQ311Constants#OVERRIDE_TOPIC} so a single static endpoint/producer can service
 * dynamic requests.
 */
@SendDynamic("hivemq311")
public class HiveMQ311SendDynamicAware extends SendDynamicAwareSupport {
    ...
    public Processor createPreProcessor(Exchange exchange, DynamicAwareEntry entry) throws Exception {
        if (exchange.getMessage().getHeader(HiveMQ311Constants.OVERRIDE_TOPIC) != null) {
            return null;
        }
        ...
        return new Processor() {
            ...
            public void process(Exchange exchange) {
                exchange.getMessage().setHeader(HiveMQ311Constants.OVERRIDE_TOPIC, topic);
            }
        };
    }
