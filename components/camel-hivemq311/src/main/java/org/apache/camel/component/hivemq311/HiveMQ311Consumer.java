...
public class HiveMQ311Consumer extends DefaultConsumer {

    private static final Logger LOG = LoggerFactory.getLogger(HiveMQ311Consumer.class);
    private final HiveMQ311Endpoint endpoint;
    private Mqtt3AsyncClient client;
    private ExecutorService executor;

    public HiveMQ311Consumer(HiveMQ311Endpoint endpoint, Processor processor) {
    ...
    protected void doStart() throws Exception {
        super.doStart();
        executor = endpoint.getCamelContext().getExecutorServiceManager().newDefaultThreadPool(this, "HiveMQ311Consumer");
    ...
    private void onMessage(Mqtt3Publish publish) {
        Exchange exchange = createExchange(false);
        exchange.getIn().setBody(publish.getPayloadAsBytes());
        exchange.getIn().setHeader(HiveMQ311Constants.MQTT_TOPIC, publish.getTopic().toString());
        exchange.getIn().setHeader(HiveMQ311Constants.MQTT_QOS, publish.getQos());
        exchange.getIn().setHeader(HiveMQ311Constants.MQTT_RETAINED, publish.isRetain());
