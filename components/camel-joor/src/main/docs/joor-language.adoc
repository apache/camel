...
You can turn off pre-compilation for the jOOR language and then Camel will recompile an external script resource for each message
...
The jOOR language has special support for defining an `org.apache.camel.AggregationStrategy` as a lambda expression.
