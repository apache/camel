...
The Camel Java language allows dependency injection by referring to beans by their id from the Camel registry.
...
When doing this we must use a different name as `myEcho` is the variable used by the dependency injection.
...
You can specify a different location with the `configResource` option on the Java language.
...
You can turn off pre-compilation for the Java language and then Camel will recompile an external script resource for each message
(an inline script is still only compiled once, on first use, as its text cannot change).
...
The Java language has special support for defining an `org.apache.camel.AggregationStrategy` as a lambda expression.
This is useful when using EIP patterns that use aggregation such as the Aggregator, Splitter, Recipient List, Enrich, and others.
...
The Java Camel language is only supported as a block of Java code that gets compiled into a Java class with a single method.
...
It is not supported on any kind of Java Application Server runtime.
