...
:description: Marshal and unmarshal Java lists and maps to/from flat files (such as CSV, delimited, or fixed length formats) using library.
:since: 2.1
:supportLevel: Stable
...
| ignoreFirstRecord | true | Boolean | Whether the first line is ignored for delimited files (for the column headers). <p/> Is by default true.
| textQualifier |  | String | If the text is qualified with a character. <p/> Uses quote character by default.
