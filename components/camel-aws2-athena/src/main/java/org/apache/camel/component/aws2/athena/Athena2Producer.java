...
public class Athena2Producer extends DefaultProducer {
  ...
  private void startQueryExecution(AthenaClient athenaClient, Exchange exchange) {
    ...
    Message message = getMessageForResponse(exchange);
    message.setHeader(Athena2Constants.QUERY_EXECUTION_ID, queryExecutionId);
    message.setHeader(Athena2Constants.QUERY_EXECUTION_STATE, getQueryExecutionResponse == null ? null : getQueryExecutionResponse.queryExecution().status().state());
    message.setHeader(Athena2Constants.OUTPUT_LOCATION, getQueryExecutionResponse == null ? null : getQueryExecutionResponse.queryExecution().resultConfiguration().outputLocation());
  ...
  private StartQueryExecutionResponse doStartQueryExecution(AthenaClient athenaClient, Exchange exchange) {
    ...
    if (ObjectHelper.isNotEmpty(encryptionOption) ||
        ObjectHelper.isNotEmpty(kmsKey)) {
