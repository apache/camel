...
class Athena2QueryHelper {
  ...
  void doWait() {
    try {
      ...
      LOG
          .trace("AWS Athena start query execution wait thread was interrupted; will return at earliest opportunity");
    }
  ...
  private boolean determineResetWaitTimeoutOnRetry(final Exchange exchange, Athena2Configuration configuration) {
    ...
    if (ObjectHelper.isEmpty(resetWaitTimeoutOnRetry)) {
      ...
      LOG.trace("AWS Athena reset wait timeout on retry is missing, using default one [{}]", resetWaitTimeoutOnRetry);
    }
