...
| *operation* (producer) | The operation to do in case the user don't want to send only a record. There are 5 enums and the value can be one of: send-batch-record, create-delivery-stream, delete-delivery-stream, describe-delivery-stream, update-destination |  | KinesisFirehose2Operations
