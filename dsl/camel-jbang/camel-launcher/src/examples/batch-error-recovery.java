...
import org.apache.camel.component.batch.BatchException;
...
// - doTry/doCatch for handling batch abort
//
// Run with:
...
public class batcherrorrecovery extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("timer:recovery?period=8000")
                ...
                .doTry()
                    .to("batch:recoveryJob?steps=direct:primary,direct:recover"
        ...
        from("direct:primary")
                .process(e -> {
                    int index = e.getIn().getHeader("CamelBatchIndex", Integer.class);
                    if (index % 3 == 0) {
        ...
        from("direct:recover")
                .log("  [recover] Retrying failed item ${header.CamelBatchIndex}: ${body}")
                .process(e -> {
