...
@CommandLine.Command(
        name = "set",
        description = "Set/change current Camel version",
        sortOptions = false,
...
public class VersionSet extends CamelCommand {
    ...
    @CommandLine.Option(
            names = {"--repo", "--repos"},
            description =
                    "Maven repository for downloading the dependencies (Use commas to separate multiple repositories)")
    String repo;

    @CommandLine.Option(
            names = {"--reset"},
            description = "Reset by removing any custom version settings")
    boolean reset;
