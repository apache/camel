...
@Command(
        name = "bind",
        description = "Bind source and sink Kamelets as a new Camel integration",
        sortOptions = false,
        showDefaultValues = true)
public class Bind extends CamelCommand {

    @CommandLine.Parameters(
            description = "Name of binding file to be saved",
            arity = "1",
            paramLabel = "<file>",
    ...
    @CommandLine.Option(
            names = {"--source"},
            description = "Source (from) such as a Kamelet or Camel endpoint uri",
            required = true)
    String source;

    @CommandLine.Option(
            names = {"--step"},
            description = "Optional steps such as a Kamelet or Camel endpoint uri")
    String[] steps;

    @CommandLine.Option(
            names = {"--sink"},
            description = "Sink (to) such as a Kamelet or Camel endpoint uri",
            required = true)
    String sink;
    ...
    @CommandLine.Option(
            names = {"--property"},
            description =
                    "Adds a pipe property in the form of [source|sink|error-handler|step-<n>].<key>=<value> where <n> is the step number starting from 1",
            arity = "0")
    String[] properties;
    ...
    public String constructPipe() throws Exception {
        try {
            String sourceEndpoint =
                    resolveEndpoint(BindingProvider.EndpointType.SOURCE, source, getProperties("source"));
            ...
            if (steps != null) {
                ...
                for (int i = 0; i < steps.length; i++) {
                    sb.append(resolveEndpoint(
                            BindingProvider.EndpointType.STEP, steps[i], getProperties("step-%d".formatted(i + 1))));
                ...
                switch (errorHandlerType) {
                    case "sink":
                        ...
                        String endpoint = resolveEndpoint(
                                BindingProvider.EndpointType.ERROR_HANDLER, endpointUri, errorHandlerSinkProperties);
    ...
    private String resolveEndpoint(
        ...
        throw new CamelException("Failed to resolve endpoint URI expression %s - no matching binding provider found"
        ...
        switch (output) {
            case "file":
                if (file.endsWith(".yaml")) {
                    ...
                    IOHelper.writeText(
                            Jsoner.serialize(YamlHelper.yaml().loadAs(pipe, Map.class)),
            ...
            case "json":
                printer()
                        .println(JSonHelper.prettyPrint(
                                        Jsoner.serialize(YamlHelper.yaml().loadAs(pipe, Map.class)), 2)
                                .replaceAll("\\\\/", "/"));
                break;
            default:
