...
/**
 * Store and retrieve objects from Google Cloud Storage Service using the google-cloud-storage
 * library.
 */
public fun UriDsl.`google-storage`(i: GoogleStorageUriDsl.() -> Unit) {
  GoogleStorageUriDsl(this).apply(i)
}

@CamelDslMarker
public class GoogleStorageUriDsl(
  it: UriDsl,
) {
  ...
  init {
    this.it = it
    this.it.component("google-storage")
  }
  ...
  /**
   * The Service account key that can be used as credentials for the Storage client. It can be
   * loaded by default from classpath, but you can prefix with classpath:, file:, or http: to load the
   ...
   * The storage client
   */
  public fun storageClient(storageClient: String) {
    it.property("storageClient", storageClient)
  }
  ...
  public fun storageLocation(storageLocation: String) {
    it.property("storageLocation", storageLocation)
  }

  /**
   * Delete objects from the bucket after they have been retrieved. The delete is only performed if
   * the Exchange is committed. If a rollback occurs, the object is not deleted. If this option is
  ...
  /**
   * The folder or filename to use when downloading the blob. By default, this specifies the folder
   * name, and the name of the file is the blob name. For example, setting this to mydownload will be
   * the same as setting mydownload/${file:name}. You can use dynamic expressions for fine-grained
   * control. For example, you can specify ${date:now:yyyyMMdd}/${file:name} to store the blob in sub
   * folders based on today's day. Only ${file:name} and ${file:name.noext} is supported as dynamic
   * tokens for the blob name.
   */
  public fun downloadFileName(downloadFileName: String) {
    it.property("downloadFileName", downloadFileName)
  }
  ...
  /**
   * If it is true, the Object exchange will be consumed and put into the body. If false the Object
   * stream will be put raw into the body and the headers will be set with the object metadata.
   */
  ...
  /**
   * Move objects from the origin bucket to a different bucket after they have been retrieved. To
   * accomplish the operation the destinationBucket option must be set. The copy bucket operation is
   * only performed if the Exchange is committed. If a rollback occurs, the object is not moved.
   */
