 ...
 */
public fun UriDsl.mybatis(i: MybatisUriDsl.() -> Unit) {
  MybatisUriDsl(this).apply(i)
}

@CamelDslMarker
public class MybatisUriDsl(
  it: UriDsl,
) {
  ...
  init {
    this.it = it
    this.it.component("mybatis")
  }

  private var statement: String = ""

  /**
   * The statement name in the MyBatis XML mapping file which maps to the query, insert, update or
   * delete operation you wish to evaluate.
   */
  ...
  /**
   * This option is intended to split results returned by the database pool into the batches and
   * deliver them in multiple exchanges. This integer defines the maximum messages to deliver in single
   * exchange. By default, no maximum is set. Can be used to set a limit of e.g. 1000 to avoid when
   * starting up the server that there are thousands of files. Set a value of 0 or negative to disable
   * it.
   */
   ...
   */
  public fun statementType(statementType: String) {
    it.property("statementType", statementType)
  }
