...
...), which is what the `camel_catalog_sample` tool of xref:manual::camel-jbang-ai.adoc[Camel JBang AI] returns as
the fallback when the catalog in use has no documentation for the name.
