...
class SetBodyTest extends YamlTestSupport {
    ...
    void setBodyDefinition(Resource resource) throws Exception {
        PluginHelper.getRoutesLoader(context).loadRoutes(resource);
        var setBody = (SetBodyDefinition) context.getRouteDefinitions().get(0).getOutputs().get(0);
        assertThat(setBody.getExpression().getLanguage()).isEqualTo("simple");
        assertThat(setBody.getExpression().getExpression()).isEqualTo("${body}");
    }
}
