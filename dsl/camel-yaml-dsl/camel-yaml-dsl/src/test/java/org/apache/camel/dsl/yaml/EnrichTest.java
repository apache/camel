...
class EnrichTest extends YamlTestSupport {
    ...
    void enrichDefinition(Resource resource) throws Exception {
        PluginHelper.getRoutesLoader(context).loadRoutes(resource);
        var enrich = (EnrichDefinition) context.getRouteDefinitions().get(0).getOutputs().get(0);
        assertThat(enrich.getExpression().getLanguage()).isEqualTo("simple");
        assertThat(enrich.getExpression().getExpression()).isEqualTo("${body}");
    }
}
