...
class MainTest {
    ...
    @Test
    void loadRoutesWithMain() throws Exception {
        Main main = new Main();
        try {
            main.bind("restConsumerFactory", new MockRestConsumerFactory());
            main.configure().withRoutesIncludePattern("routes/ro*.yaml");
            main.start();
            main.getCamelContext().getEndpoint("mock:result", MockEndpoint.class).expectedBodiesReceived("Hello World");
            main.getCamelTemplate().sendBody("direct:start", "Hello World");
            main.getCamelContext().getEndpoint("mock:result", MockEndpoint.class).assertIsSatisfied();
        } finally {
            main.stop();
        }
    }
