...
// camel-4.18.x: keep this link pointing at camel-jbang.adoc. The Camel CLI docs were split into
// separate pages (camel-jbang-beans, camel-jbang-projects, ...) on main only; those pages do not
// exist on this maintenance branch, so re-pointing the link there breaks the docs reference check.
